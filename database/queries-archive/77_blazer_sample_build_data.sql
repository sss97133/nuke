...
DECLARE
    ...
    supplier3_id UUID;
BEGIN
    -- Create suppliers for the build
    INSERT INTO suppliers (name, type, website, user_id) VALUES
    ('Classic Industries', 'vendor', 'https://classicindustries.com', NULL),
    ('Summit Racing', 'marketplace', 'https://summitracing.com', NULL),
    ('Precision Machine Shop', 'shop', NULL, NULL)
    RETURNING id INTO supplier1_id;

    SELECT id INTO supplier2_id FROM suppliers WHERE name = 'Summit Racing';
    SELECT id INTO supplier3_id FROM suppliers WHERE name = 'Precision Machine Shop';

    -- Get category IDs
    SELECT id INTO engine_cat_id FROM part_categories WHERE name = 'Engine';
    SELECT id INTO trans_cat_id FROM part_categories WHERE name = 'Transmission';
    SELECT id INTO susp_cat_id FROM part_categories WHERE name = 'Suspension';
    SELECT id INTO body_cat_id FROM part_categories WHERE name = 'Body';
    ...
    -- Create build phases (invoice periods)
    ...
    SELECT id INTO phase1_id FROM build_phases WHERE build_id = build_id AND phase_number = 1;

    -- Create detailed line items

    -- Engine Components (Phase 1)
    ...
    -- Transmission Components (Phase 1)
    INSERT INTO build_line_items (
        build_id, phase_id, category_id, supplier_id,
        name, description, quantity, unit_price, total_price,
    ...
    -- Add benchmark vehicles for comparison
    ...
    RAISE NOTICE 'Total line items created: %', (SELECT COUNT(*) FROM build_line_items WHERE build_id = build_id);
