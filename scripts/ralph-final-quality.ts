...
async function main() {
  ...
  // Get sample of BaT vehicles using LIKE instead of ILIKE
  const { data: vehicles, error } = await supabase
    .from('vehicles')
    .select('id, year, make, model, vin, mileage, exterior_color, transmission')
  ...
  for (const v of vehicles) {
    ...
    if (v.exterior_color && v.exterior_color.trim()) colorCount++;
  ...
  console.log(`Exterior Color:  ${colorCount.toString().padStart(3)}/${n} (${Math.round(colorCount/n*100)}%)`);
  ...
  for (const v of vehicles.slice(0, 5)) {
    ...
    console.log(`   Miles: ${v.mileage || 'N/A'} | Color: ${v.exterior_color || 'N/A'} | Trans: ${v.transmission || 'N/A'}`);
  }
