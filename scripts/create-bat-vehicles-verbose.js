/**
 * CREATE BAT VEHICLES - VERBOSE MODE
 * Shows detailed progress for each vehicle one at a time
 */
...
console.log('═══════════════════════════════════════════════════════');
console.log('  IMPORTING ALL 55 VIVA BAT LISTINGS');
console.log('  One vehicle at a time with detailed feedback');
console.log('═══════════════════════════════════════════════════════\n');
...
for (let i = 0; i < ALL_55_URLS.length; i++) {
  ...
  const shortName = url.split('/listing/')[1]?.replace('/', '').slice(0, 40) || url;
  
  console.log(`\n┌─ [${i + 1}/${ALL_55_URLS.length}] ────────────────────────────────────`);
  console.log(`│ Listing: ${shortName}`);
  console.log(`│ URL: ${url}`);
  console.log(`└────────────────────────────────────────────────────`);
  
  try {
    console.log('  ⏳ Calling import-bat-listing edge function...');
    ...
    if (result.error) {
      ...
      console.log(`     Details: ${result.listing?.year} ${result.listing?.make} ${result.listing?.model}`);
      console.log(`     Sale Price: $${(result.listing?.salePrice || 0).toLocaleString()}`);
      created++;
    } else {
...
console.log(`  Existing vehicles updated: ${updated}`);
console.log(`  Errors:                    ${errors}`);
console.log(`  Total processed:           ${created + updated}`);
console.log(`  Success rate:              ${((created + updated) / ALL_55_URLS.length * 100).toFixed(1)}%`);
console.log('═══════════════════════════════════════════════════════\n');
console.log('✅ ALL 55 BAT LISTINGS PROCESSED!\n');
