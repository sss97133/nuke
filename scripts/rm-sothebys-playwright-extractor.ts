/**
 * RM Sotheby's Playwright Extractor
 * Handles anti-bot protection with real browser automation
 * Extracts auction lots from current and past sales
 */
...
// From auction-source-extractor.ts
const RMSOTHEBYS_ORG_ID = '5761f2bf-d37f-4b24-aa38-0d8c95ea2ae1';

interface RMSothebysFullData {
  url: string;
  listingId: string;
...
async function extractSaleUrls(page: Page): Promise<string[]> {
  const url = 'https://rmsothebys.com/en/auctions/';
  ...
  const saleUrls = await page.$$eval('a[href*="/en/auctions/"]', links => {
    ...
    for (const link of links) {
      ...
      // Match sale URLs: /en/auctions/{code}
      if (href.match(/\/en\/auctions\/[a-z0-9]+\/?$/i)) {
        seen.add(href);
        const fullUrl = href.startsWith('http')
          ? href
          : `https://rmsothebys.com${href.split('?')[0]}`;
...
async function extractLotUrlsFromSale(page: Page, saleUrl: string): Promise<string[]> {
  ...
  // Try to navigate to lots page
  const lotsLink = await page.$('a[href*="/lots"]');
  if (lotsLink) {
    await lotsLink.click();
    await page.waitForTimeout(2000);
  }

  const lotUrls = await page.$$eval('a[href*="/lots/"]', links => {
    const seen = new Set<string>();
    const results: string[] = [];
    ...
    for (const link of links) {
      ...
      // Match lot URLs: /en/auctions/{code}/lots/{slug}/
      if (href.match(/\/en\/auctions\/[^/]+\/lots\/[^/]+/)) {
        seen.add(href);
        const fullUrl = href.startsWith('http')
          ? href
          : `https://rmsothebys.com${href.split('?')[0]}`;
...
async function extractFullLotData(page: Page, url: string): Promise<RMSothebysFullData | null> {
  try {
    ...
    // Extract sale code and lot number from URL
    // Pattern: /en/auctions/{code}/lots/{slug}/
    const urlMatch = url.match(/\/en\/auctions\/([^/]+)\/lots\/([^/]+)/);
    const saleCode = urlMatch ? urlMatch[1] : null;
    const lotSlug = urlMatch ? urlMatch[2] : null;
    ...
    const data = await page.evaluate(() => {
      ...
      const vinMatch = bodyText.match(/VIN[:\s#]*([A-HJ-NPR-Z0-9]{17})/i) ||
                      bodyText.match(/Chassis[:\s#]*([A-HJ-NPR-Z0-9]{17})/i);
      result.vin = vinMatch?.[1] || null;

      // Extract year/make/model from title
      const titleEl = document.querySelector('h1, .lot-title, [class*="title"]');
      const titleText = titleEl?.textContent || '';

      // RM Sotheby's format is often: "Year Make Model"
      const ymMatch = titleText.match(/(\d{4})\s+([A-Za-z-]+(?:\s+[A-Za-z-]+)?)\s+(.+)/);
      if (ymMatch) {
        result.year = parseInt(ymMatch[1], 10);
        result.make = ymMatch[2].trim();
        result.model = ymMatch[3].split('|')[0].split('\n')[0].trim();
      }

      // Extract estimate
      const estimateMatch = bodyText.match(/Estimate[:\s]*\$?([\d,]+)\s*[-–]\s*\$?([\d,]+)/i);
      if (estimateMatch) {
        result.estimateLow = parseInt(estimateMatch[1].replace(/,/g, ''), 10);
        result.estimateHigh = parseInt(estimateMatch[2].replace(/,/g, ''), 10);
      }
      ...
      } else if (bodyText.includes('Upcoming') || bodyText.includes('Pre-Auction')) {
      ...
      // Extract specs from lot details section
      const specs: Record<string, string> = {};

      // Method 1: Look for specification sections
      document.querySelectorAll('[class*="spec"], [class*="detail"], [class*="info"]').forEach(section => {
        const rows = section.querySelectorAll('dt, dd, tr, .row, [class*="row"]');
        let currentKey = '';
        ...
        rows.forEach(el => {
          ...
          if (text.endsWith(':') || text.length < 30) {
            currentKey = text.replace(':', '').toLowerCase();
          } else if (currentKey) {
            specs[currentKey] = text;
            currentKey = '';
          }
        });
      });

      // Method 2: Parse from body text for common fields
      const engineMatch = bodyText.match(/Engine[:\s]*([^\n]+)/i);
      if (engineMatch) specs['engine'] = engineMatch[1];

      const transMatch = bodyText.match(/Transmission[:\s]*([^\n]+)/i);
      if (transMatch) specs['transmission'] = transMatch[1];

      const driveMatch = bodyText.match(/(?:Drivetrain|Drive)[:\s]*([^\n]+)/i);
      if (driveMatch) specs['drivetrain'] = driveMatch[1];

      result.engine = specs['engine'] || null;
      result.transmission = specs['transmission'] || null;
      result.drivetrain = specs['drivetrain'] || null;
      result.exteriorColor = specs['exterior color'] || specs['paint'] || null;
      ...
      // Extract history/provenance
      const historyEl = document.querySelector('.history, .provenance, [class*="history"]');
      result.history = historyEl?.textContent?.trim().substring(0, 5000) || null;

      // Extract equipment/features
      result.equipment = [];
      document.querySelectorAll('.equipment li, .features li, [class*="equipment"] li').forEach(li => {
        const text = li.textContent?.trim();
        if (text && text.length > 0 && text.length < 200) {
          result.equipment.push(text);
        }
      });

      // Extract all images - RM Sotheby's uses cdn.rmsothebys.com
      result.imageUrls = Array.from(document.querySelectorAll('img'))
        .map(img => img.src || img.getAttribute('data-src') || img.getAttribute('srcset')?.split(',')[0]?.split(' ')[0])
        .filter(src =>
          src &&
          src.includes('cdn.rmsothebys.com') &&
          !src.includes('logo') &&
          !src.includes('avatar') &&
          !src.includes('icon') &&
    ...
    const images = allImageUrls.map((url, idx) => ({
      url,
      category: 'other' // RM Sotheby's doesn't categorize images in URLs
    }));

    const listingId = `${saleCode}-${lotSlug}`;
...
async function extractGalleryImages(page: Page): Promise<string[]> {
  try {
    ...
    // Check if gallery/lightbox opened
    const galleryOpen = await page.$('.gallery, .lightbox, [class*="gallery"], [class*="lightbox"]');
      ...
      for (let i = 0; i < 100; i++) {
        try {
          const imgSrc = await page.$eval(
            '.gallery img, .lightbox img, [class*="gallery"] img',
            img => img.src
          ).catch(() => null);

          if (imgSrc && !images.includes(imgSrc) && imgSrc.includes('cdn.rmsothebys.com')) {
            images.push(imgSrc);
          }
...
async function saveToDatabase(data: RMSothebysFullData): Promise<string | null> {
  try {
    ...
    const vehicleEventData: any = {
      ...
      source_organization_id: RMSOTHEBYS_ORG_ID,
      ...
      metadata: {
        source: 'rmsothebys_playwright_extractor',
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════════════════════════════╗');
  console.log('║       RM SOTHEBYS PLAYWRIGHT EXTRACTOR                            ║');
  ...
  await page.goto('https://rmsothebys.com', { waitUntil: 'load', timeout: 60000 });
  await waitForCloudflare(page);
  ...
  if (singleUrl) {
    ...
    console.log('Discovering current sales...\n');

    const saleUrls = await extractSaleUrls(page);
    ...
    for (let saleIdx = 0; saleIdx < Math.min(saleUrls.length, 5); saleIdx++) {
      ...
      for (let i = 0; i < Math.min(lotUrls.length, 20); i++) {
        ...
        const lotMatch = lotUrl.match(/\/lots\/([^/]+)/);
        const name = lotMatch ? lotMatch[1].replace(/-/g, ' ').substring(0, 40) : 'Unknown';

        process.stdout.write(`[${i + 1}/${Math.min(lotUrls.length, 20)}] ${name}... `);
        ...
        if (data) {
          ...
          if (vehicleId) {
            ...
            console.log(`✓ VIN:${data.vin ? '✓' : '✗'} $${data.soldPrice?.toLocaleString() || data.estimate.low?.toLocaleString() || 'N/A'} ${data.images.length}img`);
          } else {
