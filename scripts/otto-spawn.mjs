#!/usr/bin/env node
/**
 * otto-spawn — multi-agent parallel task executor
 *
 ...
 *
 * Usage:
 *   dotenvx run -- node scripts/otto-spawn.mjs
 *   dotenvx run -- node scripts/otto-spawn.mjs --concurrency 8
 *   dotenvx run -- node scripts/otto-spawn.mjs --agent vp-extraction
 *   dotenvx run -- node scripts/otto-spawn.mjs --dry-run
 *   dotenvx run -- node scripts/otto-spawn.mjs --list
 */
...
async function runAgent(task) {
  ...
  const sessionId = `otto-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
...
async function main() {
  if (!process.env.VITE_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error(`${C.red}Missing env vars. Run with: dotenvx run -- node scripts/otto-spawn.mjs${C.reset}`);
    process.exit(1);
  }

  console.log(`\n${C.bold}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${C.reset}`);
  console.log(`${C.bold}  otto-spawn${C.reset}  ${C.dim}multi-agent executor${C.reset}`);
