#!/usr/bin/env node
/**
 * Extract Broad Arrow inventory in batches
 * Runs multiple small extractions to avoid timeouts
 */
...
const batchSize = parseInt(process.argv[2]) || 15; // Default 15 per batch
const maxBatches = parseInt(process.argv[3]) || 10; // Default 10 batches = 150 vehicles

async function extractBatch(batchNum) {
  console.log(`\n📦 Batch ${batchNum}: Extracting ${batchSize} vehicles...`);
...
async function main() {
  console.log('🔍 Broad Arrow Auctions - Batch Extraction\n');
  ...
  if (results.errors.length > 0) {
    ...
    results.errors.forEach(err => {
      console.log(`   Batch ${err.batch}: ${err.error?.message || 'Unknown error'}`);
    });
  }
