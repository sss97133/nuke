...
function extractYearRange(text) {
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      const parts = match[0].match(/(\d+)-(\d+)/) || match[0].match(/(\d+)\/(\d+)/) || [];
      const [_, start, end] = parts;
      if (start && end) {
...
async function main() {
  ...
  console.log('\n📄 Step 2: Parsing PDF...');
  const pdfData = await pdfParse.default ? pdfParse.default(pdfBuffer) : pdfParse(pdfBuffer);
  ...
  console.log('   (Processing first 50 pages for initial run)');
  
  const charsPerPage = Math.floor(pdfData.text.length / pdfData.numpages);
  let totalParts = 0;
  const maxPages = Math.min(50, pdfData.numpages);
  ...
  console.log('='.repeat(70));
  console.log('\n💡 To process remaining pages, increase maxPages limit in code');
}
