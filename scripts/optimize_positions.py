...
def optimize_positions(workspace_data, target_coverage, config):
    try:
        ...
        optimized_cameras = optimize_camera_positions(
            model,
            point_cloud,
            cameras,
            target_coverage,
...
def optimize_camera_positions(model, point_cloud, cameras, target_coverage, config):
    optimized_cameras = []
    ...
    for camera in cameras:
        ...
        bounds = [
            (0, dimensions['length']),  # x
            (0, dimensions['width']),   # y
            (0, dimensions['height'])   # z
        ]
        ...
        def objective(pos):
            ...
            # Calculate coverage with current position
            coverage, _ = analyze_coverage(model, point_cloud, [camera], config)
            
            # Return negative coverage (for minimization)
            return -coverage
        ...
        # Update camera with optimized position
        camera['position'] = {
            'x': float(result.x[0]),
            'y': float(result.x[1]),
            'z': float(result.x[2])
...
def main():
    parser = argparse.ArgumentParser(description='Optimize camera positions using SpatialLM')
    ...
    parser.add_argument('--target-coverage', type=float, required=True,
                      help='Target coverage score (0-1)')
    ...
    try:
        ...
        sys.exit(optimize_positions(workspace_data, args.target_coverage, config))
    except Exception as e:
        error = {
            'status': 'error',
            'message': str(e)
