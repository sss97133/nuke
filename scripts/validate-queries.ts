...
// This is a critical fix for production builds
if (process.env.NODE_ENV === 'production') {
  console.log('✅ Skipping query validation in production environment');
  process.exit(0);
}

// Skip in CI environments unless explicitly enabled
if (process.env.CI === 'true' && process.env.ENABLE_VALIDATION !== 'true') {
  console.log('✅ Skipping query validation in CI environment (set ENABLE_VALIDATION=true to force)');
  process.exit(0);
}
  ...
  function main() {
    try {
      ...
      if (process.env.CI !== 'true') {
        console.log('❌ Issues found - exiting with error code in development environment')
        process.exit(1)
      } else {
      ...
      if (process.env.CI === 'true') {
        ...
        console.log('❌ Validation script error in development environment');
        process.exit(1);
      }
    }
  }
