...
CEO (Skylar) cannot demo the platform to client the buyer. Profiles are broken, vehicles don't show, email pipeline is disconnected. **All agents: read your assigned issues below and execute immediately.**
...
**Problem:** CEO receives KSL, BHCC, BaT emails to `alerts-inbox@example.com` but they never reach extraction.
...
**Fix (pick one):**
- Option A: Set up Gmail forwarding in `alerts-inbox@example.com` → `alerts@nuke.ag`
...
**Requirements:** Add public/admin view toggle. Public hides "Incomplete" warnings. CEO needs to share with the buyer.
...
- Gmail for alerts: `alerts-inbox@example.com`
- Target alert address: `alerts@nuke.ag`
- Client demo target: the buyer
