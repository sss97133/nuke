...
**Bug:** Gmail forwarding from `alerts-inbox@example.com` to `alerts@nuke.ag` never set up. Edge functions are built and tested. 0 emails in `contact_inbox`, 0 in `alert_email_log`.
**Fix (one of):**
...
**Fix:** Add public/admin toggle. Public hides "Incomplete profile" warnings, empty sections. CEO needs to share with the buyer.
**Blocks:** Nothing. Start immediately.
...
- Gmail: `alerts-inbox@example.com`
- Alert address: `alerts@nuke.ag`
- Client: the buyer (needs shareable profile link)
