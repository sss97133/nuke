-- Organization activity view: actions-first profile aggregation
-- Aggregates vehicle timeline events (via vehicles.owner_shop_id) and org-native events (licenses).

BEGIN;

-- Drop & recreate view for idempotency
...
-- Helpful index on vehicles.owner_shop_id already created in shops_core.sql
