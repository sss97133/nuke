 ...
 * - Top line: "+3,903 TODAY | +47 THIS HOUR" in big Courier New
 * - Source flow bars: horizontal proportional bars (BAT +135 | FB +1,928 etc)
 * - Make heatmap: mini treemap of top 10 arriving makes
 * - Price bracket bars: 6 bars showing distribution by price
 *
...
interface NewestData {
  ...
  make_ingest: MakeIngest[] | null;
  price_brackets: PriceBracket[] | null;
  total_24h: number;
  total_1h: number;
}
...
const SOURCE_LABELS: Record<string, string> = {
  bat: 'BAT',
  facebook_marketplace: 'FB MKTPLACE',
  bonhams: 'BONHAMS',
  gooding: 'GOODING',
  pcarmarket: 'PCAR',
  hagerty: 'HAGERTY',
...
function sourceLabel(source: string): string {
  return SOURCE_LABELS[source] ?? source.toUpperCase().replace(/_/g, ' ');
}
...
function SourceFlowBars({
  ...
  const maxCount = Math.max(...sources.map((s) => s.count_24h), 1);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', flex: 1, minWidth: 0 }}>
      ...
      {sources.map((s) => {
        const pct = (s.count_24h / maxCount) * 100;
        return (
          <SourceBar
            key={s.source}
            source={s.source}
            count24h={s.count_24h}
            count1h={s.count_1h}
            widthPct={pct}
            onClick={() =>
              onFilter({ sources: [s.source], sort: 'newest' as any })
            }
          />
        );
      })}
...
function MakeTreemap({
  ...
  // Take top 10 for treemap
  const top10 = makes.slice(0, 10);
  const treemapHeight = 120;
  const rects = useMemo(
    () => squarify(top10.map((m) => ({ label: m.make, count: m.count_24h })), 0, 0, containerWidth, treemapHeight),
...
function PriceBracketBars({
  ...
  const maxCnt = Math.max(...brackets.map((b) => b.cnt), 1);
...
export function HeroNewestPanel({ onFilter }: HeroNewestPanelProps) {
  const [data, setData] = useState<NewestData | null>(null);
  const [loading, setLoading] = useState(true);
  const [treemapWidth, setTreemapWidth] = useState(0);
  const [middleRef, setMiddleRef] = useState<HTMLDivElement | null>(null);

  // Measure middle column for treemap
  useEffect(() => {
    if (!middleRef) return;
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (error) {
        console.error('hero_newest RPC error:', error);
        return;
      }
      setData(result as unknown as NewestData);
    } catch (err) {
      console.error('hero_newest fetch error:', err);
    } finally {
      setLoading(false);
    }
  }, []);
