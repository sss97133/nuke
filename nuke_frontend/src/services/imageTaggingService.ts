...
class ImageTaggingService {
  ...
  private static async calculateUserQuality(userId?: string): Promise<number> {
    ...
    try {
      ...
      const { data: profile } = await supabase
        .from('profiles')
        .select('user_type, reputation_score')
        .eq('id', userId)
        .single();
      ...
      if (profile.reputation_score) {
        const reputationMultiplier = Math.min(1.2, Math.max(0.8, profile.reputation_score / 100));
  ...
  static async getTagStats(vehicleId?: string): Promise<{
    ...
    try {
      let query = supabase.from('image_tags').select('source_type, verified, user_id, user_quality_score');
      ...
      const tags = data || [];
      const uniqueUsers = new Set(tags.map(t => t.user_id).filter(Boolean)).size;
      const qualityScores = tags.map(t => t.user_quality_score).filter(s => typeof s === 'number');
