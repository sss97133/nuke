...
const WorkspaceContent: React.FC<WorkspaceContentProps> = ({
  ...
  // Sticky pane top offset = site header + vehicle header
  const stickyTop = `calc(var(--header-height, 40px) + ${vehicleHeaderHeight}px)`;
  const paneHeight = `calc(100vh - var(--header-height, 40px) - ${vehicleHeaderHeight}px)`;

  return (
    <>
      {/* === SECTION 1: Vehicle Info (scrolls away) === */}
      <section className="section" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
              ...
              return (
                <div style={{ fontSize: '13px', lineHeight: '1.8' }}>
                  <div style={{ fontWeight: 600, fontSize: '15px', marginBottom: '4px' }}>
                    ...
                    {confPct != null && (
                      <span style={{
                        marginLeft: '8px', padding: '2px 6px', fontSize: '11px', fontWeight: 400, borderRadius: '2px',
                  ...
                  <div className="text-small" style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '2px 12px' }}>
                    {eba.fuel_system_type && (<><span className="text-muted">Fuel System</span><span>{eba.fuel_system_brand && eba.fuel_system_brand !== 'unknown' ? `${eba.fuel_system_brand} ` : ''}{eba.fuel_system_type.replace(/_/g, ' ')}</span></>)}
                    {eba.ignition_type && (<><span className="text-muted">Ignition</span><span>{eba.ignition_brand && eba.ignition_brand !== 'unknown' ? `${eba.ignition_brand.replace(/_/g, ' ')} ` : ''}{eba.ignition_type.replace(/_/g, ' ')}</span></>)}
                    {eba.headers && eba.headers !== 'unknown' && (<><span className="text-muted">Exhaust</span><span>{eba.headers.replace(/_/g, ' ')}</span></>)}
                    {eba.condition && eba.condition !== 'unknown' && (<><span className="text-muted">Condition</span><span>{eba.condition.replace(/_/g, ' ')}</span></>)}
                    {eba.modifications?.length > 0 && (<><span className="text-muted">Modifications</span><span>{eba.modifications.length} identified</span></>)}
                  </div>
                    ...
                    <div style={{ marginTop: '8px', fontSize: '12px' }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                        {eba.modifications.map((mod: string, i: number) => (
                          <span key={i} style={{ padding: '2px 6px', backgroundColor: 'var(--bg-secondary, rgba(0,0,0,0.05))', border: '1px solid var(--border-color, rgba(0,0,0,0.1))', fontSize: '11px' }}>{mod}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  <div className="text-muted" style={{ marginTop: '8px', fontSize: '11px' }}>
                    Analyzed {eba.analyzed_at ? new Date(eba.analyzed_at).toLocaleDateString() : ''}{eba.analysis_version > 1 ? ` (v${eba.analysis_version})` : ''}
                  </div>
                </div>
              );
            })()}
        ...
        <VehiclePricingValueCard vehicle={vehicle} auctionPulse={auctionPulse} valuationIntel={valuationIntel as any} readinessSnapshot={readinessSnapshot as any} />
        ...
        <CollapsibleWidget title="Auction Quick Start" defaultCollapsed={true}>
          <VehicleAuctionQuickStartCard vehicle={{ id: vehicle.id, year: vehicle.year, make: vehicle.make, model: vehicle.model, trim: (vehicle as any)?.trim ?? null, mileage: (vehicle as any)?.mileage ?? null }} canManage={Boolean(isRowOwner || isVerifiedOwner)} />
        </CollapsibleWidget>
        ...
        {(isRowOwner || isVerifiedOwner) && (
          <>
            <CollapsibleWidget title="Live Streaming" defaultCollapsed={true}>
              <VehicleStreamingCard vehicleId={vehicle.id} vehicleName={getVehicleTitle(vehicle)} session={session} canManage={Boolean(isRowOwner || isVerifiedOwner || hasContributorAccess)} liveSession={liveSession} onSessionUpdated={onLoadLiveSession} />
            </CollapsibleWidget>
          ...
          <div>
            ...
            {dataSources.length === 0 ? (
              ...
              <ul style={{ margin: '0', paddingLeft: '18px' }}>
                {dataSources.map((url) => (
                  <li key={url} className="text-small"><a href={url} target="_blank" rel="noreferrer">{sourceLabel(url)}</a></li>
                ))}
              </ul>
            )}
          </div>
          ...
          <CollapsibleWidget title="Privacy Settings" defaultCollapsed={true} badge={<span className={`text-xs ${isPublic ? 'text-green-500' : 'text-gray-500'}`}>{isPublic ? 'Public' : 'Private'}</span>}>
            <div className="vehicle-detail">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span className={`badge ${isPublic ? 'badge-success' : 'badge-secondary'}`}>{isPublic ? 'Public' : 'Private'}</span>
      ...
      {/* === SECTION 2: Split Pane — Comments (left) | Images (right) === */}
      {/* Both panes are sticky + independently scrollable once you reach them */}
      <section className="section">
        <div style={{
          ...
          {/* Left Pane: Comments — sticky, independently scrollable */}
          <div style={{
            position: 'sticky',
            top: stickyTop,
          ...
          {/* Right Pane: Images — sticky, independently scrollable */}
          <div style={{
            position: 'sticky',
            top: stickyTop,
            maxHeight: paneHeight,
