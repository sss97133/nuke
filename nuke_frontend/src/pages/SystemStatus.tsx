  ...
  async function loadStats() {
    try {
      ...
      const { data: recentCatalog } = await supabase
        .from('catalog_parts')
        .select('part_number, part_name, price_current, created_at')
        .order('created_at', { ascending: false })
        .limit(5);
                    ...
                    >
                      ...
                      {expandedSection === 'catalog' && (
                        <>
                          ...
                          <td style={{ padding: '6px' }}>{item.part_name?.substring(0, 40)}...</td>
                          <td style={{ padding: '6px', fontWeight: 700 }}>${item.price_current?.toFixed(2)}</td>
                        </>
                      )}
