      ...
      {lightboxOpen && currentImage && (
        <div style={{
          ...
          <div style={{ position: 'relative', display: 'inline-block' }}>
            <img
              src={currentImage.large_url || currentImage.medium_url || currentImage.image_url}
              ...
              style={{
                ...
                cursor: showTags && canCreateTags ? 'crosshair' : 'default'
              }}
              onClick={handleImageClick}
            />

            {/* Tag Overlays - Always visible */}
            {imageTags.map((tag) => (
              <div
                key={tag.id}
                style={{
                  ...
                  fontSize: '12px',
                  fontWeight: '600',
                  pointerEvents: tag.isEditing ? 'none' : 'auto',
                  zIndex: 10001,
                  boxShadow: '0 2px 8px rgba(0,0,0,0.3)',
                  minWidth: tag.isEditing ? '24px' : 'auto',
                  minHeight: tag.isEditing ? '24px' : 'auto',
                  cursor: tag.isEditing ? 'default' : 'pointer'
                }}
                title={tag.isEditing ? '' : `${tag.type}: ${tag.text}`}
              >
            ...
            <div style={{
              ...
              <button
                onClick={() => {
                  ...
                  if (!showTags && currentImage?.id) {
                    loadImageTags(currentImage.id);
                  }
                }}
                style={{
                  width: '100%',
                  background: '#c0c0c0',
                  border: showTags ? '1px inset #c0c0c0' : '1px outset #c0c0c0',
                  padding: '2px 4px',
                  fontSize: '8pt',
                  fontFamily: '"MS Sans Serif", sans-serif',
                  cursor: 'pointer',
                  ...
                  {activeTagId ? (
                    <div>
                      <label style={{
                        ...
                        {selectedTagType === 'tool' ? 'SEARCH YOUR TOOLS' : 'TAG TEXT'}
                      </label>
                      <input
                        type="text"
                        value={tagText}
                        ...
                        onKeyPress={(e) => {
                          if (e.key === 'Enter' && selectedTagType !== 'tool') {
                            saveTag(activeTagId);
                          }
                        }}
                        placeholder={selectedTagType === 'tool' ? "Search tools (e.g. 'snap on', 'FR80')..." : "Enter tag text..."}
                        style={{
                          width: '100%',
                          padding: '6px 8px',
                          border: '1px solid #ced4da',
                          borderRadius: '4px',
                          fontSize: '12px',
                          marginBottom: showToolSearch ? '4px' : '8px'
                        }}
                        autoFocus
                      />
                          ...
                          {filteredTools.length > 0 ? (
                            filteredTools.map((tool) => (
                              <div
                                ...
                                onClick={() => {
                                  // Use direct fields from user_tools table
                                  const toolName = tool.description || 'Unknown Tool';
                                  const brand = tool.brand || '';
                                  const partNum = tool.part_number || '';
                                  const tagValue = `${brand} ${partNum} - ${toolName}`.trim();
                                  setTagText(tagValue);
                              ...
                              >
                                <div style={{ fontSize: '12px', fontWeight: '600', color: '#212529' }}>
                                  {tool.brand || ''} {tool.part_number || ''}
                                </div>
                                <div style={{ fontSize: '11px', color: '#6c757d' }}>
                                  {tool.description || 'No description'}
                                </div>
                                {tool.first_purchase_date && (
                                  <div style={{ fontSize: '10px', color: '#6c757d', marginTop: '2px' }}>
                                    Purchased: {new Date(tool.first_purchase_date).toLocaleDateString()}
                                  </div>
                                )}
                              </div>
                            ))
                            ...
                            <div style={{
                              ...
                              {userTools.length === 0 ? 
                                'No tools in inventory. Import your tool receipts first.' :
                                'No matching tools found. Try different search terms.'}
                            </div>
                          )}
                          {filteredTools.length > 0 && userTools.length > filteredTools.length && (
                            <div style={{
                              padding: '8px',
                              textAlign: 'center',
                              fontSize: '10px',
                              ...
                              Showing {filteredTools.length} of {userTools.length} tools
                            </div>
                          )}
                        </div>
                      )}
              ...
              <div>
                ...
                {imageTags.filter(t => !t.isEditing).length > 0 ? (
                  ...
                  <div style={{
                    ...
                    No tags yet.<br />
                    {canCreateTags ? 'Enable tagging and click on the image.' : 'Login to add tags.'}
                  </div>
                )}
              </div>
            </div>
          ...
          <div style={{
            ...
            bottom: '20px',
            left: '20px',
            right: canCreateTags ? '220px' : '20px',
            textAlign: 'center',
            color: 'white'
          }}>
            ...
            <div style={{ fontSize: '8pt', opacity: 0.7 }}>
              {getDisplayDate(currentImage)} • {currentImageIndex + 1} of {displayedImages.length}
              {imageTags.length > 0 && ` • ${imageTags.filter(t => !t.isEditing).length} tags`}
            </div>
          </div>
        </div>
      )}
