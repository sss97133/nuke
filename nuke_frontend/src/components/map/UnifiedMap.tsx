  ...
  return (
    <div style={{ width: 260, fontFamily: 'Arial, sans-serif', fontSize: '11px', lineHeight: 1.4 }}>
      ...
      {thumb && (
        <a href={`/vehicle/${v.id}`}>
          <img
            src={thumb}
            alt={title}
            style={{ width: '100%', height: 140, objectFit: 'cover', display: 'block', background: '#222' }}
      ...
      <div style={{ padding: '6px 2px 2px' }}>
        ...
        {subtitle && <div style={{ color: '#666', fontSize: '10px', marginBottom: 2 }}>{subtitle}</div>}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 3, marginBottom: 3, alignItems: 'baseline' }}>
          ...
          {v.mileage && <span style={{ color: '#999', fontSize: '10px' }}>{fmtMiles(v.mileage)}</span>}
        </div>

        {/* Specs row */}
        {specs.length > 0 && (
          <div style={{ color: '#aaa', fontSize: '10px', marginBottom: 3 }}>
            {specs.join(' · ')}
          </div>
        )}

        {/* Interior color */}
        {v.intColor && <div style={{ color: '#777', fontSize: '10px' }}>Int: {v.intColor}</div>}
        ...
        {/* Location */}
        {v.loc && <div style={{ color: '#555', fontSize: '9px', marginTop: 3 }}>{v.loc}</div>}
      </div>
    </div>
  );
}
...
export default function UnifiedMap() {
  ...
  return (
    <div style={{ position: 'relative', height: '100%', width: '100%' }}>
          ...
          <Marker key={`b${b.id}`} position={[b.lat, b.lng]} icon={BUSINESS_ICON}>
            <Popup>
              <div style={{ fontFamily: 'Arial, sans-serif', fontSize: '11px' }}>
                ...
                {b.type && <span style={{ color: '#666' }}>{b.type}</span>}{b.type && <br />}
      ...
      <div style={{ position: 'absolute', top: 10, left: 54, zIndex: 1000, display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ display: 'flex', gap: 4 }}>
          <input
            ...
            style={{ width: 340, padding: '6px 8px', fontSize: '11px', fontFamily: 'Arial, sans-serif', border: '1px solid #555', background: '#111', color: '#e0e0e0' }}
          />
          <button onClick={handleSearch} disabled={searching} style={{ padding: '6px 12px', fontSize: '11px', fontFamily: 'Arial, sans-serif', background: '#333', color: '#e0e0e0', border: '1px solid #555', cursor: 'pointer', fontWeight: 600 }}>
            {searching ? '...' : 'Query'}
          </button>
          {activeQuery && (
            <button onClick={clearSearch} style={{ padding: '6px 8px', fontSize: '11px', fontFamily: 'Arial, sans-serif', background: '#333', color: '#e0e0e0', border: '1px solid #555', cursor: 'pointer' }}>
              Clear
            </button>
          )}
        </div>
        {activeQuery && !searching && (
          <div style={{ background: 'rgba(17,17,17,0.95)', border: '1px solid #444', padding: '4px 8px', fontSize: '10px', color: '#aaa', fontFamily: 'Arial, sans-serif' }}>
            ...
            {queryNoLoc.length > 0 && <>, <span style={{ color: '#666' }}>{queryNoLoc.length.toLocaleString()} no location</span></>}
          </div>
        )}
      </div>

      {/* Layer toggles */}
      <div style={{ position: 'absolute', top: 10, right: 10, zIndex: 1000, background: 'rgba(17,17,17,0.92)', padding: '8px 10px', border: '1px solid #444', fontSize: '11px', fontFamily: 'Arial, sans-serif', color: '#e0e0e0' }}>
        <div style={{ fontWeight: 600, marginBottom: 4, fontSize: '9px', textTransform: 'uppercase', letterSpacing: '0.5px', color: '#888' }}>Layers</div>
      ...
      {/* Stats */}
      <div style={{ position: 'absolute', bottom: 10, left: 10, zIndex: 1000, background: 'rgba(17,17,17,0.85)', padding: '4px 8px', border: '1px solid #444', fontSize: '10px', fontFamily: 'Arial, sans-serif', color: '#aaa' }}>
        {hasQuery
      ...
      {/* Unmapped sidebar */}
      {queryNoLoc.length > 0 && (
        <div style={{ position: 'absolute', top: 80, left: 54, zIndex: 1000, width: 340, maxHeight: '50vh', overflowY: 'auto', background: 'rgba(17,17,17,0.95)', border: '1px solid #444', fontSize: '11px', fontFamily: 'Arial, sans-serif', color: '#e0e0e0' }}>
          <div style={{ padding: '4px 8px', borderBottom: '1px solid #333', color: '#888', fontSize: '9px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            No location ({queryNoLoc.length})
          </div>
          {queryNoLoc.slice(0, 50).map(r => (
            <a key={r.id} href={`/vehicle/${r.id}`} style={{ display: 'block', padding: '5px 8px', borderBottom: '1px solid #222', textDecoration: 'none', color: '#ccc' }}>
              {r.title}
              {r.loc && <span style={{ color: '#555', marginLeft: 8, fontSize: '10px' }}>{r.loc}</span>}
            </a>
          ))}
          {queryNoLoc.length > 50 && <div style={{ padding: '4px 8px', color: '#555', fontSize: '10px' }}>+{queryNoLoc.length - 50} more</div>}
        </div>
      )}
    </div>
  );
...
function LT({ label, color, checked, set, n, dim, loading }: {
  ...
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', padding: '2px 0', opacity: dim ? 0.4 : 1 }}>
      ...
      <span>{label}</span>
      <span style={{ color: '#666', marginLeft: 'auto' }}>{loading ? '...' : n.toLocaleString()}</span>
    </label>
  );
}
