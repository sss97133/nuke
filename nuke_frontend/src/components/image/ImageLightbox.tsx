...
// Removed ShoppablePartTag - using spatial dots only now
    ...
    <div style={{
      ...
      {(title || description) && (
        <div style={{
          ...
          {/* Shoppable parts indicator - simple */}
          {visibleTags.filter(t => t.is_shoppable).length > 0 && (
            <div style={{ marginTop: '8px', fontSize: '8pt', color: '#00ff00' }}>
              {visibleTags.filter(t => t.is_shoppable).length} shoppable parts (click green dots)
            </div>
          )}
        </div>
      )}
