...
export default function ReviewPage() {
  ...
  if (!page) return <div className="flex justify-center py-20 mt-12 text-gray-400">Loading...</div>
    ...
    <div className="h-screen flex flex-col mt-12">
      <div className="bg-white border-b border-gray-200 px-4 py-2 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(`/deal/${dealId}`)} className="text-sm text-gray-500 hover:text-gray-700">Back to Deal</button>
        ...
        <div className="flex items-center gap-2">
          {prevPage && (
            <button onClick={() => navigate(`/review/${dealId}/${prevPage.id}`)} className="p-1.5 border border-gray-300 rounded hover:bg-gray-50">
              <ChevronLeft className="w-4 h-4" />
            </button>
          )}
          {nextPage && (
            <button onClick={() => navigate(`/review/${dealId}/${nextPage.id}`)} className="p-1.5 border border-gray-300 rounded hover:bg-gray-50">
              <ChevronRight className="w-4 h-4" />
            </button>
          )}
          <button onClick={handleApproveAll} disabled={saving} className="flex items-center gap-1.5 bg-green-600 text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-green-700 disabled:opacity-50">
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-1/2 bg-gray-900 relative overflow-auto">
          <div className="absolute top-2 right-2 flex gap-1 z-10">
            <button onClick={() => setZoom(z => Math.min(z + 0.25, 3))} className="bg-black/50 text-white p-1.5 rounded hover:bg-black/70" title="Zoom in"><ZoomIn className="w-4 h-4" /></button>
            <button onClick={() => setZoom(z => Math.max(z - 0.25, 0.5))} className="bg-black/50 text-white p-1.5 rounded hover:bg-black/70" title="Zoom out"><ZoomOut className="w-4 h-4" /></button>
            <button onClick={() => setRotation(r => (r + 90) % 360)} className="bg-black/50 text-white p-1.5 rounded hover:bg-black/70" title="Rotate clockwise"><RotateCw className="w-4 h-4" /></button>
            <button onClick={() => { setZoom(1); setRotation(0) }} className="bg-black/50 text-white px-2 py-1 rounded text-xs hover:bg-black/70" title="Reset"><RotateCcw className="w-3 h-3" /></button>
          </div>
          {imageUrl && (
            <img
              src={imageUrl}
          ...
          <div className="p-4">
            ...
            <div className="space-y-1">
              {fields.map(([key, value]) => {
                ...
                return (
                  <div key={key} className={`flex items-center gap-2 p-2 rounded-lg ${conf !== undefined && conf < 70 ? 'bg-red-50 border border-red-100' : conf !== undefined && conf < 90 ? 'bg-amber-50 border border-amber-100' : 'hover:bg-gray-50'}`}>
                    <div className="flex-1 min-w-0">
                      <span className="text-xs text-gray-500 capitalize block">{key.replace(/_/g, ' ')}</span>
                      {editingField === key ? (
                        <div className="flex gap-1 mt-0.5">
                          <input autoFocus value={editValue} onChange={e => setEditValue(e.target.value)} onKeyDown={e => e.key === 'Enter' && saveEdit()} className="flex-1 border border-blue-300 rounded px-2 py-1 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                    ...
                    {editingField !== key && (
                      <button onClick={() => startEdit(key, displayValue)} className="text-gray-400 hover:text-blue-600 p-0.5"><Pencil className="w-3 h-3" /></button>
                    )}
                  </div>
                )
              })}
