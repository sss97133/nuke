...
export default function BillingPage() {
  ...
  const handlePurchase = async (planId: string) => {
    setLoading(planId)
    try {
      // Desktop: open Stripe checkout in external browser
      const checkoutUrl = await createCheckout(planId, 'dealerscan://billing')
      if (window.electronAPI) {
        await window.electronAPI.openExternal(checkoutUrl)
        toast.success('Checkout opened in browser. Complete payment there.')
      } else {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.electronAPI.onDeepLink((url) => {
      if (url.includes('status=success') || url.includes('billing')) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 mt-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {plans.map(plan => (
          <div key={plan.id} className={`border rounded-xl p-5 ${plan.popular ? 'border-blue-500 ring-2 ring-blue-100' : 'border-gray-200'}`}>
            {plan.popular && <span className="text-xs font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">Best Value</span>}
            <h3 className="mt-2 font-semibold text-gray-900">{plan.name}</h3>
            <div className="mt-1"><span className="text-2xl font-bold text-gray-900">{plan.price}</span></div>
            <p className="text-sm text-gray-500">{plan.per}</p>
            <button
              onClick={() => handlePurchase(plan.id)}
              disabled={!!loading}
              className={`mt-4 w-full flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium ${plan.popular ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'} disabled:opacity-50`}
            >
              {loading === plan.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CreditCard className="w-4 h-4" />}
              {loading === plan.id ? 'Opening...' : 'Purchase'}
            </button>
          </div>
        ))}
      </div>
      ...
      {transactions.length === 0 ? (
        <p className="text-sm text-gray-500 bg-white border border-gray-200 rounded-xl p-6 text-center">No transactions yet.</p>
      ) : (
            ...
            <tbody>
              {transactions.map(tx => (
                <tr key={tx.id} className="border-b border-gray-50 last:border-0">
                  ...
                  <td className={`px-4 py-2 text-right font-medium ${tx.amount > 0 ? 'text-green-600' : 'text-gray-700'}`}>{tx.amount > 0 ? '+' : ''}{tx.amount}</td>
