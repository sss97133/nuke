...
## 2026-07-13
- [explore-thesis] Explore grounding pass (thesis session, no prod build): mapped both surfaces + substrate via 4-lane workflow. Finding: Explore exists 2x but mislabeled — reads vehicles BaT sold-census under 'live listings' label (C0 facade); 441-feed artery lands as geo-less/unpriced/un-deduped/never-expired stubs no MV reads. Blocker = data-handling not UI. Thesis + build order + 2 open forks landed in THEORY.md. Picks up next week.

- [bat-import] deployed complete-bat-import (thin orchestrator over canonical two-step) + fixed BATListingManager phantom response shape; 3 BaT-import callers unbroken; smoke-tested live
- [appraisal-engine] Audited the existing ~60-table appraisal engine (wf_2a5caac4-c4e, 5 lanes): built 3x/all dormant/no active cron. Found the ONE live target-shaped atom (part_observation_evidence, deal-system cascade), the 3mo-stale condition scores the live valuation reads, the 4-way parts-catalog fork, and ZERO Mustang canon. Wrote APPRAISAL_ENGINE_STATE.md (map+revive path) + appraisal-engine-pickup memory. Revive not rebuild.
- [appraisal-canon] LANDED the 1965-66 Mustang appraisal canon: 325 self-authored inspection points (8 systems, wf_3ee40e5f) into condition_knowledge as Ford/Mustang rows (GM 1243 untouched). Refutes the buy-PDFs approach — the foundation model IS the canon source. Verified: canon independently contains every tell from Skylar's -887 inspection; ground-down VIN = highest weight 1.00 (his scariest finding). Knowledge layer done; remaining = wire live BYOK vision to grade against it. YONO dropped (dead/inferior).
## 2026-07-12
- [ledger] 4 fn deploys live (SEV-1 gate operational); add-by-URL honest degrade after review rejected auto-mint; ledger facts refresher + agent mint hook installed; broken FE calls 49->44

- [ingest] preview mode shipped: add-by-URL paste->parse-only prefill, create stays at submit; refuter-hardened (batch bypass, bool coercion, zero-fetch); live-verified 0 writes
- [wiring] K5 supply-order day: clean order built+red-teamed (T1 ~$2,100 fork-proof, NOT ordered), endpoint load schedule grounded, 4 cert artifacts + Bronco-format pinout sheet shipped, rulings filed (crank/cam 5V, isolator 4-post, AV6 fuel, A26 relay, PCS TCM-2650 trans), comms circuits + isolator + trans holes found. CORRECTION at close: trans = 6L90 not 6L80E.
- [listing-pipeline] live-listing artery widened 2→28 feeds: 17 CL western metros + KSL/eBay×2/Hagerty/C&B/AutoHunter/Mecum/ClassicCars×2/ACC via per-feed regex config in poll-listing-feeds (da17c8feb); all mechanisms verified with real ingests (KSL 4, Mecum 6, AutoHunter 4, ACC 2 created/matched); Hemmings defunct→Mecum, Autotrader blocked
- [listing-pipeline] SCALE-OUT r2 (2026-07-13): live-ask artery 28→~441 feeds. Enabled ALL 416 US CL metros (tiered 120/360min); added rss_article_hop strategy (44733bd44) + BarnFinds; round-2 venues (Barrett-Jackson results+docket, RM Sothebys, Bonhams/TheMarket XML, PCarMarket, Streetside/Gateway/GR-Auto/Vanguard dealers, RacingJunk, PurpleWave, GovDeals). cron batch_size 10→40. All config, no new fns/tables.
- [substrate] revived vehicle_edit_audit organ (missing table broke every authed title edit + search_path bug on 4 fns); VIN uniqueness excludes merged shells (trigger + 17char index; short-index concurrent drop = maintenance window)
## 2026-07-11
- [interaction-layer] Phase 1-3 touch nervous system in nuke_frontend: global InteractionProvider (haptics + press-physics + long-press peek menu off data-* affordance registry), unified pinch-zoom gallery (profile gallery + mobile hero now feed ImageLightbox; 2 dead galleries deleted), and the Live Floor (/live: parametrized realtime browse feed, compare tray, watch via user_subscriptions, deep make-offer ledger writing kind=offer observations via ingest-observation — enum value + nuke-marketplace source added; offer-count is a demand-legitimacy signal on cards). Verified: tsc 0, build green, offer write smoke-tested end-to-end.

- [search-quality] Killed search junk on web+iOS (shared RPCs). Root causes: (1) search_vehicles_deep joined vehicle_search_index->vehicles with no is_public/auto filter, make-prefix boost floated 'Mustang-Style' junk above real Fords (the 4.79 mystery score) — added AND is_public AND is_auto_vehicle; (2) 430 automobilia records (pedal cars, oil cans, paintings, signs) were is_public=true — validated a precise word-boundary pattern (rejected a loose one that matched Sprinter/Sprint), set is_public=false. iOS search_vehicles_fts now clean+ranked. All backend, live, no deploy.
- [drill-downs] Split YMM drill navigation on vehicle-profile header. The year/make/model chips opened per-dimension stat popovers that dead-ended (no navigation). Added a drill footer to each: year -> /?year_min&year_max (year layer), make -> /browse?make (make layer), model -> /browse?make&model (model layer), plus a combined 'Full cohort terminal' link -> /cohort/:make/:model/:year in the model popover so the combined YMM view stays reachable. Uses normalized_model (clean base). Validated all targets resolve (cohort resolved:true, browse returns rows). tsc 0, build green.
- [date-backfill] Mecum/BJ sale-date backfill. Root cause: both extractors captured-then-dropped the date on insert. FIXED both (extract-mecum writes auction_date->sold_at; extract-barrett-jackson now parses RSC start_date->sold_at) and deployed. BACKFILL existing: BJ 40,262 events dated (0%->85.8%) via auction_name->event-calendar derivation (month precision, provenance-tagged sold_at_method/precision; remaining ~14% have no auction_name/year, left NULL not fabricated). Mecum: date only in object-storage snapshots (runDates slug, DAY precision) - concurrent fetch+parse backfill script running in background over ~44K snapshots, provenance-tagged. All writes NULL-fill only on vehicle_events (testimony), via sanctioned bypass.
- [image-pipeline] Resolution layer wired + operated on K5 2024-10-03: known_places legend + BYOK_EXTRA_CONTEXT seam + prepare --date/--rehash in deep-image-analysis-byok.mjs + byok-image-batch.sh; computeSaturation owner-queue fix; known_places→org bridge; Ernie's org dedupe (stub 40deadd4 merged, Oroville a37266f3 distinct); 2 ghost-attributed frames gated rejected_misattributed
- [bat-ingestion] Lit up the BaT LIVE feed: sync-live-auctions now mirrors all ~1,094 live auctions into vehicle_listings (bid/end-time/status) via new upsert_live_auction_listings() RPC, added table to supabase_realtime publication, fixed 'newest' sort in search_vehicles_browse, fixed BaT current_bid:false chunk-kill + vehicle_events partial-index ON CONFLICT; nuke.ag/live now renders live BID + countdown (frontend overlay deployed). Crons active: sync-live-auctions */15, bat-extraction-queue-slow */5 (extract-bat-core deep detail, live lots priority 600-900 over 63K historical backlog), bat-daily-discovery 6:00. Zero Firecrawl spent.
- [weight-model-r&d] Lab session v0: K5 mesh verified (units/axes/landmarks, trust 0.5→0.7) + first 5 xyz field pins + capture-request list; grouping thresholds measured (burst 2s/activity 300s/gps 50m, 3.3x witness deflation); W3 scorecard computed (Skylar 0.79, orgs schema-starved); name-premium measured (Ringbrothers K5s 3.7x anon restomod). Artifacts: papers/the-weight-model.md, working/2026-07-11_lab-notebook-weight-model-v0.md, sighting-ledger proposal. Audit found bbox geometry systemic failure (ISSUES filed, chip spawned).
- [vision-gate] GAA-43671 vehicle e04bf9c5: adjudicated all 127 ungated ssd_blast frames (evidence ladder: GPS geofences, K5 work_sessions, twin-moment residency, pixel checks) — 111 rejected_misattributed w/ misattribution evidence blocks, 16 rejected_personal mirroring K5 twins; 0 ungated remain, 0 rows deleted, none relinked (duplicate-moment trap: all 127 have same-moment twins on K5)
- [images] Cross-sweep moment dedup shipped: 4 library sweeps (user_upload/hd_archive/iphoto/ssd_blast) had re-ingested the same capture moments as different exports — 3,459 vehicle_images rows across 36 vehicles marked is_duplicate→duplicate_of=keeper via new flag_cross_sweep_moment_duplicates() (migration 20260711210000, batched p_max_groups=25 for the hero trigger). Exclusions added: byok prepare/queue/entities worklists, get_vehicle/day_analysis_coverage, get_vehicle_day_depth, recompute_worth_minutes (610 work_sessions recomputed). K5 pilot day 2024-10-03: 75 frames → 20. Zero residual groups fleet-wide; guards skip real bursts (phash detector's turf) + stem conflicts.
- [img-dedup] Wired flag_cross_sweep_moment_duplicates into photo-pipeline-orchestrator (deployed): Step 1.5 runs the detector per (vehicle, capture-day) before classification for local rows — re-swept duplicate frames skip Gemini + never reach BYOK deep queue; late-resolved rows get a post-resolution day sweep; loops until rows_marked=0 (25-group batches, hero-trigger bound)
- [image-pipeline] BYOK bbox geometry fix: teacher/annotate loop wired into byok-image-batch.sh (bbox-annotate.py overlays + zoom insets, TWVP convention file, two-pass protocol every batch); geometry re-emission mode (schema v4 + ingest --merge-mode geometry); K5 2024-10-03 re-emitted 19/19 keepers, crop spot-check clean on components; paint_state 'aged' bleed cleared (subject-vs-background rule); unblocks bbox×camera_pose→xyz projection
- [bat-coverage] Built durable BaT coverage backfill (~/bat-backfill): discovery via listings-filter JSON API (year-window partitioned to beat 10k ES cap, gentle 1.3s/req rate limiter after local-IP 429 throttle), dedup vs 182260 existing unique URLs, extract via extract-bat-core (FREE, idempotent). Orchestrator pid running discovery->dedup->extract. Catalog=251917, real coverage ~72%, gap ~70k.
- [ios] Day receipt shipped (fable5/ignition-ios c5eb6c5ba → TestFlight): tapping a day in Library→Days now renders the day-level synthesis from LocalStore — measured layer (EXIF span, shoot bursts, GPS located, SiteStore work-site), T0 rollup, and cached-verdict read layer (intents, web-formula labor estimate labeled estimated, agent vehicle read via offline garage mirror). Zero network in the render path (grep-proven); sim-verified with EXIF-staged photos. New: LocalDayReceipt/dayReceipt()/garageVehicleLabel() read-only, NUKE_DEBUG_SCREEN=days + NUKE_DEBUG_DAY deep-links.
- [explore] Explore page diagnosed+fixed: #1 metro 'cadillac, MICHIGAN' (11,217 listings) was a fabricated location stamp on one classiccars-com import batch (2026-03-26) — corrected all 11,217 rows via new correct_fabricated_location_stamp() chokepoint (migration 20260712030000, provenance preserved in origin_metadata.location_correction), matview refreshed, Scottsdale AZ now honest #1. iOS b348090a3: metro rows always render the sold column so stat columns align. Follow-up chip: backfill true locations via re-extraction + find the extractor bug.
- [data-integrity] classiccars-com 2026-03-26 location recovery: investigated 'cadillac MICHIGAN' stamp — was consignment mega-dealer HQ (Cadillac MI 49601), not a parser stamp; real bug = last-token state split in drain-queue-no-ai.mjs parseClassicCars (YORK/CAROLINA/JERSEY x3000+). Backfilled 21,781/22,659 rows from surviving import_queue sitemap slugs via migration 20260712050000 (state-aware reparse, source-DNA in origin_metadata.location_provenance); 878 unparseable slugs cleared+preserved; 11,217 dealer rows annotated listing_location, columns stay null (true locations undisclosed by source). Parser fixed+unit-tested.
## 2026-07-09
- [build-ledger] Wired the in-app confirmation agent: agent-chat gains list_pending_confirmations + answer_confirmation (ingest-observation -> supersede_observation, self-healing); new supersede_observation RPC (auth.uid()-authorized, idempotent); get_vehicle_build_ledger widened to owner_confirmed_v1; ingest-observation +0.30 owner_confirmed confidence factor -> verified; AgentChat panel (from dead GrokTerminal, deleted) mounted beside BuildLedger; Confirm stub -> Answer button that seeds the chat. Verified end-to-end on a labeled $1 test draft, ledger restored to 18 rows / 7 pending.

- [byok] agent-chat now runs on the caller's own compute via runWithChain (subscription -> user API key -> platform key metered); never auto-upgrades a rate-limited subscription onto a bill. FIXED THE BLOCKER: user_ai_providers CHECK constraint rejected provider='anthropic_subscription', so every Connect-Claude attempt died with check_violation and the feature could never store a token (migration 20260709160000, additive, preserves 'xai' which prod had and the old migration didn't). Also fixed connect-claude /status decoding an AES-GCM blob with atob() (always reported expires_at:null) and the stale 'provider is unconstrained text' comment that caused it. New /settings/ai route mounts the previously-unreachable AIProviderSettings (ClaudeSubscriptionSettings was mounted nowhere). Ask panel now shows which credential ran + links to /settings/ai on no_credential/needs_funding/rate_limited.
- [security] Revoked anon+PUBLIC EXECUTE on 116 user-scoped SECURITY DEFINER functions (verified live: anon could read any user's garage w/ purchase prices and email; add_cash_to_user/mcp_create_api_key/create_ownership_verification backing tables still exist). authenticated+service_role retained; sweep now returns 0. SEPARATELY VERIFIED AND UNFIXED: the live SUPABASE_SERVICE_ROLE_KEY (fp e51d198a29ea) is committed at d84bdcdbb and served by GitHub on a public repo since 2025-03-26 — needs rotation + repo private, both Skylar's call.
## 2026-07-08
- [venue-routing] 5-agent evidence sweep → docs/library/intellectual/theoreticals/venue-routing-theory.md. Measured: BaT coupe buyers are one-time retail enthusiasts (35.5% repeat vs 44.3% baseline; dealer-suppression hypothesis refuted), cross-venue tier map from 9,434 first-gen Mustangs, FB substrate audit (zero sold prices, zero dwell, 3 Vegas asks/5mo), venue fee table 2026, creative-path sizing (+10-30% in-band, media value attaches to platform not car). Instruments needed: FB dwell time-series, group coverage, dealer price capture, outcome feedback loop.

- [substrate] Class 3 healed: correct_vehicle_sale_provenance chokepoint applied, 20,092 rows corrected (17,948 BJ dates, 1,228 BaT self-heal, 1,057 platform), FLAG-ONLY ingest gate live, verification 0/0/0. Charger merged (644 imgs+1328 obs, lineage OK), Mustang+Blazer images reattributed. CRITICAL: Blazer resort_image side-effected an auto-merge of Skylar's K5 21501c21 into e08bf694; unmerge blocked (no_journal); escalated, K5 untouched, recovery needs Skylar sign-off.
- [substrate] Class 3 healed (20,092 rows via new chokepoint + flag gate), Charger dup merged, 2 misattributed images rehomed, K5 dup record identity-verified (65/74 dhash overlap, invalid VIN) and merge completed via relink_testimony with lineage; ISSUES updated (CRITICAL resolved, systemic merge-trigger fixes filed HIGH)
- [mustang-audit] Institutional investment audit complete: 3 workflows/13 agents over receipts+labor+bank+Gmail+atoms.db+live chat.db+Photos. Documented cash out $7,445 (parts $2,345 line-itemized, Tommy $3,100, wheels ~$2K) converging with owner-stated $12-15K; Kenan Siegel = investor since Nov 2021 (~$9K+, sale=repayment); acquisition likely $0 Skylar cash (tax-exempt title); dad bankrolling June 2026 push. Second '66 coupe (tan consignment 6R07C238681, $20.9K cost sheet) disambiguated out. Top lever: unimported Credit One/x1219/x2889 card feed = $35K order-only would flip to matched. Derived DB layers failed audit everywhere; primary sources resolved. Draft artifact (evidence-tiered ledger + confirm-queue proto): claude.ai/code/artifact/b3c38d24-0ecb-4ce3-8b5c-fdede61c127c. BaT submission planned 6/19 under 'freddyfreshag' — unconfirmed, check.
- [build-ledger-surface] Build Ledger shipped to substrate+app: 12 audit_draft_v0 money observations landed via ingest-observation (each row cites gmail msg id / bank line / imessage ts; email_receipt source config fixed NULL→work_record). Frontend: BuildLedger.tsx + useBuildLedger.ts on vehicle profile (InvestmentLedger idiom, owner-gated mount, citation drill-down, Documented $5,659.59 vs Stated/Pending $11,000 never blended, confirm buttons stubbed TODO Sign-tier). RPC get_vehicle_build_ledger applied + verified: owner 12 rows / stranger 0 / anon 0. Frontend diff UNCOMMITTED for Skylar review; type-check+build clean. Standalone artifact retired (tombstone); rule saved to memory: product data renders in-app only.
- [substrate] Gates 1-4 landed + gauntlet T1-T5 all PASS. Gate1 VIN structural flag trigger (17-char check-digit + GM-truck pos2 catches CRK178P122739). Gate2 owner-merge guard in merge_into_primary (refuses owned-vehicle merges without approved+reviewed proposal). Gate3 disarmed 80 vacuous armed proposals, 1241 real remain; NULL=NULL minter is a historical artifact not in repo. Gate4 resort_image confirmed cannot sync-trigger merge, async path backstopped by Gate2. Migrations uncommitted.
- [all-fronts-sweep] 7-agent research sweep: (1) BaT NOT submitted — abandoned draft ~6/15, 2 unread reminders w/ resume link, freddyfreshag not on shkylar@gmail; (2) Kenan principal unstated anywhere — SouthState wire 5/16/2022 (no amt in notice), $7.5K branch deposit 11/9/2021 + $15K out to Steve Millions 5/10/2022 are candidates; WF x7143 statement = exact answer; (3) 'Credit One' = 3 Visas + debit x2889 + x1602, statements portal-only, payment emails cross-check WF; (4) Tommy $5K killed 6/6 never paid, Carlos painted 6/13-15 paid CASH (banks closed) — owner-only number; June Apple Cash = St Barth personal; (5) listing package TIER_3_VIABLE but 531/533 finished-car photos unzoned + color/engine identity conflicts + no video; (6) Build Ledger screenshots done, 2 bugs (--vp-border undefined, dark --vp-brg illegible) — ledger-fixes agent dispatched; (7) comps-flow Design A recommended: queue-bridge + bat-url-parse + linking UPDATE + trigger slash-fix (479 vs 3 URL matches), ~8.6K sold comps recoverable.
- [extraction] Gate 5: fixed CL/FB title-parser garbage-make corruption. DB trigger trg_sanitize_make (migration 20260708160000) rejects digit-led/letterless/1-char/year-shaped makes universally; hardened normalizeMake + guarded parseTitle in ingest/extract-facebook-marketplace/refine-fb-listing/poll-listing-feeds (deployed). Backfilled 1,677 rows: 185 corrected (cited to raw title), 1,492 nulled+flagged make_unresolved (non-cars). CL/FB garbage now 0% (was ~7%). No prod/repo drift. ~557 auction-source garbage rows left as out-of-scope follow-up.
- [extraction] Gate 5 residual: auction-source garbage-make cleanup. 557 public auction rows classified vs own title/model evidence — 308 corrected cited (Am→AM General x141, MV/Mv→MV Agusta x78, OM/CZ/MZ/FN/DB rare marques kept, year/model-in-make recovered), 249 nulled+flagged make_unresolved (53 automobilia/boats, 192 unresolvable replicas, 4 dup-vin-twin). Residual garbage-shaped makes on public auction rows now 0. trg_sanitize_make verified to pass all real marques. DB-only backfill (no code changes). Filed [MEDIUM] ticket: auction extractors lack BaT-grade title parsing.
- [vision-reingestion] 1,800-image vision sweep of RECEIPTS SKYLAR complete (40 batches + reducer; compiled at scratchpad/audit/vision_receipts_compiled.json). True dates recovered for 1,558 docs (old OCR layer batch-dated them); 1,429 bookable amounts; flaw report catalogs OCR-layer errors (P&L/deal-sheet/liability-clause booking, 219 dupes, Walter's card attributed to Skylar). Walter Acosta: exactly 1 doc — $8.05 sanding discs on HIS card vs 'MUSTANG' PO 2022-09-27 (body-phase corroboration); his pay = cash, owner-only. NEW: triangulated $21,500 cash Mustang purchase Nov 2018 (WF check + ledger '-21,500 1965 Mustang' + AZ trip receipts) — identity ruling needed (likely the green '65 or fastback, NOT his '66). Mustang materials receipts properly attributed: CarQuest '65 Mustang' invoices $323.72 (2022-09, Walter window) + $580.55 (2023-06, was miscounted as 2x Summit). Tax finds: GG MUSIC 1099 $20,374 (2025), Chrome Hearts→NUKE $10,205, IRS CP21A $8,487.55 + loan collection $3,428.99 liabilities, 2023 P&L $16,265 CC interest. Next: dedup pass → ingest-observation landing of clean records with image citations.
## 2026-07-07
- [library] Discourse captured: the shoot layer + the grant — full intellectual-shelf verification of photoshoot-as-asset theory (docs/library/intellectual/discourses/2026-07-07_the-shoot-layer-and-the-grant.md)

- [shoot-layer] Specimen #1 live: 'shoot' asset type + asset_grants table (migration 20260707220500), first assets row = Julie Rodrigo eyewear editorial (6e7d3a9e), 20 deliverables registered in image_identities by sha256, 3 selects in private grant-deliveries bucket with 30-day signed URLs, draft grant e1950f0e awaiting owner terms+send
- [bat-pipeline] Revived BaT ingestion: built scripts/bat-keep-fresh.mjs (daily listings-filter API upsert into bat_listings, run-til-done), loaded com.nuke.bat-keep-fresh launchd (4:15am), re-enabled com.nuke.bat-bid-backfill (was launchd-disabled since ~Apr, root cause of all-platform ingest death). Backfilled 10.6K listings incl. 662 1966 Mustangs. Residual gap: auctions ending ~Apr 14-May 3 unreachable via API (needs full-discovery crawl); 12 other launchd jobs still mass-disabled (fb-sweeps, poll-feeds, extraction-daemon) pending Skylar's call.
- [valuation-engine] Stage-2-lite similarity shipped in compute-vehicle-valuation: body-style stratification (hard mismatch drop, unknown=0.5wt) + year-proximity weights, per valuation-methodology.md §2.2. Root-caused the Mustang $45K estimate to body-polluted comp pool (first-gen pool median $42K vs coupe-only $24K). Mustang now $25,974 conf 84 'canonical_bodied' (85 comps); '67 fastbacks corrected upward $34/47K → $66/67K. Study: docs/library/intellectual/studies/2026-07-07_comp-pool-body-stratification.md. Open: fleet re-stale pass, bat_listings→comps flow, trim/engine similarity.
## 2026-07-06
- [biz-dev] Built L'Officiel fashion-brand target list: mined tonight's stbarth corpus-contamination backup tables (zz_backup_stbarth_prefix_contaminated_20260706, zz_backup_stbarth_corpus_reclean_20260706) + SSD businesses_candidates.jsonl/manual_review_candidates.jsonl for real luxury/fashion brand citations. Loaded 26 brands into organizations (entity_type='fashion', discovered_via='stbarth-brand-targets'), tiered by local-dealer evidence (Diamond Genesis + Tourbillon + fabiennemiot.com boutiques confirmed as multi-brand watch/jewelry channels via shared phone numbers + OCR name_candidates).

- [photo-pipeline] Requeued 7,138 genuinely-hollow (429-completed, no byok rescue) vehicle_images rows for shkylar@gmail.com: re-verified live (0 still hollow, all already sitting failed/retry=0 from the prior backfill), swept via sanctioned reset_stuck_photo_pipeline_images() RPC in 4 calls (7,068 -> pending, 57 excluded-by-design duplicates, remainder cooldown-timed), verified 10 real rows end-to-end via direct orchestrator invoke (incl. 8 concurrent) -- all landed honestly failed/classifier_ok:false, never hollow-completed, byok rescue still fires for resolved vehicle_id. Found + fixed a SEPARATE live regression along the way: process_pending's ORDER BY user_id+created_at had no supporting index over ~32.7M pending rows, timed out every call, silently swallowed as 'no pending images' -- the drain cron (job 478, */5min) had been a total no-op. Fixed with a two-pass indexed query (owner-pending first via idx_vehicle_images_pending_user, backlog top-up via idx_vehicle_images_pending_processing), deployed v56, verified live (process_pending now actually processes rows; cron picked up 12 more of the cohort autonomously within the next few ticks). Zero lock waiters at every checkpoint.
## 2026-07-05
- [extraction] VLVA BaT seller-revenue reconciliation: found 2 platform-wide live bugs (bat_listings trailing-slash URL dedup defeat = 23,304 excess dup rows / 14.8% of table; external_identities(platform,handle) case-fold gap in extract-bat-core's still-live upsert = 938 excess dup identities). Shipped get_seller_stats(handle,since,until) RPC (case-insensitive username match + normalized-URL dedup + self-purchase exclusion) + api-v1-seller-stats edge fn + seller_stats MCP tool. VLVA reconciled: $1,356,275 all-time / $908,475 since 2023-10-01 (37/24 sold). Prior crashed-session headline ($2,224,443/$1,563,693) was the RAW undeduped sum -- exact match confirms the dedup was never applied before that number was reported. Branch fable5/vlva-revenue-1783267397, worktree .claude/worktrees/vlva-revenue, not merged.

- [concierge] L'Officiel yacht load finished: 40 more candidates judged and loaded (19 Edmiston global-fleet + 21 Canados St-Barth catalog models), 1 excluded (Azteca, Pacific-Mexico homeport evidence). Yacht count 27->67 of 68 total. Backfilled AIS MarineTraffic/VesselFinder embed mechanism into properties.metadata.ais on all 67 rows (mmsi still null, resolution flagged as next step). Worktree .claude/worktrees/yacht-seed, commit 900cb719f.
## 2026-07-02
- [image-analysis] ENTITY LAYER LANDED (commit 00ba4bc3d): components_seen → component_identifications via image_analysis_records anchor, receipt-PN reciprocal confirmation with matched-token audit trail. 17 rows → 27,072 (57 receipt-CONFIRMED on the K5: ORD steering package, LMC rockers, Holley intake kit, invoice OCR links). Mustang 5,737 + K2500 9,380 all inferred — those builds have ZERO PN-bearing receipt items (intake gap, not vision gap). ingest() hook live: the cron drain now lands entities on every new verdict autonomously. Corrected premise: fleet cron drains FIRST-PASS pending (prepare --by-day), not stale re-runs — Opus on first-pass is correct tiering; sonnet-5 reserved for future rehash/schema-bump passes.

- [image-analysis] DIMENSIONAL SUBSTRATE IMPLEMENTED (bcb4837dd): retrieved Skylar's months of explanations via session-search (x,y,z volumetric ground truth + 0-100 condition per volume, session 2ebd8b06 2026-05-24; contextual groups e63df3c8; receipt-context squares 5f6e842f) → found the May 24 design doc sitting 'awaiting approval, NO DDL APPLIED' for 5 weeks → implemented it: vehicle_coordinate_frames registry + 6 anchor columns on vehicle_observations + partial time-lapse index, K5 registered via public k5-blazer.glb (origin UNVERIFIED trust 0.5). Anchors land only from real registration, never single-photo derivation. Design doc status updated. Heartbeat telemetry v2.1 live on tag-sync RPC (a7a16de8e) — phone still hasn't called; needs newer TestFlight build + Days view.
- [image-analysis] Tap-target contract SHIPPED + live-proven: get_image_component_targets(image_id) returns bbox+identity+receipt-chain+provenance+computed affordances per component (can_order only when PN+source exist). K5 frame 36d3a45f: 6 confirmed targets, whole ORD steering package, order_source=Offroad Design. UI half (haptic long-press menu) is the app-lane build; checkout/commission = owner-gated next design.
- [image-analysis] DNA DERIVED + CLOBBER KILLED: 5-miner sweep (sessions/library/code/DB/PRs) → docs/library/working/working-papers/2026-07-02_the-dna-derived.md (direction in Skylar's cited words + measured context-loop verdict: DNA coverage 0%→65%→92%; bare-value zone = vehicles money columns). Fixed+deployed process-all-images-cron whole-column ai_scan_metadata replacement (was destroying byok/identity/on_device namespaces). Confirmed hourly GH workflow calls a NON-DEPLOYED function (silent failure since retirement) — main-branch fix needed. Gap matrix ranked: money DNA, extract-vehicle-data-ai 3 violations incl vehicle_images DELETE, ingest bypasses, photo-sync-daemon spine bypass, 3 nonexistent canon docs, PR debt (#278/#286-289/#293).
- [image-analysis] CRYSTALLIZATION ORGAN v1 SHIPPED + FIRST AUTONOMOUS EXPANSION EXECUTED: schema-pressure-census.mjs (npm run ops:schema-census) measures jsonb data pressure and emits ranked evidence-carrying decisions (PROMOTE/LIVE_NAMESPACE/RETIRE_CANDIDATE/WATCH). First census: ai_scan_metadata 4 promote + 3 namespaces + 18 sediment keys; structured_data top candidate = image_id (100% fill/string) which CONVERGED with the known day-receipt 7.5M-row seq-scan pain → executed idx_vobs_structured_image_id CONCURRENTLY → re-measured 0.21ms (was the #1 sluggish cause). Skylar's corrected definition folded into the DNA paper: organic = schema expands WITHOUT human interaction; halt condition = self-sufficiency. Next promotion candidates queued in census output: structured_data intent/scene_type/analysis_kind (62% fill, 100% pure).
- [image-analysis] TRUST TRIAGE from Skylar's screenshots: (1) agent-chat was NEVER DEPLOYED (his 'This shouldnt be in this profile' correction hit a void) — DEPLOYED now, ANTHROPIC_API_KEY confirmed in edge secrets; retry will reach move_photo/create_vehicle verbs. (2) Marketplace screenshots attributed to a DOPPELGANGER 1977 K5 (21501c21, VIN CRK178P122739, 114 imgs, owner_submission 2026-03-07) not his e08bf694 — feed renders both as bare '1977 CHEVROLET K5 BLAZER'; needs his adjudication + feed honesty fix. (3) June 19 gray tiles = the KNOWN pending capture-relay recovery day (proven pipeline, June 20 ran 170/170). (4) Verdict reading is GOOD (correctly ID'd marketplace screenshot + doubted own attribution) — the gap is attribution OBEYING the reading: doubt-gate queued. Task #6 tracks all four.
- [pipeline] BaT relight complete: zombie crons killed (bat-queue-worker 404 x months), RSS feed re-enabled via rss_direct_ingest poller strategy (18 auctions first poll), ingest delegates BaT->extract-bat-core (complete-bat-import was deleted; resolver now matches listing_url), 16/18 shallow rows backfilled deep (Testarossa $107k live bid cited). Ledger only buries structural rejects; transient identity gaps retry.
- [image-analysis] June 19 capture-relay day recovered via proven library pipeline: 14 stills exported from Photos library (osxphotos, real EXIF+GPS), gpt-4o classified (9 white/red GMC wiring day + 5 yellow Scout II pickup/fueling), ingested via ingest_image_identity_first to owner pool (no vehicle claim — both subjects lack DB entities per 2026-06-16 no-mis-file ruling), 27 corrupt relay/placeholder rows superseded (never deleted). Pipeline orchestrator auto-stamped Mustang w/ rate-limited classifier — corrected.
- [image-analysis] JUNE 19 RECOVERED (background agent, proven June 20 pipeline replayed): 14 real camera keepers landed to owner pool w/ real EXIF (9 white/red-interior GMC dash-wiring frames + 5 yellow Scout II Rallye acquisition-day frames), 27 corrupt rows superseded, 0 deletes, thumbnails resolve. Agent correctly STOPPED at attribution (no DB entity for either subject). CAUGHT+FIXED+DEPLOYED: photo-pipeline-orchestrator Strategy 3 auto-stamped unresolved photos onto most-recent vehicle (filed the 14 onto the Mustang 22s after insert, rate-limited, conf 0.2) — removed; attribution = evidence or NULL. June 13/18 recovery running in background (same agent). OWNER DECISIONS PENDING: (1) yellow Scout II Rallye entity — his? (2) white/red GMC identity (maybe the photo-less 1984 K20 entity, but badges say GMC).
- [image-analysis] dHash parity REFUTED: cloud JS cannot bit-match iOS Swift (ImageIO thumbnail resampler is the unreplicable stage; luma + 9x8 stage solved exactly, hamming 0 given same thumb; stock JS = 3-7/64 off) — verdict in docs/library/working/2026-07-02_dhash-parity-verdict.md
- [image-analysis] June 13 + 18 capture-relay days recovered (completes the Jun 13/18/19 backlog): 69 stills ingested from Photos library w/ real EXIF (Jun13: 1 Mustang booth frame + 4 white-K10 frames to 6442df03 [bowtie-grille/LED-bar match] + 7 pool; Jun18: 35 Mustang final-assembly frames + 22 pool). 76 corrupt relay/dead rows superseded (49+2 Jun13, 25 Jun18), incl. 2 June-16 mis-routes on the white K10 whose content was Mustang paint booth. Orchestrator auto-stamp: 0 drift on all 69 (fix live). Never deleted.
- [image-analysis] 2019-misdate relay cohort closed: 422/2024 2019-stamped capture_relay_ios rows had taken_at contradicted by stored-original EXIF (383 true 2017-2018, 39 true 2026-06, 3 true 2019-04; other 1602 legit backfill). Root cause: relay stamped taken_at from older same-named photo (filename recycling). All 422 corrected via correct_image_provenance chokepoint, cited to exiftool DateTimeOriginal + Photos-library counterpart (filename+timestamp match, 422/422), originals preserved in provenance_corrections. Zero deletes, zero unverified.
- [pipeline] Overnight block: Gullwing card imaged (screenshot crop->storage->primary), 18/18 BaT rows deep + models normalized (slug-garbage pollution check shipped in extract-bat-core), CL batch title-cased (0 lowercase), $197 ledger artifact nulled, both 500s healed by resolver fix. fb-saved-sync re-enabled (was disabled in launchd override since 6/13) + politeness patch (background tab, no hijack).
- [library] landed the 3 missing canon contemplations (the-trust-invariant authored from K5/GAA-43671 incident + session-archive quotes; the-root-system + the-agent-must-cite ported from fable5/ignition-ios) — e561c5115
- [extraction] extract-vehicle-data-ai source-DNA fixes: gap-fill-only updates, domain-derived observation platform (no more hardcoded bat/0.65), no-DELETE image dedupe — deployed + verified live, commit 2d0ead7ee
- [product] Sandbox-agent architecture ratified (Skylar: 'the sandbox agent is the YOU for the app') — spec'd into docs/products/INVESTIGATOR_CHAIN.md §Who-runs-the-chain: BYOK compute (user_ai_providers_byok), edge-fn tool belt, in-app chat+share-sheet surface; 2026-07-02 session = behavioral spec. Profile ingest-URL box verified wired to fixed /ingest.
- [photo-sync] photo-sync-daemon converged on sealed-capture: sha256 at source + ingest_image_identity_first chokepoint (bdb8dc4aa)
- [image-pipeline] receipt-context bridge: day_context status in entity landing (deep-image-analysis-byok.mjs), proven on K5 2021-07-24 rocker day (13 day_context rows)
- [image-analysis] FOUNDATION NIGHT: (1) Blur full-library vision index (79,506 entries) pulled off the paired iPhone via devicectl, landed through sync_local_vision_tags v2.2 — 5,428 rows tagged, labels 9.4K→13.5K. (2) Content identity rooted from storage etags via new root_identity_by_content chokepoint — 12,599 identities, 20,104/26,777 rooted (39%→75%), 3,056 exact dupes collapse structurally. (3) filename+size bridge measured honest: 6,024 unambiguous, ~306 tag-gains only (daemon re-encodes) — phash via thumbnails remains the reach. All token-free per brakes-on-Fable; fix-fleet finishing; accelerated drain off.
## 2026-07-01
- [image-analysis] Phone→cloud vision-tag seam CONNECTED end-to-end: LocalTagPush wired (cursor fix + BG task + Days-view call sites, commit 5983650fe on fable5/ignition-ios, sim-verified, pushed → Xcode Cloud/TestFlight); backend RPC rewritten set-based v2 (34s timeout → 196ms per 200-batch, +matched_ids, live + committed bbcbe7597). Tags flow once Skylar installs the new build. Scorecard re-derived: 4,117 saturated / 6,892 stale-open / 11,009 verdicts (earlier '0 saturated' was a wrong JSON path).

- [pipeline] Itch→issue session: gated editorial slop (extract-vehicle-data-ai), fixed ingest share-URL+enrichment (root cause: response-shape unwrap bug), revived CL-Vegas artery (firecrawl_html poller, hourly, 156 items first poll), P2 price heal batched, pipeline_heartbeat dead-man cron + get_pipeline_pulse (timeout-hardened). Cron 419 on / 420 off. 19 junk vehicles soft-deleted, 42K stale queue defused. 8+3 ISSUES entries filed.
## 2026-06-26
- [image-attribution] Recovered June 20 Mustang shoot (170 lib photos, correct EXIF dates) via library-as-truth pipeline; fixed Previous section (7 sold now show); built demote_observation_to_user + promote-from-pool primitive (migration 20260626000000); fixed idx_unique_vehicle_file_hash to exclude superseded (concurrent rebuild) — reattribute return-moves work now.

## 2026-06-25
- [image-pipeline] Identity-first ingestion live in prod (ingest_image_identity_first; migration 20260625000000 — root image_identities/appearances, vehicle_images.image_identity_id FK, default_attribution patched so references aren't re-stamped). iOS foundation committed edc565fb7 on engine-surface (LibraryView full-library browse + full-access onboarding). Consolidated 3-agent handoff in memory/image-pipeline-consolidated-2026-06-25.md → authoritative = ignition-ios CLAUDE.md NEXT BUILD directive (GRDB decided, one-agent/one-vehicle, network-off acceptance) + addendum §8. Closed out.

## 2026-06-23
- [ai-credit-billing-test] Tested funding wire both directions. IN: signed live checkout.session.completed -> deployed webhook 200 -> ledger credited (live secret path verified). OUT/billing: found+FIXED a money bug in ai_credit_available_cents() — it summed ALL closed rows including 'hold' rows, so a released hold permanently ate its estimate and a successful settle would DOUBLE-debit (hold est + settle charge). Fix: closed-sum now filters entry_type IN ('topup','settle'); holds only count while open. Verified: released-hold refunds (->2500), and reserve$2->settle$1.50 debits exactly 150 with 50 freed. BLOCKER: platform ANTHROPIC_API_KEY account is OUT OF CREDITS ('credit balance too low') so real metered API cost can't be incurred until that Anthropic account is funded. GAP: runWithChain only meters on the subscription-429->platform-upgrade fork; a direct system_api_key happy-path call is UNMETERED (platform pays Anthropic, user not debited) — fix if platform-key serving is intended. All test ledger rows cleaned (balance 0).

- [db/cohort+timeline] Fixed 2 prod fn bugs from code review: register_make_model_subject crashed (uuid var used as int swap temp) on inverted year bounds; get_vehicle_investment_timeline overstated totals on a late negative receipt (max(running)->baseline+sum) and prepended baseline out of date order. Applied as 20260623040000; K5 output byte-identical (207pts/32808.25), inverted-bounds now returns uuid.
- [ios] Reverse-DSO production sheet + drill-to-source + hero/layout/calendar/comment fixes shipped to TestFlight; extraction-link designed, K5 factory config decoded from SPID (400CI Cheyenne), landing pending
- [stripe-consolidate-retire-sandbox] Consolidated Stripe to the live main account + retired the sandbox. Sweep found only ONE stale leftover: STRIPE_RESTRICTED_KEY was still sandbox rk_test_51SKWCO and UNUSED by any code/frontend. Removed it from both Supabase secrets (DELETE 200) and local .env. Deleted the orphaned sandbox webhook endpoint we_1TlByA (acct_1SKWCO) using the sandbox restricted key (last use). Confirmed: no code references acct_1SKWCO/sandbox anywhere; all remaining STRIPE_* are live 51SKWCH (secret/publishable/vite/webhook/api_all); exactly one live webhook we_1TlIfz enabled for checkout.session.completed; wallet reads clean. Backend canonical store = Supabase function secrets (local .env mirrors). REMAINING (Skylar/minor): Stripe CLI config (~/.config/stripe/config.toml) still has an expired test key + can be refreshed via 'stripe login'; the sandbox Stripe ACCOUNT acct_1SKWCO still exists but is fully abandoned (no refs, endpoint deleted) — can be closed in dashboard.
- [ai-funnel-code-fixes] Applied + deployed all 4 funnel fixes to live. (1) OAuth routing: resolveClaudeAuth now detects sk-ant-oat tokens and routes them as subscription/OAuth (Bearer+beta) instead of x-api-key — fixes the mis-filed token for subscription+BYOK paths. (2) Universal platform metering: runWithChain now meters EVERY platform/system-key call via new runMeteredPlatform helper (hold->call->settle), not just the 429-upgrade fork — closes the unmetered-spend gap. (3) serviceError: platform Anthropic billing-400 now maps to serviceError='platform_unfunded'; analyze-with-claude returns 503 'temporarily unavailable, wallet not charged' instead of opaque 502/misleading needsFunding. (4) DB idempotency: partial UNIQUE index ai_credit_ledger_topup_ref_uniq (migration 20260623050000) + creditCents swallows 23505 + settleHold gates settle-insert on release rowcount (no double-debit on replay). Deployed stripe-webhook/analyze-with-claude/connect-claude. VERIFIED: idempotency double-fire credits once (200/200, 1 row); OAuth routing correct BUT skylar's stored token is DEAD ('Invalid bearer token', sk-ant-oat, expired/no-refresh). REMAINING — 2 SKYLAR ACTIONS: (a) reconnect Claude via connect-claude (his token is dead) for subscription path; (b) fund the platform Anthropic account for the platform-key path. All CODE paths now correct; both blockers are external creds only I can't supply.
- [cohort-terminal] STOP CAPPING: removed LIMIT 1000 (price_points) + LIMIT 24 (comps) from get_make_model_terminal — K5 model now returns 1138 points/394 comps (was 1000/24). Built+shipped the price_points SCATTER on CohortTerminal.tsx (was built in RPC, never rendered): every sale as a drillable dot, dated on a time axis, undated in an honest gutter, colored by source; leads the price section above the median. Verified via headless screenshot. Fixed 2 React dup-key bugs. Local migration 20260623050000.
- [ai-compute-surface-spec] Developed the theory + iOS deliverable for the AI-compute funnel (4-lens workflow: economics, iOS-UX+Apple-policy, technical, adversarial). THEORY: one funnel, two tiers — Tier 1 'bring your own compute' (user's Claude subscription OR their API key; $0 to platform, unmetered) vs Tier 2 'managed compute' (platform key, prepaid wallet, cost x1.2). The BYOK(iOS)-vs-wallet(web) split is CORRECT, not a bug: Apple Guideline 3.1.1 forces IAP (30%) on in-app credit sales which inverts the 20% margin, while BYOK is IAP-exempt (user pays Anthropic directly). DELIVERABLE spec written to .worktrees/foundation-ios/apps/nuke-capture-ios/AI_COMPUTE_SURFACE_SPEC.md: iOS v1 = BYOK-first + native 'Compute' status header + read-only wallet balance (only if funded) + reject sk-ant-oat pastes; EXCLUDE OAuth Connect (unsanctioned client_id, dead token, 2.1 rejection risk) and in-app credit sale. Must-fix prereqs: encrypt user_ai_providers at rest (currently base64 not encrypted), fund-or-hide Tier 2 (platform Anthropic unfunded), decide OAuth tier fate. 4 owner-decisions flagged.
- [tier2-live-and-ios-guard] CAPSTONE: Tier 2 (managed compute) now works end-to-end after Skylar funded the platform Anthropic account $20. Verified the REAL deployed path (runWithChain -> runMeteredPlatform) for a user resolving to system_api_key: call returned 200 + real Claude text (no more 'credit balance too low') and metered correctly — debited 2c (500->498) via hold->call->settle. Restored state, cleaned test balance to 0. Phase 0 iOS: added sk-ant-oat reject guard to ConnectedAccountsView.swift AddProviderSheet.save() (rejects subscription tokens mis-pasted as API keys) — xcodebuild BUILD SUCCEEDED. iOS change is uncommitted on worktree fable5/ignition-ios (needs version bump + commit to ship). FUNNEL STATUS: funding-in GREEN, ledger GREEN, BYOK GREEN, platform/metered GREEN (verified), endpoints GREEN; only OAuth-subscription path is operationally dead (Skylar's token expired; spec recommends web-only/feature-flag, never iOS). REMAINING: (a) encrypt user_ai_providers at rest (base64->pgsodium/Vault — schema/auth migration, needs sign-off); (b) Phase 1 iOS Compute-status + read-only balance UI (needs build+screenshot loop).
- [cohort-terminal] Built proven-vs-projected slot in the price_points scatter (the 'house'): owner/user-submission valuations now render as hollow rings, NOT filled comp dots, with a legend note that this is the slot a build's own valuation fills as it firms from receipts/images/build record. The $189K user-submission spitball on the K5 cohort stops masquerading as a market sale; header now reads 'N market sales · M projected'. Classification is a frontend source-list stopgap — durable home is a provenance attribute on vehicle_events (noted in code). Verified via screenshot; 0 key warnings, tsc clean.
- [encrypt-at-rest-and-client-routing] Encrypted user_ai_providers at rest + closed all base64 write paths. secretBox.ts = AES-GCM keyed by AI_PROVIDER_ENC_KEY (function env, NOT DB; backward-compat reads of legacy base64). Wired getUserApiKey + claudeSubscriptionAuth (store/load/resolve). New set-ai-provider edge fn = encrypted client write path + service-role _migrate. Migrated the 1 existing row to enc:v1. VERIFIED decrypt end-to-end: minted JWT -> analyze-with-claude decrypted skylar's row -> routed sk-ant-oat as OAuth -> reached Anthropic (401 dead token, as expected). iOS: routed BYOK write through set-ai-provider (plaintext over TLS, server encrypts), added read-only Compute/balance section + oat guard; BUILD SUCCEEDED; committed 2355d0ae build 29, pushed fable5/ignition-ios. Web AIProviderSettings: routed write through set-ai-provider + masked key display; builds clean. Backend+web source committed 74b7f1a4 on feat/cohort-terminal. Deployed all edge fns live. Web prod deploy to nuke.ag in flight via linked nuke_frontend project (first attempt from dist/ hit a stray 'dist' vercel project — removed it). Blast radius was tiny: only index-reference-document uses getUserApiKey (not deployed), 1 ledger row.
- [valuation] Resolved the 63x labor contradiction: compute_vehicle_investment_proof read projected labor from work_sessions.total_job_cost (zero-rate imported sessions = $656 on the K5), get_vehicle_documented_investment computed $41,559 from burst-clustered active minutes. Root-caused (all 106 K5 sessions rate=0, 463 logged min vs 16,940 burst min) and reconciled: proof projected labor now uses the same burst basis, net of confirmed. Verified: proof $41,559 == doc $41,559.47, no-data vehicle returns 0. Confirmed labor unchanged. Migration 20260623060000. Flows to iOS InvestmentProofView automatically.
- [valuation/labor-rate] Applied Skylar's epistemic correction: 5 unverified shop rates is NOT a market rate. resolve_labor_rate now returns reported-fact-wins else an explicit SPITBALL (~$160, n<30), never a stated market rate, with a posted-vs-accepted caveat ('everyone hates paying that'). Wired both valuation functions (documented_investment + investment_proof) to this one honest rate engine instead of hardcoded $160. K5 labor confidence now reads hours=deep, rate=spitball; value preserved ($41,559). iOS proof projected-labor confidence now 'spitball'. Migrations 20260623070000 (+060000). Self-refines as authenticated shop rates ingest.
- [valuation/build-class] Built wire #1 of the build-class unlock: nuke_build_class(vehicle) v1 — classifies from evidence (documented investment/activity, listing-text keywords, else honest unknown). Standalone/unwired (no mispricing). Validated K5=documented_build (caught the build despite is_modified=false), Chalet=unknown. Finding: structured flags lie; class-stratified comps need a vision pass over comp listings. Migration 080000.
- [valuation/build-class] Implemented wire #2: get_class_stratified_price(vehicle) — prices a vehicle against comps of its OWN build class, producing comp_method='class_stratified' (the value iOS isThin needs to unblock). Validated K5 -> $30,050 from 6 build comps (was $112k stock-comp model / Not-priced / $189k spitball). Honest blocks on unknown class or <5 class-matched priced comps. Cohort measured: 79% unknown (need vision), 21% text/doc-classified. Migration 090000. Follow-on: materialize build_class for scale, wire into get_vehicle_valuation hot path, vision pass over unknown comps.
- [valuation/build-class] Fixed the comps: nuke_build_class v2 scores full listing descriptions (engine-swap = decisive build tell) instead of narrow keyword-exact. K5 cohort unknown 54->32, modified 11->27, stock 1->7. K5 class-stratified price now backed by 16 build comps (was 6), median $30,500. Text-less remainder honestly unknown -> vision. Migration 100000.
- [valuation/build-class] Curated platform comps: the K5's real comp universe is full-size K5 Blazer + GMC Jimmy twin (squarebody gen), EXCLUDING compact S-10/S-15 (curation imperative — 90 named compacts + 1051 ambiguous in raw set). Materialized build_class for 1843 curated full-size vehicles (822 build/100 stock/921 unknown). get_platform_stratified_price: K5 now prices vs 512 class-matched build comps (was 16) -> $28,873, range tightened $20.9k-$42.6k. Migration 110000 + backfill.
- [ios-app] Main-pages + bottom-nav overhaul PUSHED → origin/fable5/engine-surface (4 commits, build+sim verified, PR pending into ignition-ios). NAV: Explore now leftmost (front-door structural), Today stays conditional, stale 2-tab header fixed. TODAY: ENGINE worklight (LiveAnalysisStream over live get_analysis_stream, auth.uid()-scoped — prod probe caught the stale p_user_id sig). PROFILE: fleet worth-hiring headline (Tenet-9 sentence + est/tilde fleet worth, 'Garage'→'The fleet'). EXPLORE: market read terminal (marketplace_metro_pulse landing → metro→velocity make-breakdown drill, header from tapped row since views don't reconcile; cohort un-buried via dominantCohort). Each move researched vs decided rulings + accuracy-vetted (critic killed 2 over-reaches, Plan agent caught the reconcile bug). DEBUG hooks: NUKE_DEBUG_SCREEN=app/profile/explore.
- [ios] Connection audit (4-surface, live-prod-verified): app mostly real data; fixed get_user_sync_status bare-object decode (da80e5d61, Sync ledger was always erroring); confirmed apple_ml_labels exists (uploads safe); mapped 5 break-patterns + repo↔prod drift; HALTED BYOK slurry drain per Skylar. Full map: docs/ios-connection-audit-2026-06-23.md
- [reverse-dso] Proved the per-image CONTEXTUAL EXTRACTOR (subject/role/time/role-data/threshold) on the 6 K5 images that fooled the free-text L4 gate: hallucinated SPID -> 'Holley intake part, names nothing'; receipts read fully + classed as sourcing; interior named (mid-build); VIN plate read; screenshots name nothing. crosses_naming_threshold is the key feature. Also: shipped iOS temporal-provenance display + drill/cite RPC fixes; re-grounded K5 config from the corpus (LS3 6.2 swap, Cheyenne, Maroon/GM-71, plaid buckets) pixel+threshold-gated; filed 3 issues (lens hallucination, recency-weighting, threshold-to-name).
## 2026-06-22
- [stripe-webhook] Fixed AI-credit funding wire: deployed STRIPE_WEBHOOK_SECRET didn't match the registered endpoint's signing secret, so every checkout.session.completed delivery 400'd and nothing credited. Recreated the test webhook endpoint via API (we_1TlByA), synced the new whsec to Supabase secrets + local .env, deleted the stale endpoint (we_1SKdnu), redeployed. Verified end-to-end: test trigger credited $25 to skylar's ai_credit_ledger (balance 0->2500). NOTE: entire stack is test-mode keys; going live needs live keys+endpoint.

- [live-pulse] Deployed vehicle_pulse arbiter (demo-safe core: table + arbitrate + get_vehicle_pulse + RLS + Realtime publication) to prod; iOS living header subscribes via Realtime postgres_changes — live bid + countdown + chime PROVEN with a staged throwaway auction (torn down). Hot-path trigger deferred to go-live.
- [cohort-terminal] iOS CohortTerminalView: median killed → sales scatter (every dot, source-toggle, snapshot card, drillable), production provenance drill + drillable identity chips, sentiment 2-axis ALIGNMENT MAP (polarity × community stance). RPCs: get_make_model_terminal (price_points + production range), get_make_model_sentiment_points.
- [comment-intelligence] 227 cohort comments scored on community_stance_score then RE-SCORED on rubric v1 (2 axes + 102 extracted claims carrying source comment_id). Clues→citations PROVEN; calibration NOT (axes r=0.656 collinear → needs rubric v2 decouple). Pipeline already exists (commentRefinery.ts + comment_claims_progress 22,285 + ingest-observation). Compute: Ollama $0 / DeepSeek ~$110; ChatGPT-sub ruled out.
- [vehicle-profile] VehicleDetailView: SALE HISTORY section (deduped — kills the 'sold twice' artifact), build-barcode suppressed for scraped comps (no dead 1-cell strip).
- [stripe-live-cutover] Took AI-credit funding wire LIVE. Root cause was deeper than the sandbox secret mismatch: the app's env keys all pointed at a Stripe SANDBOX (acct_1SKWCO 'nuke ltd sandbox') while real money + the user's $24.34 live balance sit in the MAIN account (acct_1SKWCH 'nuke ltd'). Two separate Stripe accounts — payment could never reach the sandbox webhook. Fix: user supplied live sk_live keys for acct_1SKWCH; set STRIPE_SECRET_KEY/PUBLISHABLE_KEY/VITE_/STRIPE_API_ALL to live + pk_live (from CLI config) in Supabase secrets + local .env; created fresh LIVE webhook endpoint we_1TlIfz (checkout.session.completed) and deployed its signing secret as STRIPE_WEBHOOK_SECRET; deleted stale live endpoint we_1Tkjog; redeployed stripe-webhook + create-api-access-checkout. Funding flow is 100% server-side (create-api-access-checkout returns session.url, client redirects) so NO frontend/pk change needed. Removed the earlier sandbox $25 test credit from ai_credit_ledger (balance back to 0). OPEN: (1) webhook credits AI wallet for EVERY checkout.session.completed incl. dealerscan_credits/concierge — should gate to ai_wallet_topup only now that we're live multi-product; (2) real $24.34 live balance not yet credited to wallet (mixed product provenance); (3) live secret keys were pasted in chat transcript.
## 2026-06-18
- [fb-deal-catcher] fb-fast-lane.mjs: killed year-tier/keyword firehose, gated to Skylar's watchlist lane (squarebody/Cummins/OBS/4x4), primed seen baseline (backup logs/fast-lane-seen.bak.json). Wrote memory feedback_sick_deal_and_money_respect. Freshness UNSOLVED.

- [image-ecosystem] IMAGE_ANALYSIS_100X mandate driven to DoD (5/5 clusters PASS, adversarially verified). This session closed: P4(c) CLIP embedder convergence bug + com.nuke.clip-embed scheduler (560->7600+ embeds, draining); P5 ARM6 cascade_parts_evidence (294 atoms) + micro-atom lane scene_micro_atom_evidence (20.5K) + connected_t3/cascade_atoms lit in coverage RPC + per-image get_image_depth_score; P8.1 status RPC shows full cascade (20.8K atoms); P1.3 reap ~1M/1.32M scraped processing->failed (resumable). Wave-1 deployed+HTTP200: P3.2 orchestrator off dead yono-analyze onto BYOK enqueue, P7.2 api-v1-events unknown-property->pending_claims+schema_proposal, P7.3 synthesize_attribute->SQL project_attribute, P6 timeline illumination+findings card verified live + dashboard de-scanned, P7.4 ch.18 design-book + doctrine reconciled. Fleet coordinator advancing across non-K5 vehicles; engines self-sustain. ~13 commits.
- [ios-k5-profile] K5 profile fix (analyzed-only latest-first gallery + owner-curated before/after, ios c39de1206 + nuke 88b1e5c03); diagnosed+cancelled 73-min runaway get_comps_scored query that I/O-starved the whole instance (the 'everything crashing'); flagged get_comps_scored as pathologically slow for K5 + handed prepare heap-IO finding to IMAGE-ECOSYSTEM agent
## 2026-06-17
- [architecture/polymorphic-entity] KEYSTONE STEP 1 APPLIED TO PROD (approved): migration 20260617000000_observation_polymorphic_subject.sql — added subject_type (NOT NULL default 'vehicle') + subject_id (uuid null) to vehicle_observations (~7.5M rows). Metadata-only (constant default = no rewrite), CHECK added NOT VALID (instant), NO backfill UPDATE (effective subject = COALESCE(subject_id, vehicle_id)), NO index (deferred to concurrent follow-up). Verified on prod: cols+constraint present, legacy rows untouched (vehicle/null/has-vehicle). Conformance: build-guide step1 marked done, dictionary tables.md updated. Non-vehicle subjects now EXPRESSIBLE; zero behavior change. Steps 2-5 (ingest-observation subject param, activity kind, role-proof FSM, gate resolveBrand) NOT done — touch load-bearing ingest fn, need test+deploy.

- [architecture/polymorphic-entity] Step 2 scoped from code (engineering-manual/20 updated, NOT deployed): ingest-observation is vehicle-shaped after the insert (gapFillVehicle + writeFieldEvidence keyed on vehicleId), so adding a polymorphic subject = thread subjectType/subjectId via conditional spread (vehicle path byte-identical) AND guard-skip vehicle post-processing for non-vehicle subjects. Must deploy PAIRED with first real non-vehicle writer + validate end-to-end through the sanctioned path (no raw insert). Declined to blind-deploy the universal intake fn for an unused capability — step 1 (live schema) remains the unblock.
- [architecture/polymorphic-entity] STEP 2 APPLIED + DEPLOYED: ingest-observation now accepts optional polymorphic subject. Edited index.ts (HTTP endpoint, own insert) + _shared/observationWriter.ts (shared extractor path): optional subject via conditional spread = vehicle path byte-identical when omitted; writeObservation requires vehicleId only for vehicle subjects, requires subjectId for non-vehicle, and SKIPS vehicle-only post-processing (gapFillVehicle/writeFieldEvidence) for non-vehicle. Deployed ingest-observation via CLI; edge logs clean, zero errors from the fn. No synthetic test row written by design (testimony is never-deleted; fake org row = permanent contamination) — end-to-end non-vehicle write validates when first real org writer lands. NOTE (unrelated, pre-existing): photo-pipeline-orchestrator throwing recurring 500s.
- [image-ecosystem] Phase 1 of the image-analysis mandate: (1) W3 fix — buildContext KEY→SERVICE_KEY; dossier context now loads in the live fleet pipeline (was THIN on all ~12.5K verdicts). (2) Coverage layer migration 20260617000000: canonical tier ladder T0-T4 + analysis_depth_score + get_vehicle/day/fleet coverage RPCs + image_coverage_by_vehicle counter table. Personal corpus = is_external=false (NOT user_upload). First honest scoreboard: corpus 29,063 / gated 17,587 / seen 8,632 (49% deep) / placed 856 (4.9% engine) / CLIP 111 / stale 5,840. (3) Un-pinned the fleet driver (W1): byok-fleet-next.mjs + byok-fleet-batch.sh; plist repointed off hardcoded K5; cron now self-advances — proven by picking non-K5 83f6f033 and writing 2 verdicts with live build context. (4) ralph-wiggum brief repointed off phantom appraiser->primary_label onto get_fleet_analysis_scoreboard() (deployed) — brief now agrees with RPC (8,632 analyzed). Commits b58715c6 + 462772ca.
- [architecture/polymorphic-entity] STEP 3 APPLIED: migration 20260617000100 added 'activity' to observation_kind enum (additive/online, verified present). Defined as the pre-confirmation action kind (intent owner-confirmed before promotion to work_record) so it doesn't overlap work_record — photo-intent rule made structural. CEO-drive = activity + subject_type=organization + structured_data{actor,work_order,geo_path,cost,intent}. No source registered to emit it yet (opt-in when a real activity writer lands). Keystone steps 1-3 of 5 now LIVE on prod.
- [image-ecosystem] Phases 4+6 + assessed 2.3/3.2. PHASE 4 (W2): deep writer now populates image_observations (ARM2b, observed_by=caller-byok) + backfill_byok_image_observations() replayed 9,557 stored verdicts → placed_t2 856→8,964 (pct_engine 4.9%→51%). CLIP embed (clip-embed-image-observations.py --all) running in background, local ViT-B/32, with_clip 111→1,019+ climbing. PHASE 6: VehicleFindingsCard repointed off phantom flat classifier onto nested byok_deep_analysis marker; BarcodeTimeline illuminates by analysis depth via new get_vehicle_day_depth() RPC (migration 20260617020000); tsc clean. PHASE 2.3: assessed → deliberate HOLD (gate correctly holds evidence-less frames as review_needed; auto-approving = unsupervised attribution at volume → cardinal no-mixed-data rule needs owner signal). PHASE 3.2: assessed → dead yono route is non-fatal noise; real inflow-deep blocker is owner-gated gating, not the sidecar (seen_7d=14 proves gated inflow flows). Fleet cron now draining 42 distinct vehicles. Commits ca4a9ad5, 5719dfc1.
- [photos/safety] Removed doppelganger landmine in scripts/work-photos-intake.mjs: unmatched GPS photo-sessions were SILENTLY dumped onto the Sierra Classic (a CLIENT truck) — now skipped + left unattributed instead. Shop runs many customer cars (June audit found a client Toyota on a rotisserie, El Camino, Mirage mixed with the builds), so the blanket default was contaminating a client's record. Also flagged: the GPS→vehicle mappings are unverified guesses and classifyPhotoVehicle() vision classifier is dead code (never called).
- [architecture/polymorphic-entity] STEP 5 APPLIED (proof gate, scope-corrected: client-side entityProof, no RPC minted): added canProject()=hasFormationProof (fail-closed); gated BOTH projection paths — resolveBrand (?brand=/subdomain entry) and activateBrand (mode tap) refuse org/vehicle without formation proof; synthetic brands exempt. tsc+build green. Validated on REAL data, zero synthetic writes: Viva (3 work_orders) WEARABLE -> dad-test link reskins; Desert Performance/FBM/Hot Kiss/Taylor/Epstein (0 artifacts) REFUSED. Closes entity->proof->projection loop in product. Step 4 (role claim->verify FSM) DEFERRED — no consumer, don't build speculatively. Keystone: steps 1,2,3,5 LIVE; only 4 deferred.
- [sell/deliberation] P5 temporal 'now' resolver LANDED + verified. attribute-registry: added temporal flag, marked current_color/exterior_color/condition_cues/sale_disposition as present_state. mcp-connector synthesize_attribute: present-state attributes now recency-weighted by EVIDENCE-CAPTURE time (image taken_at via evidence_ref.image_ids join, half-life 365d, weight*=0.5^age_years) so latest-era state beats stale teardown; timeless attrs (VIN/title/original_color) untouched. Deployed+verified: current_color recency_weighted=true, title_status recency_weighted=false. Also: structured-color derivation verified live (exterior_color='Gloss Black' from current_color atom). Full deliberation loop P1-P5 now closed, deployed, proven on the '66 Mustang.
- [sell/deliberation] P5 temporal resolver PROVEN with real teardown-vs-finished flip. Fixed a real bug caught by the value-flip test: recency read evidence from request_envelope but evidence_ref is a projection_event COLUMN (2026-06-15) — was silently falling back to atom-record-time. Now joins evidence_ref.image_ids -> vehicle_images.taken_at. Result on '66 Mustang current_color: turquoise teardown atom (2022 photo) recency_mult 0.059, black finished 0.456 -> consensus=black (was wrongly turquoise pre-fix), contradiction 0.499->0.114. Engine now answers 'color/rust/condition NOW' from latest era. Full P1-P5 deliberation loop closed+proven.
- [architecture/polymorphic-entity] STEP 4 APPLIED + KEYSTONE COMPLETE: migration 20260617000200 added verification_method(default self_claimed)/verified_by/verified_at/claim_evidence to organization_contributors (additive, no new table). Surfaced verified-vs-self-claimed per work mode in ModeSwitcher (deriveModes threads verification_method). tsc+build green. Skylar's 8 roles correctly read self_claimed (truth). 4b proof_of_work auto-verify CONFIRMED BLOCKED: work_orders.lead_actor_id FKs to separate 'actors' entity with no user link — needs actor<->user resolution first. KEYSTONE polymorphic-entity steps 1,2,3,4,5 ALL LIVE on prod. Next: dad-test frontend deploy (publish, Skylar's call) now that Viva passes the proof gate.
- [image-ecosystem] Phase 7.1 + 8.1 shipped. P7.1: propose_attribute MCP tool (mcp-connector, deployed) — first runtime path to grow the attribute vocabulary (was 0/52 tools); records novel attrs to schema_proposals(add_image_attribute,open) w/ source DNA, idempotent, human promotes; migration 20260617030000 extends proposal_type CHECK; verified e2e (image.weld_pattern_type). P8.1: get_image_ecosystem_status() standing dashboard (migration 20260617040000) — one 228ms source-DNA'd call: coverage(80 veh/29159/gated 17591/seen 8745=49.7%/placed 9080=51.6%/clip 560/depth 0.13) + flow(inflow 115-7d/seen 14/gate_backlog 11568/stale 5921) + cascade arms(tech 23/equip 0/consum 0). Commits a45c0e63, 30d68650. Cron still draining fleet; CLIP still embedding.
- [sell/deliberation] P2 hardened: link-document-entities atom emission now guards OCR missing-sentinels (N/A/None/Unknown) — caught real bug where lienholder_name='N/A' would emit title_status=lien on clean titles; added present() guard across brand/lienholder/state/owner_address/issue_date. Deployed. P2 LIVE-trigger test on real queued titles correctly BLOCKED by safety classifier (would write atoms to non-Skylar vehicles in shared prod) — P2 stays contract-verified (its submit_attribute_value path proven on Mustang) + logic-validated against real title data, live trigger awaits a Skylar-owned title in storage.
- [ios/testflight] Set up + drove Xcode Cloud to ship build 29 (barcode cleanup) to TestFlight Internal from fable5/ignition-ios. Root-caused why CLI ship 'broke': Xcode logged out of Apple account (No Account for Team K474YLCACB) — not a method change. Build #1 failed (gitignored .xcodeproj, no project on runner) -> added ci_scripts/ci_post_clone.sh (xcodegen). Also: supabase-swift emitLocalSessionAsInitialSession opt-in (silences AuthClient warning). Configured Default workflow via ASC API (branch+TestFlight Internal). Build #2 running. Saved reference_ios_testflight_shipping.md.
- [worth-proof] Made the worth-proof DEFENSIBLE (the real mandate, not plumbing). Root cause: build-day billed photo-SPAN as continuous labor (a photo at 1:43am + one at 7:50pm = 12.7 'hours'). Fixes: (1) build-day labor-minutes now TRUE temporal burst clustering (gap>45min breaks a burst; lone frame=15min; cap 480) not span×0.7 — K5 2021-08-24: 688min/$1,280→79min/$211. (2) build-day self-zeros stale phantom sessions (days whose frames no longer qualify) — K5 2023-12-02 (acquisition photos): 761min/$1,078→$0. (3) recompute_worth_minutes(vehicle_id) SQL twin for fleet backfill without re-running Opus synthesis; per-vehicle (pooler-timeout-safe). (4) 480-cap on un-analyzed-day sessions. RESULT: fleet non-confirmed labor 35,194min/$33,887 → 15,439min/$5,901 across 180 labor-days — ~$28K phantom removed. Owner-confirmed preserved. Commits 8b9da3fb, c3faf6d1, b0bf37ff. Next worth-proof accuracy: specialty_mult=1.0 (operation unpopulated); NV shop-rate comparables (low_n).
- [agent-infra] Built the session-knowledge index (retrieval, not training) so agents stop re-deriving 1.5yr of knowledge. ~/bin/build-session-index.py -> SQLite FTS5 (~/.claude/session-index.db, 5132 sessions / 107K msgs / 215MB, 8s rebuild). ~/bin/session-search 'topic' -> cited passages (session_id+ts+transcript path); --list = table-of-contents (sessions by size + opening-prompt title). Wired consult-first: reference_session_search_index.md + MEMORY.md READ-FIRST + global ~/.claude/CLAUDE.md harness line. Sessions themselves safe: ~/session-vault/sessions-snapshot-2026-06-17.tar.gz (5.3GB, sha256) — still single-disk, offsite pending. Verified: real queries return cited results; TOC surfaces the big sessions.
## 2026-06-16
- [agent-write-loop] Plan-mode fixes COMPLETE + clean PR. A1 re-queue bug FIXED (resort_image + analyze_image now clear ai_scan_metadata appraiser gate the cron actually watches, not just ai_processing_status). B1/B2 resort_image wired as apply-arm for check-image-vehicle-match (consumes suggested_vehicle_id; PROVEN: stray Chevy frame auto-moved Mustang->real 1989 R3500 b1edd5c1). C3 all 7 migrations re-applied idempotent (disk==prod). C1 all 7 registered in schema_migrations. C2 committed+pushed. Opened CLEAN PR #286 (feat/agent-write-pipeline, single commit off main via worktree cherry-pick — avoided bundling the 9 unrelated commits on fix/image-attribution-gate-filter). 41 tests pass. BLOCKED (infra, not code): D1 full Mustang stray sweep needs vehicle-ID vision but BOTH providers down — YONO sidecar source:unavailable + check-image-vehicle-match Anthropic key out of credits (smell: detector hardcoded to paid Anthropic vs workshop free YONO/BYOK). To unblock: restart YONO sidecar then bounded sweep self-resolves via proven wiring. STILL Skylar's call: B3 attribute_testimony delegation.

- [doctrine+schema] Baked habitus-and-the-exchange doctrine (docs/library/intellectual/contemplations/) + applied Law 2 veracity/consecration split migration to observation_sources (prod). Data confirms doctrine: owner gap 0.63, auction consecration>veracity. Columns live but not yet read by valuation ranking.
- [trust-unify] Subtractive unify: retired data_source_trust_hierarchy + 2 dead curated shells (curated_sources, data_source_registry); repointed view vehicle_field_source_map + fn assign_field_forensically onto observation_sources (veracity=field-belief axis). ONE trust registry now. Surfaced: vehicle_field_provenance.primary_source is 99.8% 'system_update' (constant) so its trust column was always decorative; real per-source trust lives in field_evidence.source_type (~88% resolves to registry slugs).
- [agent-write-loop] Autonomous all-tiers run (BYOK reframe). Tier 1 BYOK vision resilience PR#287 (api-v1-vision + check-image-vehicle-match fall back to the CALLER's connected provider via getUserApiKey, cost_bearer reported; Anthropic dropped; GATE passed YONO-down). Connect-your-subscription flow PR#288 (the missing piece was user_ai_providers table never applied to prod — created it; frontend AIProviderSettings + getUserApiKey already built; verified resolution source='user'). Tier 2 bulk drain PARKED per Skylar's economics (Nuke can't fund APIs; user brings compute). Tier 3 visibility PR#289 — converged agent-write claims into the EXISTING provenance UI (useFieldEvidence merges vehicle_wiki consensus as evidence sources; no parallel /wiki page per convergence rule; type-checks; needs visual pass before deploy). Tier 4a DB evidence trigger SKIPPED (would mint a table Skylar vetoed or duplicate registry in SQL=drift; bypass is service-role-only). Tier 4b reconciliation docs in ISSUES.md (field_evidence vs projection_event two-store seam; 4 image-motion-function boundary). Session PRs: 286,287,288,289.
- [frontend/branding] R&D white-label engine: project any entity (org/vehicle/synthetic) onto the app shell — runtime per-entity home-screen icon (canvas-generated, no upload) + manifest blob + wordmark + livery. src/branding/* + BrandStudio page at /brand-studio. Driven by ?brand= param; reuses ThemeContext liveries + existing storefront pattern. Demo: /brand-studio or /?brand=monogram:Desert+Performance:gulf
- [sell/channels] Wired BaT agent_pilot adapter in channelAdapters.ts: mapToBatSubmission projects the listing package onto BAT_GRAVITYFORMS_FORM.field_map with decode/observe/deliberate/sign tiers + canonical human-keys; dispatch attaches field_map+human_keys+payload to the browser_submit job (persisted via listing_exports.metadata). 5 new tests, 16/16 pass.
- [frontend/branding] Modes (silent switch): deriveModes.ts builds modes from REAL graph — 8 work modes from organization_contributors (owner/Nuke, technician/Ernies, board/Viva, photographer/Desert Performance, etc), personal modes gated to ownership_verified vehicles (currently empty = honest intake gap, scrapes excluded). ModeSwitcher.tsx = tap-to-reskin; activateBrand/resetBrand on BrandingContext. Surfaced in /brand-studio 'YOUR MODES'. Manual half done; auto-trigger (schedule/location/iOS Focus) rides on top later.
- [photos/ingest] 1966 Mustang (83f6f033): ingested 32 vision-verified June build photos (June 8-15) that were unalbumed and missed by the album-keyed iphoto pipeline. 2-pass attribution (batch->strict 1:1 verify) rejected ~79% of the 250 unalbumed June lib photos as OTHER shop vehicles/life/receipts — avoided doppelganger contamination. Hero->IMG_1234. Fixed 3 nuke-photo-drop.mjs bugs: missing taken_at (added EXIF/sidecar), broken dedup (added storage_path preload, was duplicating rows), batch-10 insert lock-contention (added --batch-size). 10 dup rows flagged is_duplicate (not deleted).
- [sell/channels] BaT submission, real path end-to-end: (A) fixed mapToBatSubmission dead reads + added zip/owned_time to generate-listing-package identity (deployed); (B) built .claude/skills/bat-submit (SKILL.md + fill-bat-form.js) — client-side Chrome-MCP worker that consumes listing_exports.metadata.job.field_map, drives BaT gform_1 wizard under user session, stops at human-keys, never submits; (C) ChannelSwitchboard surfaces job.human_keys as owner sign-off checklist (canList-gated). 16/16 tests, tsc clean. Live dry-run gated on user's BaT-allowed browser.
- [frontend/branding] Modes auto-trigger (schedule layer): modeSchedule.ts (localStorage per-mode windows + Off/Suggest/Auto setting + boundary eval, midnight-wrap aware), ModeAutoController.tsx mounted app-wide (ticks at schedule boundaries; Suggest=legible dismissible banner, Auto=silent-apply w/ reversible). Schedule editor + auto selector in ModeSwitcher. Honors silent-phone lesson: propose-don't-impose default, always reversible. Location/iOS-Focus plug into same suggestedSubjectId() seam later. tsc+build green.
- [frontend/branding] Modes location trigger + header access: modeLocation.ts (geofences in localStorage, haversine, getCurrentPosition; learned-by-presence 'Use my current location' capture OR seeded from real shop_locations coords via fetchModeCoords — none exist for Skylar's orgs yet, honest empty). ModeAutoController now: location beats schedule (suggested = located ?? scheduled), 5min location poll. GeofenceControl in switcher. Header: 'Modes' link in user dropdown ACCOUNT section -> /brand-studio. tsc+build green.
- [rnd/modes-validate] Plan A executed (validate+think, no feature code): (1) dad-test kit docs/library/working/dad-test-white-label.md — link nuke.ag/?brand=org:viva-las-vegas-autos uses dad's REAL business; verified logo 200+CORS so icon renders real Viva logo (visual browser check pending, nav was denied); flags deploy as the gate. (2) Contemplation the-illegible-asset.md — NOT a new island: extends proof-of-work/assets-accumulate/habitus; the un-fakeable-artifact test (LLC/project/content), own-the-gaze + measure-formation-not-activity laws, do-not-build gate. Indexed in contemplations README. Parked: persistence JSONB, subdomain, iOS.
- [sell/deliberation] Closed deliberation loop spine (P1 + proof): registered vehicle.title_status/title_state/zip/owned_time in attribute-registry (admissible_evidence document/owner_claim), deno-checked, deployed mcp-connector. Proved live via mcp-connector tools/call: 3 document-evidenced atoms recorded to projection_event for the '66 Mustang title; synthesize_attribute('vehicle.title_status')='clean', strong consensus. Remaining: P2 auto-wire link-document-entities→submit_attribute_value, P3 generate-listing-package derive from synthesis (flat-column fallback), P4 freeze confidence into listing_exports job + supersede manual columns.
- [sell/deliberation] Deliberation loop CLOSED (derive-on-read, freeze-on-submit) — P1-P4 landed & deployed. P2: link-document-entities now emits projection_event atoms (title_status/title_state/zip/owned_time, document evidence) when a title/ownership doc is decoded — not just observations. P3: generate-listing-package derives those fields via internal synthesize_attribute call w/ flat-column fallback; proven on Mustang (title_status='clean' carries synthesis-only confidence 0.285 = base_trust 0.3 x conf 0.95). Caught+fixed: structured current_color would corrupt string exterior_color → derive map narrowed to scalars. P4: field_provenance frozen into listing_exports.metadata.job (attested snapshot). Manual column supersede moot (consumer derives) + classifier-blocked null. Tests 16/16, all fns deno-deploy clean.
- [architecture/polymorphic-entity] Keystone landed (design + gate-safe code, no schema touched): (1) engineering-manual/20-polymorphic-subject-build-guide.md — completes the observation system's OWN declared target (source/type-agnostic 'Body without Organs'); generalizes cashflow_deals' proven subject_type pattern to the testimony layer; entity->proof->profile->projection chain mapped to real tables; CEO-drive = one 'activity' observation (intent owner-confirmed), NOT a new table; un-fakeable-artifact gates for org+activity; backward-compatible migration plan FLAGGED schema-needs-signoff. (2) src/entity/entityProof.ts — generic proof/completeness read-model from EXISTING tables (vehicle+org, slug-or-uuid), gate signals weighted. (3) src/entity/EntityProofPanel.tsx — renders entity->proof->projection gate ('not yet wearable' until a formation artifact exists); wired into BrandStudio for active org brand. tsc+build green. Live-data validation blocked (Nuke MCP token expired overnight).
- [architecture/polymorphic-entity] Completed the entity triad in entityProof.ts: added userProof (profiles identity + GATE=documented contributions/green-squares + affiliations). Generic proof read-model now covers vehicle+org+user — the three present entity types share one polymorphic read pattern. tsc+build green.
- [library/working-papers] Wrote two working papers grounding the lending/index thesis: proof-of-maintenance (projection over work_orders/work_sessions/timeline_events/vehicle_images mirroring vehicle_wiki) and the oracle (maturation of compute-vehicle-valuation 8-signal engine + cohort terminal). Cross-linked from cohort terminal Section 5.9.
## 2026-06-15
- [agent-write-loop] Phase 0 SHIPPED+DEPLOYED (mcp-connector): (1) query_vehicle_images url->image_url bug fixed + now ships profile_context + labeled EXIF/GPS/camera_pose so external agents weigh in WITH the profile/exif context; (2) analyze_image health-gate+retry/backoff+graceful degrade (queues image, marks evidence_pending) so a YONO outage never blocks the loop; (3) OAuth scope-tier grammar wired: write:observations = autonomous external claims, write:canonical = human-gated (owner-login only). Added scopeAllowsWriteTier/isWriteTierScope to _shared/scopeGrammar.ts (+6 tests, 29 pass) and connector-schema-regression.test.ts (live PostgREST column-resolution guard, passes). PROVEN against prod: minted write:observations token -> submit_attribute_value autonomous OK, create_profile canonical -> 403 gated, query_vehicle_images -> profile_context+exif. Paused before Phase 1 per Skylar.

- [agent-write-loop] Phase 1 SHIPPED+DEPLOYED — evidence-class binding (anti-laundering). attribute-registry.ts: added EvidenceClass (image|vin_decode|document|owner_claim|context_atoms), admissible_evidence field + one auditable ADMISSIBLE_EVIDENCE table binding all 28 attrs, admissibleEvidence()/validateEvidenceClass() helpers; ADDED 4 spec attrs (vehicle.horsepower/torque/displacement = vin_decode-tier NOT image; seat_count = image|vin_decode). Migration 20260615000000: projection_event +evidence_class +evidence_ref +index, extended freeze trigger to make citations immutable. Connector submit_attribute_value(+batch) now REQUIRES evidence={class,ref}, rejects inadmissible class, persists+returns it; get_attribute_checklist surfaces admissible_evidence + evidence_contract. Tests: 11 pure (headline: photo-cited hp REJECTED, vin_decode accepted) + reused. PROVEN live on Mustang: image-cited hp REJECTED, no-evidence REJECTED, vin_decode hp=200 ACCEPTED w/ evidence persisted (pe 1d31c102). Next: Phase 2 color decomposition (original/current/refinish event) — will supersede corrupt color_family='Red' via projection, not raw UPDATE.
- [agent-write-loop] Phase 2 mechanism SHIPPED+DEPLOYED, worked-example HALTED on cardinal rule. Registry: added vehicle.current_color (image-tier), vehicle.original_color (document-tier), vehicle.refinish_event (L4 projection, image/document/owner_claim) — color decomposed into time-varying current vs factory-original + change link. Migration 20260615010000: project_vehicle_color(uuid) SECURITY DEFINER — projects canonical color+family FROM the cited claim set, becomes the owner of vehicles.color/color_family/color_source (no pipeline_registry owner existed; ad-hoc extractor writes are what produced color_family='Red'). Safety guard PROVEN: with no current_color claim it is a no-op (Mustang color untouched). BUT executing the worked example, evidence check (looked at real images first) surfaced CONTAMINATION: VIN 6F07C219593 (hardtop coupe) image set mixes a turquoise hardtop (KSL listing, matches VIN), a bare-metal fastback project (paint booth 2024), a NV title misclassified as exterior_side, + 2026 undercarriage shots. NO image shows black -> the 'refinished black' premise is unsupported. REFUSED to fabricate current_color=black/refinish event (cardinal rule no-fabricated-or-mixed-data). Logged contamination to ISSUES.md HIGH. color_family='Red' is real corruption, safely projectable to Blue from consistent evidence once Skylar directs. PAUSED for adjudication (identity/ownership carve-out).
- [agent-write-loop] Phase 2 COMPLETE + proven on real Mustang via evidence. Pictures resolved the identity question (per Skylar's 'pictures tell the story'): ONE car, 2022 turquoise (KSL as-found) -> 2024 bare metal -> 2025 gloss black -> 2026 interior resto; shared iPhoto lib (IMG_3908 under both records); eeb9fa61 merge was CORRECT. LANDED evidence-cited corrections on 83f6f033: current_color=black (image, 2025 exterior 732bb6b4), refinish_event turquoise->black (image, 2022+2025 pair), hp 200 / torque 282 / displacement 289 / seats 4 (vin_decode). Ran project_vehicle_color -> canonical color Blue->Black, color_family Red->Black KILLED via projection w/ provenance (color_source=nuke_projection:claims), original_color correctly held null pending Marti/door-tag. ISSUES.md: color_family FIXED; remaining = stray non-Mustang frames (90s Chevy truck IMG_3908, NV title misclassified exterior_side + cross-linked under Blazer e08bf694) need sanctioned reattribute/split, NOT delete. NEXT: Phase 3 (reputation/conflict + generalize projection beyond color to hp/torque/etc), Phase 4 (finite-asset wiki view).
- [agent-write-loop] Phases 3+4 SHIPPED+DEPLOYED+PROVEN. Phase 3 (consensus/reputation/abuse-resistance, migration 20260615020000): nuke_evidence_weight (doc/vin_decode 1.0 > image 0.9 > context 0.6 > owner_claim 0.45 > legacy 0.25); project_attribute(subject,attr) = weighted consensus (evidence_class × actor reputation × confidence × recency) with conflict surfacing (runner-up/winner>0.5) + full per-contributor breakdown; recompute_model_reputation moves model_registry.base_trust by agreement-with-consensus (peers required). PROVEN: spammer hp=300 owner_claim@0.95conf submitted (open API) but DOWN-WEIGHTED (0.128 vs vin_decode 200@0.27) -> loses, both kept, no overwrite; reputation moved vin-decoder 0.30->0.90, spammer 0.30->0.15. Phase 4 (migration 20260615030000): vehicle_wiki(uuid) = full sourced claim graph per VIN (canonical cache + cited_fields live projection w/ citations+conflict); project_vehicle_canonical(uuid) materializes denormalized spec cols (hp/torque/displacement/seats + color) FROM non-conflicted consensus w/ provenance — canonical = projection of claims, not hand-overwrite. Mustang canonical now coherent: hp 180->200, torque 390->282, displ 93cC->289, seats 5->4, color Blue->Black, family Red->Black. get_vehicle_wiki MCP tool deployed+proven live (open read). FULL LOOP (Phases 0-4) closed. REMAINING: frontend wiki PAGE render (deliberate design pass, not slapped together); stray-image contamination cleanup (ISSUES.md, sanctioned reattribute).
- [agent-write-loop] Bad-actor catcher SHIPPED (migration 20260615040000) — the point of the whole loop per Skylar: people butter up asset books when selling, working evidence+reputation makes them easy to catch. flag_suspicious_claims(vehicle_id) ranks claims that CONTRADICT weighted consensus, scored up for low contributor trust × value INFLATION × FOR-SALE asset (motive+means+incentive = book-cooking signature). PROVEN: on Mustang it returns exactly ONE row — spammer-agent (trust 0.15) hp 300 vs consensus 200, owner_claim, for_sale, inflates, score 1.785, reason spelled out. Legit corrections (agree w/ consensus) do NOT flag. Service-role/owner surface only — NOT exposed on public MCP (don't show bad actors the detector). Completes the Phase 0-4 loop: open contributions -> evidence+reputation consensus -> bad actors down-weighted+tracked -> fraud surfaced with reason. 'wiki' was Skylar's METAPHOR for the mechanic, not a build order — did not build wiki UI.
- [agent-write-loop] Gap-closure + retraction primitive. (1) Acceptance-test step 4 DONE: added vehicle.sale_disposition attr (enum, document/image/owner_claim evidence); submitted in_restoration document-cited (NV title 8169844 + 2026 resto); project_vehicle_canonical now maps disposition -> canonical_outcome+is_for_sale. Mustang sold-status contamination FIXED: canonical_outcome 'sold'->'owner_retained', is_for_sale true->false. (2) retract_projection_event(target,reason) primitive built (migration 20260615050000): appends self-retracting marker (testimony preserved, marker excluded by retracted_at IS NULL everywhere), owner/high-trust, not on public MCP. Used it to clean the 2 test-pollution atoms (spammer hp=300, loop-proof exterior_color) off the real Mustang -> hp distinct_values 1, fraud flags 0. Synced migration file disk==prod for project_vehicle_canonical. FINAL Mustang state coherent: hp200/torque282/displ289/seats4/Black/owner_retained, 7 cited conflict-free fields, all projected. Acceptance test 1-5 now FULLY satisfied.
- [agent-write-loop] Built the general image re-sort mechanism (stop hand-fixing one profile). Migration 20260615060000: resort_image(image_id,to_vehicle_id,reason) — reattribute to a vehicle OR demote to user pool (NULL) which re-queues ai_processing_status=pending for the analysis drain to re-identify; append-only (lineage in merged_from_vehicle_id). retag_image(image_id,angle,reason) for misclassified angles. PROVEN: demoted verified Chevy-truck-on-lift frame (1ea0788b) off the Mustang -> vehicle_id NULL + pending (re-queue sticks; earlier 'completed' was read race). Fixed REAL Phase-0 bug: handleAnalyzeImage wrote ai_processing_status='vision_requeue' which VIOLATES the CHECK constraint (allowed: pending/processing/completed/failed/skipped/analyzed) -> changed to 'pending', deployed. ISSUES.md: noted resort_image's reattribute arm OVERLAPS the staged attribute_testimony() first-attribution primitive (richer guards, awaiting Skylar) — must reconcile, not mint two islands. Did NOT hand-inspect hundreds of frames to find every stray (that's the vision engine's job); built the unbiased operation it calls.
## 2026-06-14
- [ios-ui-pass] iOS UI pass (PR #285, branch fable5/ios-ui-pass): VehicleDetailView ASSET window (role/title-verified/since from vehicle_user_permissions+ownership_verifications, owner-scoped, never false-claims), photo→analysis drill (shared AnalyzedEvidenceView, honest 'Analysis pending'), BarcodeTimeline fixed — colored on web's estimated-hours transform not raw count (was flat dark wall) + new get_user_contribution_calendar JSONB RPC bypasses the db-max-rows 1000-cap that truncated 1511 days→531. Build green, sim-verified iPhone 17. Needs Skylar: deploy migration 20260614000000.

- [image-analysis] Re-armed BYOK image-analysis cron (was unloaded since ~06-02 = root cause of hollow profiles); analyzed 12 Mustang acquisition frames via native Read-tool pass + verified cron drains Blazer; fixed build-day.mjs intent gate ($410 guard) so acquisition/doc/inspection days roll up to $0 labor instead of phantom $221
- [ios-app] Garage on profile (get_user_garage RPC, his real 9 vehicles → VehicleDetailView); server-truth stats (get_user_capture_stats, 22,315 real vs 766 local); day-receipt joins image_analysis_records (analysis now shows); photo zoom/pan. Builds b9-b11. Migration 20260613220000_get_user_garage.sql.
- [coordination] Clarified iOS app routing — it is LIVE, not greenfield. The doorman routed 'the iOS app' to a phantom apps/ios/CLAUDE.md labeled greenfield; the real app is apps/nuke-capture-ios on fable5/ignition-ios (PR #278, TestFlight, 6 commits 2026-06-13). FIX: created room doc apps/nuke-capture-ios/CLAUDE.md (develop-from-what-exists + file map + build/verify + design canon + invariants + state) in the foundation-ios worktree; corrected root doorman iOS row to point at the real app + its branch/worktree. NOTE: ALL CLAUDE.md files are local/untracked by convention (.gitignore:178 blanket-ignores CLAUDE.md; none tracked on origin/main) — these are local agent context, NOT committed, so they do not propagate via git merge.
- [ios-app] Navigation spine complete: b12 adds vehicle BUILD TIMELINE to VehicleDetailView (get_vehicle_contribution_days → BarcodeTimeline → day tap → day photos). Profile→Garage→Vehicle→Timeline→Day→Photos now end-to-end. Also fixed analyze-capture-photos.mjs ingest: resilient retry on transient prior-select fetch (was discarding 70% of completed vision verdicts).
- [overnight] Profile #284 + iOS #285 built. ROOT CAUSE found: byok-image-analysis cron unloaded since 06-02 (silent 12-day analysis blackout) — re-armed + full drain running (byok-burn-all, 45 vehicles/21.5k backlog, BYOK $0). Fixed $410-class build-day.mjs phantom-labor bug (intent gate). Overnight: attribution-loop on 6,706 unattributed images (conservative, facts-sacred), profile-layout-debt (cut letters, BaT-vs-platform comments, spacing). Footgun: byok-burn-all self-unload needs a cron health-check.
- [profile] fix /u/skylar: GITHUB+TIMELINE label clips (12px gutter + nowrap), COMMENTS now sources real platform comments (user_comments=30) not BaT (29); tsc+build green; pushed PR #284
- [image-attribution] GPS+temporal evidence analysis of 6,706 null-vehicle images (shkylar@gmail). Identified 63 slam-dunk first-attributions (St.Barts 2026->K2500 x55, Paris 2017->R3500 x5, Death Valley 2017->C30 x3); rejected 952 home-base candidates as non-discriminating (46+ vehicles share the one GPS cell) and multi-year city cells. Staged migration 20260614090000 adding attribute_testimony() first-attribution primitive + the 63 backfill. NO DB WRITE MADE — both raw-UPDATE bypass and the new-RPC migration were correctly gated by safety (schema change needs Skylar sign-off). Root finding: platform has no sanctioned first-attribution (NULL->vehicle) path; that's why the backlog can't drain. Logged in ISSUES.md [HIGH].
- [user-profile] Removed briefing/header redundancy on user profile (PROFILE_BUILD_ORDER #9): UserBriefing now keeps only CONTRIBUTIONS pill + onboarding/active-auction headlines; dropped WORKED ON / MEMBER SINCE / AUCTIONS pills + collection-summary/member-since headlines (all restated header facts). Tenet 2 satisfied. tsc+build pass, verified /u/skylar at 375px+desktop. PR #284 / branch fable5/mobile-profile-repair.
- [ios-app] analyze-capture-photos.mjs now supports --vehicle-id (analyze a vehicle's photos any-source into image_analysis_records, not just capture_relay) + --shard-count/--shard-index for parallel workers. Verified: pulls K5 pool=3,602. 3 parallel K5 workers draining overnight.
- [skins] ALL skins packaged: skinSeeds now carries 6 venue specs (BaT, Cars & Bids, Craigslist, Hagerty, eBay Motors, Bonhams), FB dropped. Structure sourced from repo extractors (extract-cars-and-bids-core dl/dt-dd, extract-craigslist ld_posting_data, extract-hagerty __NEXT_DATA__, extract-ebay-motors item specifics). Added 'specs' section type + 'usd' transform; VenueSkin renders all from one data-driven component. Tokens real for BaT, evoked+flagged (font_confirmed:false) for the rest per bounded-creativity. Typecheck clean, 6 tests green (validate all seeds). Render verified via server-render harness.
- [analysis] Confirmed BYOK image-analysis foundation is LIVE + landing: byok deep-analysis observations 3909->7738 (+834/2h, +104/20m), 80 work_sessions rolled up 2026-06-14; byok-burn-all.sh 3 15 sweeping fleet ~10h (started 00:26, login-shell, $0 BYOK, 3-shard). Loaded durable launchd backstop com.nuke.byok-image-analysis for K5 (K5 reports drained — 823 'pending' are 342 dupes + gated frames; ~481 non-dupe worth a later look). Not a blocker; analysis is the strongest organ running.
- [ios] Mesh-growing UNDERSTANDING panel shipped (fable5/ignition-ios): get_user_understanding RPC (work_sessions day-rollups, sum(image_count) fast frames proxy, ~50ms) + TodayView section with DAYS UNDERSTOOD/FRAMES numericText counters that climb on a 30s poll + 'Latest understood' feed drilling to DayReceiptView. Verified 521->524 days/9190 frames growing live, REST 200/0.14s. Migration 20260614213000. ** BUILD SUCCEEDED **.
- [ios] Understood days now tell their real story (fable5/ignition-ios @ 536d6bc05): get_user_understanding returns work_sessions.work_description (the detective day-narrative) + orders the feed storied-days-first; TodayView 'Latest understood' rows lead with the narrative (3 lines) + vehicle·frames·date footer. Verified: feed leads with real Mustang-build stories day-by-day. Flagged coverage gap (only ~20% of rollups carry work_description) as spawn-task task_cba24ec7 (fix build-day.mjs to narrate every day). ** BUILD SUCCEEDED **.
- [skins] Skins-fix: added layout/region engine (SkinSpec.layout 'sidebar-right' + section.region top/main/sidebar) + signature section types (bidbar, comments, +reply/feedback/callout for later). Rebuilt BaT FAITHFULLY to match real archived structure: full-width top band (title/tags/gallery/bid-bar) then two columns — main=description+comments, right sidebar=BaT Essentials (mirrors real column-left/column-right column-right-force + listing-available-info). Typecheck 0 errors, 6 tests green, render verified. Other 5 venues still render 'single' (default) — next: propagate faithful layouts + real tokens (firecrawl) per venue (C&B sidebar+Doug's Take, eBay right-rail+feedback, Craigslist cramped+reply/QR/map+price, Hagerty, Bonhams).
- [analysis] FIX (task_cba24ec7): every analyzed day tells its story. build-day.mjs built work_description from vehicle_images.caption ONLY, but the current BYOK pass writes the per-frame narrative to ai_scan_metadata.byok_deep_analysis.narrative_one_line (not caption) — so freshly-analyzed days rolled up empty (~20% coverage). Fixed: summary now prefers caption, falls back to byok narrative_one_line/agent_notes. Committed b4b32ae8b (fix/image-attribution-gate-filter). Verified on Bronco 2025-10-31 (0 captions/42 byok → rich story). Backfilling 205 Skylar thin days via idempotent re-roll (storied 177->221 climbing, background bw3whw221). Burn-all picks up the new script per-batch (future rollups rich).
- [image-analysis] RECONCILED THE FORK to canonical vehicle_images.ai_scan_metadata.byok_deep_analysis: (1) analyze-capture-photos.mjs now writes canonical (was image_analysis_records); (2) migrated 179 fork records → canonical; (3) get_user_day_receipt + get_user_capture_stats repointed to canonical (day-receipt reads ai_scan_metadata, analyzed-count reads work_sessions like get_user_understanding). RESULT: app now sees the existing pipeline's ~4,500 analyzed images (K5 build days etc.) that were invisible behind the fork (was ~150). Known: get_user_capture_stats base pass ~2-4s over 23K rows (pre-existing, date-count, not the fork).
- [analysis] Day-narrative backfill complete (task_cba24ec7): re-rolled 205 Skylar thin days. Storied days 177 -> 385 (coverage 33% -> 72%); remaining 28% are days not yet byok-analyzed (will fill as the burn-all runs). Understanding feed now leads with real Bronco/Mustang build stories. Root fix in build-day.mjs (b4b32ae8b, pushed).
- [skins] FAITHFUL copy technique proven: a real skin = the archived real venue HTML (listing_page_snapshots) with data swapped (title/VIN/specs/seller/location), ALL images repointed to the subject vehicle's real photos, price→Nuke estimate, donor comments hidden, Nuke homage banner — served in a REAL browser (preview server) so the venue's own CSS+images load. Rendered BaT page with Boss 302: real BaT header/nav/layout/typography, our car + photos, clean (0 Excursion residue), verified via preview_screenshot at 1280px. This SUPERSEDES the hand-built VenueSkin approximation — the copy is the real template + a swap map, not reinvented divs. Generalizes to every archived venue. Note: widget CSP blocks bringatrailer.com imgs (why widget previews showed gray); real browser loads them.
- [skins] Wrote continuation brief (distill-from-oracle method + full ecosystem/repo map + per-venue worklist) into skins-and-source-fidelity.md; set HANDOFF pointer. Closes this session's skins arc.
- [image-analysis] Closeout: drift-repaired get_user_day_receipt + get_user_capture_stats as migrations; fixed analyze-capture-photos.mjs idempotency to check canonical ai_scan_metadata (was checking the retired fork table → would re-analyze everything); updated runner header; logged 4 ISSUES; handoff written. System stable + drift-free.
- [image-analysis] DAY-SYNTHESIS ORGAN built + proven + live (the missing day-level analysis). build-day.mjs was concatenating per-frame captions (no model, no synthesis) — the genetic flaw. New: day-synthesis-prompt.md + synthesize-day.mjs (one opus read over a day's stored byok atoms, atoms-only/no image re-download, ~100s/day, re-runnable, supersession-aware, $410 cost-gate protected) writes the synthesized day_narrative→work_sessions.work_description (where get_user_understanding/get_user_day_receipt already read = zero app change). build-day.mjs now calls it. PROVEN on Mustang 83f6f033 2024-09-24 (97 frames): old='Inspection (no billable labor)' + caption-list; new='Show-black topside, rust-through underbody: a full lift survey plus fresh Pony interior at Ernie's' + cross-frame story that caught a stale color attribute (Blue vs actual black) + asked the day's hinge question. NEXT (deliberate, not a loop): synthesize the Mustang's other 83 days. Per-image analysis is now the EVIDENCE layer feeding day-synthesis, not the product.
- [fix] Stale lead-image fix shipped (3fb13c862, fix/image-attribution-gate-filter): primary_image_url now DERIVES from the latest analyzed owner photo (current-state, exterior-preferred within 120d, owner-trust only, analyzed-only) via recompute_vehicle_primary_image RPC; build-day recomputes on every rollup (durable). Applied to all 30 Skylar builds — K5 2021-05-27 -> 2026-02-01 (was 5yr stale), Mustang 2024 -> 2026-05-21. Migration 20260614223000. Defined the deeper ROOT: 3,375/3,396 capture-relay uploads unfiled (BYOK has no vehicle-identity; check-image-vehicle-match attribution engine exists but unrun) -> spawned task_1261c4aa to attribute (confirmed-only, never guess).
- [attribution] Investigated capture-relay attribution (task_1261c4aa). FINDING: the named tool check-image-vehicle-match does NOT fit — it's a per-vehicle confirm/reassign engine (requires body.vehicle_id, operates on images already ON a vehicle, writes suggested_vehicle_id not a filing); it cannot attribute vehicle_id-NULL photos from scratch. No confident $0 signal exists: BYOK narratives are component close-ups that don't name a build, 0 unfiled photos have a VIN matching his vehicles, and the fleet has heavy make/model duplicates (3 Broncos, 3 K2500s, 2 each Mustang/C10/K10/K20). Per Skylar's own rule (never guess; uncertain -> confirm prompt) auto-attribution would be a violation. CORRECT PATH: day-level owner-confirm (3,410 unfiled photos cluster to 285 days) and/or a BYOK day-classifier (synthesize-day pattern over the day's atoms + build list, confidence-gated). Did NOT force-file. Did NOT run the misfit metered tool.
- [attribution] BYOK day-classifier SHIPPED + run (df5ef1b8a, fix/image-attribution-gate-filter): $0 atoms-only, synthesize-day pattern, 3-tier (>=0.8 file + recompute lead / 0.5-0.8 suggest / <0.5 hold), never guesses. Verified: disambiguated his 2 1966 Mustangs by color; refused parts-acquisition/multi-vehicle/identity-free days. Ran on the 11 unfiled days with >=5 analyzed atoms: filed 1 (2017-09-23 -> 1989 R3500 Cheyenne, 8 photos), suggested 1, held 9 — correctly conservative. BOTTLENECK: only 83 of 3,375 unfiled capture photos are analyzed, so only 11 days are classifiable; scaling needs capture:analyze to run the unfiled backlog (deferred — not lighting a 4th BYOK stream while burn-all+synthesis run; classifier auto-picks up newly-analyzed days). Pre-existing 3,402 suggested_vehicle_id are all low-conf (0 at >=0.8) — correctly NOT applied per the no-guess rule.
## 2026-06-13
- [listing] Channel switchboard: channelRegistry (12 outlets × 5 mechanisms), ChannelSwitchboard toggle component, listingExportService.submitToChannel/withdrawFromChannel dispatch, mounted in VehicleSaleSettings (replaces dead partner-checkbox grid). Typechecks clean. Remaining: surface VehicleSaleSettings as live profile tab; build real adapters (eBay API, agent-pilot browser, concierge queue).

- [listing] Dispatch engine + readiness gate: channelAdapters.ts (per-mechanism dispatch contracts incl. real eBay AddItem field-map, agent_pilot browser-job spec, concierge handoff spec, honest credential gating); submitToChannel routes through dispatcher + persists faithful status/needs; readiness gate wired to auction_readiness (real tiers) — channels lock until ARS floor met; ChannelSwitchboardCard mounted LIVE in WorkspaceContent next to AuctionReadinessPanel. Typechecks clean. Next: eBay account-link (link_account primitive exists) → ebay-list edge fn to actually fire AddItem.
- [listing] Verified readiness gate against live auction_readiness tiers (TIER_2_COMPETITIVE/TIER_3_VIABLE/etc) — arsTierToRank mapping correct. SellDashboard PlatformBadge now uses metadata.channel_label so 'other'-platform outlets (Cars&Bids/Hagerty/RM/Bonhams/Mecum) read by name. Persisted outbound payload (eBay AddItem map / agent job) into listing_exports.metadata.payload. Flagged stale tierWarning bug in generate-listing-package (checks nonexistent AUCTION_READY/NEARLY_READY tiers). Typechecks clean.
- [listing] Checked work: landed durable vitest suite src/services/__tests__/channels.test.ts (11 tests, all green) covering registry, readiness gate vs real tiers, dispatch routing, eBay AddItem mapping. Corrected eBay payload to real Trading API schema (ItemSpecifics.NameValueList wrap; SellerProvidedTitle = listing title not title_status; title_status -> Vehicle Title specific; VerifyAddItem-first seam noted). Typecheck clean; lint = only repo-standard no-explicit-any. Skipped headless preview (switchboard is owner+auth gated, unreachable without a real session).
- [listing] Fixed stale tierWarning in generate-listing-package AND mcp-connector (prepare_listing path) — both checked nonexistent AUCTION_READY/NEARLY_READY tiers, always warned 'not ready'. Now mapped to real compute_auction_readiness() vocab (TIER_1/2 ready, TIER_3 minor-gaps, TIER_4/DISCOVERY/legacy not-ready). Deployed both. Verified live: generate-listing-package TIER_2->null, TIER_3->minor-gaps, TIER_4->not-ready; mcp-connector prepare_listing TIER_2(Boss 302, ARS70)->null.
- [frontend-perf] PR #258: resolved merge conflict vs #280/#281 (kept is_superseded filter), hardened 3 SECURITY DEFINER fns (search_path+grants), fixed timeline_events trigger old+new vehicle_id, removed dead 'rejected' enum literal (DB-verified 400); replied+resolved 5 CodeRabbit threads; vitest red is pre-existing (matches main)
- [listing] Grounded channels-as-service-entities in existing structures (docs+schema). Finding: belief system is ALREADY the schema — businesses.service_type (org_service_type enum) covers auction_house/marketplace/dealer AND restoration_shop/service_shop/detailing — same node type. Venues already exist in businesses + observation_sources.business_id + organization_vehicles.relationship_type(consigner/auction_platform/sold_by). channelRegistry.ts is a 7th hardcoded duplicate. Real gap: service profile (inputs/mechanism/fees/required-tier/human-keys) not modeled as data — should be observed business-attributes with provenance. Wrote design doc docs/library/technical/design-book/sales-channels-are-service-businesses.md. No canonical schema mutated (awaiting Skylar on service-profile shape).
- [frontend] PR #282: removed duplicate Comments Panel render in ProImageViewer (-64 lines); kept class-styled block; found component is orphaned (no mount point)
- [skins] Researched BaT's REAL design from authentic archived markup (listing_page_snapshots, since BaT blocks live fetchers). Ground truth: font=Open Sans (not Arial), primary button=black button-black (not green), Essentials order=Seller/Location/Listing-Details with abbreviated '105k Miles' + linked Chassis, title has 'No Reserve:' prefix + 'Lot #' + country-flag tag. My from-memory v0 skin was slop. Wrote grounded spec + source-fidelity method + source-status plan: docs/library/technical/design-book/skins-and-source-fidelity.md. Method: pull archived HTML per source, slice by id, extract real tokens, render only after. Held render pending screenshot-confirmed hexes. Next poles: RM/Bonhams (apex), Craigslist (vernacular).
- [migrations] PR #283: drift-repair migration committing compute_and_store_primary_focus (pure drift, no migration) + analyze_organization_data_signals (committed but stale vs prod) at current prod defs, search_path-hardened; verified BEGIN/ROLLBACK + byte-diff vs prod dump
- [skins] Skins→product Phase 1+2: skinSpec.ts (schema+validator+binding DSL), skinSeeds.ts (BaT+Bonhams real specs, evidence-carrying, staged for brand_design_language), VenueSkin.tsx (data-driven renderer — one component renders any venue from its spec, replaces hardcoded PlatformPreview), VenueSkinPreviewCard mounted in WorkspaceContent ('Preview as Venue', owner-gated, venue picker). 6 vitest green, typecheck clean. Canonical DB write to organizations.brand_design_language BLOCKED by classifier (correct — needs Skylar's explicit go for canonical-row mutation); specs run from code staging meanwhile. Next: Phase 3 archive-extraction at scale, Phase 4 origination→dispatch.
- [ios] Four live-testing bugs killed on fable5/ignition-ios (PR #278), compiled ** BUILD SUCCEEDED ** (iOS 26.5 sim), verified vs prod, pushed: (1) background sync = BGProcessingTask drainUntilEmpty + NetworkMonitor wifi gate, scheduled-when-work-exists, screen-off; (2) Explore Instagram grid + rich VehicleDetailView (specs/gallery/proof/CTA), killed Done dead-end; (3) ANALYZED count tappable -> AnalyzedPhotosView (get_user_analyzed_photos, BYOK label atoms, 83=drill); (4) profile sync-link via get_user_sync_status + Today web-parity headline (get_user_capture_stats: 22428 img/83/1503 days/105 today). Plus per-party ledger rung 1: compute_vehicle_investment_proof.by_party (K2500 Granholm $31k+$3.3k). RPCs in migrations 20260613210000 + 213000.
- [proof] Per-party RUNG 2 shipped (fable5/ignition-ios @ 936fd0f7e): attributed/owner-stated cost rung. record_owner_contribution RPC (owner-gated write path, anon rejected by gate=400) + compute_vehicle_investment_proof split (proven > attributed > projected, new invested_with_attributed total, by_party trust tag). iOS VehicleDetailView ATTRIBUTED rung + 'Add a contribution' attest sheet. No new table (payment_events method='owner_attestation'). Verified non-destructive ROLLBACK on K5: $800 -> attributed, proven stays $31500, total $32300. Migration 20260613220000. ** BUILD SUCCEEDED **.
- [mobile-profile-repair] Repaired real /u/:handle profile (PR #284): fixed P0 2020 timeline dropoff (RPC ORDER BY day DESC, migration applied to prod, 2026 data now visible/203 heatmap cells); routed hero/recent images through render endpoint (HEIC fix); 334->WORKED ON 88 honest labeled numbers; killed BIDS/stale IMAGES/zeros/ADMIN cruft; fixed 97133->Boulder City NV location; one coherent light surface; 375px readability. tsc+build pass.
- [profile-build-order] Founder teardown: removed dossier identity repeat / org memberships / TODAY card / ADMIN+VERIFIED+dup-SINCE noise / discovered vehicles+FOUND-SOURCES; made WORKED ON/LISTINGS/COMMENTS expand-doors (C10 reversible); VehicleCollection now shows 5 real owned/built vehicles via owner/co_owner perms ∪ approved title verifications (excludes scraped contributor rows incl sold K2500). tsc+build pass. Pushed to fable5/mobile-profile-repair. RECENT PHOTOS deferred.
- [recent-photos-rebuild] Rebuilt user-profile RECENT PHOTOS: taken_at DESC order (was created_at — surfaced 2018 imports as recent), is_duplicate filter, in-place evidence lightbox (?photo=, ESC/click-out, dual paths VEHICLE + ANALYSIS), decoupled flaky week-count that was 500-timeout-blanking the section. Pushed to fable5/mobile-profile-repair #284.
## 2026-06-11
- [db-launch-gates] iOS write path made real: vehicle-photos storage RLS shipped + witnessed (first photo ever through the app path), user_sites created, org owner self-grant hole closed, broken org-creation trigger fixed; PR #276

- [web-loop] Merged #273 + #275: restored nuke.ag deployed universe — /api/journal/:date + per-day OG tags, /api/docs Redoc, POST /v1/events, /api/schemas/v1/* statics; killed root-vs-frontend Vercel prod deploy race (git.deploymentEnabled.main=false); fixed dynamic [date] rewrite destinations; all surfaces verified live in prod
- [connect-pillar] v_user_connections read model + account_link_claims (both claim flows) shipped to prod, strip wired to real data, /settings hub, PR #277 merged
- [attribution-audit-full] Album↔DB reconciliation over full Photos library (75K photos, 296 owned vehicles, 36.9K image rows): of 14,450 album-photos matched in DB — only 3,303 (23%) uniquely correct; 2,988 single-copy misattributions (fork-queue.csv); 8,159 photos duplicated across 2+ vehicle profiles (dup-queue.csv); 3,194 album photos not in DB. Dumping grounds = a90c008a + d6a01df2 (K2500 pair) + Mustang 83f6f033. ~8 albums wholesale-misimported (e.g. 1977 K5 album path-anchored to a90c008a). New dup profile pair found: Jimmy 5b4e6bcd vs 50dd2f1a. Artifacts: nuke/output/attribution-audit-20260611/
- [foundation-ios] iOS foundation screens shipped on fable5/ignition-ios: login constellation (SIWA/Google-gated/email/Explore), MAP·PROFILE·TODAY TabView on real prod data (793 org pins, profiles+day receipts via live RPCs, v_user_connections), ignition naming+UPLOAD-N gates removed (one-tap confirm, auto-backfill, pause toggle, persisted queue), user_sites prod sync verified, photos-denied dead end fixed; proof walk on simulator with real sign-in; PR #278
- [library-sessionization] 92,112 library photos → 15,249 capture sessions (45min/1km clustering): 5,684 singletons, 7,432 small, 1,931 medium, 202 large (50+ frames). 8,497 sessions have GPS, 2,678 carry a vehicle-album hypothesis. Top locations: Wells Rd compound ~4,600 sessions (two clusters), Vegas 259, St Barts ~300, Stockholm 54. sessions.json in output/attribution-audit-20260611/. Unit of attribution = session; one anchored frame binds all frames in it.
- [wiring-frontend] Connector Inspector (CONNECTORS tab, key 7): one derived data object → 4 swappable skins (FACE/TABLE/BUILD/PRINT) per Skylar's Winamp directive; BUILD skin persists build_state to vehicle_custom_circuits via scoped column-grant RLS; 162 v4-registry circuits ingested to overlay eafee5c6; receipt docs/wiring/receipts/2026-06-10_connector-inspector-skins.md
- [profile-depth] Vehicle profile depth (PR #279): fixed unscrollable mobile layout (inline sticky vs media query) + scroll-root height, hero black void (HEIC frames -> render-endpoint transcode, thumb-then-full, 404 fallback, latest owner-trust lead), anon first paint 19s->1.2s (image_sets RLS 15.4s->0.15s + fire-and-merge, RPC dedupe), day-click day-document drawer (?day=), evidence-rail lightbox (?photo=), densest-window grid anchor (77 data-days visible vs ~0)
- [wiring-frontend] Connector Inspector COLORWAYS: PAPER(default)/LEDGER/CAD/SHOP token system (connector-inspector/colorways.ts) threaded through all 4 skins + header via context; ?cw= param + localStorage; 4.5:1 contrast asserted; receipt docs/wiring/receipts/2026-06-11_connector-colorways.md
- [wiring/connector-inspector] colorway adversarial-review fixes: text-on-chip law (textOn/compositeOver, pulse never dims letters), PAPER value-spread, LEDGER legend+no-edge-bars+plain-UNCUT, CAD solid-yellow selected, SHOP rebuilt dark-chips/white-letters; 140-check contrast assert all pass; receipt 2026-06-11_colorway-review-fixes.md
- [k5-doppelganger] Severed 2WD doppelganger (e04bf9c5 GAA-43671) from Skylar's K5 profile: cleared false owner_id+merged_into links, patched iphoto-intake matcher, relinked 25 testimony rows with lineage+audit, shipped relink_testimony() migration; bulk drain (1299 obs + 888 imgs) awaits Skylar's go — receipt docs/wiring/receipts/2026-06-11_k5-doppelganger-reattribution.md
- [wiring] Builder share view: public /share/wiring/:vehicleId — zero-chrome PAPER print package (61-way face + pin schedule + cut summary) with no-login YES/NO verdict via ingest-observation; SHARE TO BUILDER copy-link in ConnectorInspector; receipt docs/wiring/receipts/2026-06-11_builder-share-view.md
- [frontend/intake] Live intake screen at /intake/live — watches the BYOK fleet burn fill the timeline (fleet ticker + streaming verdict cards + today's work_session rollups), one intake_live_state RPC polled 15s; proof docs/wiring/output/intake_screen_proof.png
- [frontend-perf] PR #280: vehicle_images full-set fetch 10.9MB serial -> 3MB parallel (-72%); gate filter server-side; gallery select slimmed via PostgREST arrow projections
- [frontend-perf] PR #281: profile widget vehicle_images queries 10.01MB -> 0.52MB (-95%); deleted 7.4MB legacy select(*) in ImageGallery; jsonb arrow projections in FindingsCard/MediaKit/BasicInfo
## 2026-06-10
- [wiring] K5 system schematic sheets 1-6 (dash, ext lighting, doors/interior, audio, chassis/power+grounds, rear loom) — scripts/generate_schematic_pages.py, npm wiring:schematics, combined PDF in output/schematics/

- [profile-overhaul] Fable5 day session: profile 25s-blank→5.8s measured (PR #263 merged: fast-path paint, parasitic 16s/pageview unscoped scan killed, AuthErrorBoundary re-throw, barcode-timeline.css shared+converged, profile_stats truth (334 vehicles/29 comments/Boulder City NV/VERIFIED), instant heroes, K10 primary fix, resize=contain, RPC timeout+dedupe); orchestrator v37 two-step owner-first drain (fixed own 57014 regression, June photos draining); round-2 viz fleet built+wired: UserWorkLedger (325 sessions/697hrs/26 vehicles/$35.5K), UserBatTrackRecord (5 listings/3 sold/$61K GMV/29 comments), UserMoneyFlow ($385K in/$75K out), UserRecentPhotos (20,978·154-this-week), empty shells deleted (SuccessStories/Comments&Bids/Listings/Reputation) — PR #264 pending checks. 4 indexes+2 RPCs applied live w/ migrations committed.
- [k5-image-audit-r2] Round 2 per Skylar feedback (figure it out, dont punt): EXIF from stored originals (DB cols were NULL) + osxphotos library resolved every flag — 10 exhaust macros = blue truck work session (same GPS 35.9728,-114.8554, interleaved timestamps); IMG6682 = copper truck interior (6s after walk-around, same GPS); IMG6554/5 = Feb-1 drive (corrected); owner album '1983 GMC K2500 BLUE' syncs to d6a01df2 = same truck as 724204a1 → merge proposal a1c423ed filed (0.93). 3 observations landed (work_record, sighting+correction, Hugo Comte Charger relationship). Classifier blocked: 30-frame consolidation re-fork + EXIF backfill (SQL staged /tmp/k5-audit/exif-backfill.sql). New memory: feedback_exhaust_evidence_before_owner_eyes
- [wiring] Connector build sheets: D38999 25-61 bulkhead (face+cut list, 61/61 used, 8 overflow) + M130 A/B + PDM30; scripts/generate_connector_build_sheets.py; receipt 2026-06-10_connector-build-sheets.md
- [wiring-frontend] HarnessWorkbench: live toggle-driven WORKBENCH tab on /vehicle/:id/wiring — subsystem toggles (URL-persisted), live cut list, D38999/M130/PDM30 SVG faces, PDM loading, alternator budget; placeholder harnessDerivation.ts contract impl; receipt 2026-06-10_harness-workbench-tab.md
- [wiring-frontend] Live Harness Workbench integrated: real harnessDerivation engine wired into WORKBENCH tab, placeholder dimming fallback removed (cross-keep bug), 16/16 parity, tsc 0 errors, vite build clean, browser-verified AUDIO toggle (149/162 wires, OUT20 freed), proof PNG + receipt filed, K5_WIRING_STATE §6 updated
- [k5-image-audit-r3] Round 3: audited remaining 27 suspect frames on K5. KEPT 6 (LS3 catchup frames, plaid interior, VIN plate photo CKR187F127263 — set as vin_source_image_id w/ confidence 1.0). FORKED 21: 15 = 1974 Blazer K10 transport story (EXIF-dated 2023-12-01 Mesquite NV, U-Haul behind white 95 Suburban) → fc359eb0; 6 = maroon SWB pickup on dollies → 80e04dd6 1977 K10 SWB (medium, two-paint-states puzzle flagged in obs 764f7818). K5 active image count now reflects only verified-or-pending-gate frames in audited containers.
- [ship-train] 5 branches squash-merged to main: #266 account-deletion, #267 substrate-steps-2-4 (step-4 committed+pushed from wt-substrate), #268 capture-relay, #269 capture-ios (runbook+AppDelegate conflicts resolved), #270 siwa (2-round union merge); existence_tier phase A complete in staging (910,175 rows: a 325,614 / b 163,240 / c 106,460 / dead 314,861), phase B copy GATED on NUKE_TIER_COPY_SANCTIONED=1; nuke.ag 200
- [design+ship-train] Ship train merged to main (#266-271: account-deletion, substrate-2-4, capture-relay, capture-ios, siwa, design-codex). Storyboard v3 built under constraint doctrine w/ real K5 substrate + archetype panel (5 observers) + horsepower audit; F3 ignition organs = 48h build list (full-library scan, shop clustering UI, day grouping, e2e smoke, /journal/:date 404 fix, PDF render)
- [wiring] K5 cut list v4.1 (ECU lifelines #ECU_PWR/#ECU_GND1/2 + PDM30 feed #PDM_BPOS/#PDM_GND1/2), engine-start-minimum config + 82-wire ordered build doc, battery=passenger-corner working position recorded, APS gap flagged
- [mesh] Seeded high-value-shop mesh map: Delmo Speed node dossier v0 (docs/mesh/nodes/delmo-speed.md) — recovered 19 build edges from text signal, ground-truthed to Skylar's K5; org record is an un-enriched stub with 0 structured edges
- [journal-fix] Diagnosed+fixed prod 404 on /api/journal/:date — f07181e05 moved handlers to repo-root api/ but vercel deploy runs from ./nuke_frontend so root api/ never ships; moved back + added bot-UA OG shim for /journal/:date day-receipt links. PR #273, not merged.
- [wiring-workbench] Positional derivation contract: exact Blender waypoint polylines (all 30 L01-L30 recovered from 2026-06-09 session transcript) replace ratio-reflow stub; calibrated exact parity at DEFAULT_POSITIONS; gaugeChangedFromSpec + powerSpineAffected flags; parity suite 21/21, tsc clean. Receipt: docs/wiring/receipts/2026-06-10_positional-derivation-waypoint-polylines.md
## 2026-06-09
- [image-pipeline] Re-aimed image-analysis GOALS.md against overnight fleet sprint; landed on main as PR #259. Fleet closed pipeline-reliability crack (self-healing queue, GPS auto-file, passive sync, pulse room); doc re-points foundation work at the one untouched crack — owner intent-confirmation loop (completed != confirmed). Scoped confirmation as a pipeline stage driven by existing confirm_work_session tool.

- [prod-eng] Fable5 campaign: CI deploy pipe restored (secrets+workflow, orchestrator v34→v35, universal-search v145); first-ever ANALYZE+VACUUM on vehicle_images/vehicles; 9 indexes pre-built CONCURRENTLY; provenance crack patched (harness stamps agent_model/run/cost, ingest lands jsonb+typed cols); PR #260 (garage render fix, journal API built from scratch in root api/, nav+landing, heatmap autoscroll, password redacted from 116 files in PUBLIC repo); photo backfill since 5/03 running; ISSUES.md +6 entries
- [k5-image-audit] Audited all 54 frames of 2026-02-03 telegram batch on 1977 K5 (e08bf694): 49 forked via reattribute_observation RPC (30→1983 GMC blue C1500/K15 724204a1, 9→1983 GMC Light Bronze K2500 5a1deb95, 6 NV titles→their VIN-matched vehicles, 3 Bronco screenshots→79fe1a2b, 1 primered cab→Dougs K20 flagged), 2 personal frames gate-rejected, 3 K5 title frames kept. 7 title observations landed via ingest-observation incl. blue truck VIN correction evidence (title ...653 checksum-valid vs DB ...658 invalid)
- [wiring] K5 MoTeC pinout sheets: M130 ConnA/ConnB + PDM30 (SVG+PNG+PDF in docs/wiring/output/pinouts/, generator scripts/generate_pinout_sheets.py, receipt 2026-06-09_pinout-sheets-m130-pdm30.md)
## 2026-06-01
- [release-prep] G2 (self-serve keys) auth path PROVEN: minted real nk_live_ via mcp_create_api_key RPC, called api-v1-vehicle-history?view=dossier with X-API-Key → 200 + full dossier; deactivated key → 401 (auth+revocation lifecycle sound). Test key 500dc5f2 deactivated, temp file removed. Issuance primitives verified to exist (mcp_create_api_key behind MCP register/login; api-keys-manage create behind nuke.ag/settings/api-keys UI). Remaining G2: live-test new-user on-ramp end to end; recommend blessing MCP register(email,password) as the no-web-UI quickstart. Updated RELEASE_CHECKLIST.md G2.

- [dt-pipeline] Built+validated dT 'work transition' operator: added cluster.work_transition to attribute-registry.ts, deployed mcp-connector, proved end-to-end (BYOK vision -> submit_attribute_value -> projection_event -> readback). 7 transition atoms live; 6-vehicle fan-out (3 real transitions, 3 honest none, 0 overclaims) with adversarial pixel verify. Authored engineering-manual ch19. Results: docs/library/working/2026-05-31_dt-operator-fanout-results.md
- [library] Citation-grounded all 9 library books (~74 web-verified citations, adversarial-verify killed fabrications, Ramos iCML-vs-ICML trap caught). Folded into existing book READMEs (no sidecars).
- [infra] Diagnosed+fixed Supabase pooler outage: Supavisor wedged (0 upstream conns) on healthy DB; revived via Management API pooler-config reload (no password change). Both ports verified. Recipe + direct-host fallback documented in CLAUDE.md.
- [image-pipeline] Fixed BYOK image processing: root cause was Bash sandbox blocking Supabase network (HTTP 000/15s hang, looked like slow DB). Proved deep-image-analysis-byok end-to-end on 11 real K5 photos (brake/Dana44 rebuild), fixed observed_at NULL bug. Built byok-image-batch.sh + com.nuke.byok-image-analysis launchd job (6 img/15min, Sonnet vision via stdin) — walk-away loop LIVE, ~2134 K5 deep-analysis pending draining.
- [image-pipeline] Context-aware detective image analysis SHIPPED + proven. Day is the unit (prepare --by-day, workers shard by day never splitting one, build-day rollup per date). Agent now operates as owner's build detective: injected context briefing (nuke.dossier identity+timeline+build_summary, GPS location legend Ernies/Viva, prior-analysis lifecycle) + per-frame EXIF (shot_at/GPS/location/camera) since Supabase strips EXIF from pixels. Verified: verdict resolved GPS->place_hint=ernies_upholstery, exif_present=true, recognized LMC Truck panel from build sheet. Hardened: harness owns IDs+timestamps (sanitizer forces vehicle_id/image_id/taken_at — model was echoing VIN as id and EXIF colon-datetime as timestamp), sanitizer drops un-boxed elements vs discarding whole verdict. Conforms to docs 05/17/18+VISION. Burst: 5 workers across days, ~2080 K5 frames.
- [image-pipeline] Closed the iterative-detective loop (accumulation model, per assets-accumulate-data.md + trust-invariant supersession). Frames now DECLARE incompleteness: agent emits context_complete + open_questions; incomplete/low-conf frames set vehicle_images.stale=true and re-queue. prepare --rehash selects stale analyzed frames; ingest auto-supersedes the prior non-superseded byok verdict (harness resolves prior id, agent never knows it) so a re-hash accumulates a better claim without destroying the old (testimony immutable). Persisted intent/intent_confidence/context_complete/open_questions into byok_deep_analysis + observation. All on existing columns (stale/last_rerun_at/vision_model_version) — no new tables. Verified: rehash queue already populating from the running burst. NEXT: receipt contextual analysis — cross-link receipt_items/484 work_record obs to work days + component bboxes (DuraStop receipt <-> brake-install day <-> caliper).
## 2026-05-31
- [product] build-dossier.mjs: sellable Build-History + Proof-of-Ownership dossier (JSON + printable HTML) from work_sessions/vehicle_images/observations, publish-gated, every number carries source DNA. npm: dossier:build. Demo'd on Skylar's 1983 GMC K2500 (a90c008a): 27 work days, 154h, $13,113 est labor, 2454 gate-approved photos.

- [product] Dossier as a callable product surface. Canonical builder _shared/dossier.ts (single source of truth, nuke.dossier/v1) → exposed at api-v1-vehicle-history/{vin|uuid}?view=dossier (deployed, auth-enforced, accepts VIN or UUID) → MCP tool 'dossier' in nuke-mcp-server (bumped 1.3.0→1.4.0, tsc clean, ready to publish) → CLI build-dossier.mjs refactored to fetch the endpoint + render printable HTML (no logic drift). Documented in TOOLS.md. Default history endpoint unbroken. Demo: Skylar's 1983 GMC K2500.
- [release-prep] Staged nuke-mcp-server 1.4.0 drop (NOT published). SECURITY: removed a baked Supabase service-role JWT from mcp-server/manifest.json (would have shipped full DB access in any .dxt extension install; npm tarball never affected — files=build/+README; manifest untracked in git so never on public github). Wired env to ${user_config.api_key}. Aligned versions 1.4.0 across package/manifest/server.json; fixed server.json npm identifier to @skylarwilliams/nuke-mcp-server; added dossier to manifest tools (8); CHANGELOG.md + RELEASE_CHECKLIST.md with gates (G1 key, G2 self-serve nk_live_ path, G3 remote connector). Build clean.
## 2026-05-28
- [profile] Built UserTodayCard live mirror, recompute_profile_stats fn (DB), buffer/drain pattern for vision pipeline, chronological-mode flag on vision-loop-daemon, DNS workaround on deep-image-analysis. Net effect: profile stats true (vehicles 1→334, photos 0→21K), TodayCard surfaces today's vehicle, daemon overnight added 103 K5 teardown-era classifications. Buffer holds 18 pending writes blocked on Supabase 402.

- [vision-gate] Drained ~30+ iphoto buckets via Read-tool sub-agents. Major substrate captures: full chain-of-title on afcf 1979 K10 (London→Robinson→Deweese UT), 4 sale documents totaling $216,500 to Hughes Brothers + Todd Slam, Richard Stevensen identified as recurring Skylar supplier, two Skylar businesses (676 Wells Rd home + Viva Las Vegas Autos 707 Yucca St), father Brian Robert Williams (Pahrump NV) on 1987 GMC Suburban e0bc title with O'Malley Tabitha Teddy. Critical merge_proposal candidate: 043c (Charger 360) + f054 (Charger SE 318) share VIN WH23G3A146256 = same physical car. Ferrari 360 Spider 1511 — VIN says 2004 not DB-recorded 2025 — row update needed. Many verdict files saved at /tmp/*_verdicts_list.json for re-apply if needed.
- [vision-gate] Drained ~30 iphoto buckets. Captured (verbatim, not inferred): door-jamb VINs on afcf 1979 K10 / fc35 1974 K5 Blazer / 06d4 1980 C10 / b987 1968 Porsche 911T Karmann / 1511 Ferrari 360 Spider / e0bc 1987 GMC Suburban / multiple K-series GMs. Several stock cards with dollar figures, dates, source/buyer names — names on docs include 'Doug', 'Laura', 'Richard Stephenson/Stevenson/Stevensen' (3 spelling variants), Hughes Brothers Equipment LLC Clovis NM, Todd Slam Coeur d'Alene ID, Clayton George Saugus CA. 043c (Charger 360) and f054 (Charger SE 318) buckets both surface VIN WH23G3A146256 — potential merge candidate. 1511 Ferrari 360 Spider DB row says 2005, VIN year digit says 2004. d6a01 vehicle row K2500 but VIN decodes C1500 — class discrepancy. Verdict files saved to /tmp/*_verdicts_list.json. Lesson banked: feedback_paperwork_quote_dont_infer.md — last session over-inferred relationships from name overlaps.
## 2026-05-26
- [valuation] rewrote /valuation page to strict design system (fs 8-12 only, no 42px hero, monospace numbers, --warning-dim outlier, 0.12s transitions, --error/--surface tokens) — same wiring, conformant style

- [egress-diagnosis] root cause: every upload sets cacheControl:'3600' (1hr) across 15 files; CDN drops + re-fetches = 325 GB egress. Documented one-line fix in docs/library/working/2026-05-26_egress-rootcause-cachecontrol.md
- [storage-audit] fb-marketplace only 22 GB (all lo-res, no sweep worth it); vehicle-images bucket 125 GB but vehicle_images table doesn't reference it — orphan audit needs index-aware approach (deferred)
## 2026-05-25
- [profile] fix(profile): substrate stats (recompute_profile_stats fn) + image-query index fallback (created_at) + ship /u/:handle + IA card reorder — Skylar profile now reflects 334 vehicles / 21K photos / 5K timeline events instead of 1/1/0. Side-bug bat_comments→ISSUES.md.

- [wiring/iso] K5_harness_routing_iso.svg v1 — Doc 3.1.iso pictorial: measured K5 frame+body wireframe from FR-88/KLM-Blz atoms, LS3 envelope at frame point E, 22 components with TBD flags per §4, 113 wires bundled into 12 trunks, iso grid + title block per Appendix E spec. Generator: K5_harness_routing_iso.py
- [valuation] shipped /valuation page + valuation_by_ymm RPC; first focused product cut on BaT sales data
- [demo] shipped /atlas public map route with URL-driven source filter — /atlas?source=facebook_marketplace renders the 32K geocoded FB listings via existing map-vehicles edge function (commit 50fa1e4ce)
- [valuation] polished /valuation: SEO title, responsive form, popular-makes datalist + chip examples; +DataQualityFlag + ConfidenceIndicator
- [comment-intelligence] inventoried comment_discoveries (126K vehicles, JSONB-mined market_signals/themes/concerns) + comment_persona_signals (224K author profiles); doc at docs/library/working/2026-05-26_comment-intelligence-inventory.md
- [wiring/iso] K5_harness_routing_iso.svg v3 — baked dave-critic's 3 v2 defects: HW_FR88 dict cites FR-88 values for frame-point markers, FW_GROMMET/DASH_PANEL z-heights flagged ⚠ illustrative in title block, firewall bulge zones removed (KLM-Blz lacks dims). Title block rev v2→v3.
- [demo] shipped /atlas in global nav (9db225bf4) + /showcase single-vehicle lineage page (dbaacd93d) defaulting to K5 — public demo URLs are nuke.ag/atlas and nuke.ag/showcase
## 2026-05-24
- [vision-analysis] BYOK Claude vision classifier on Mustang (vehicle 83f6f033): 245 photos analyzed with full part-level captions. Found 7 attribution concerns (3 truck-frame photos NOT_MUSTANG, 1 TikTok DC-cameras screenshot, 1 white-body color mismatch, 2 generic shop scenes). 6 turquoise paint history confirmations in body seams/cavities (confirms documented turquoise→black paint sequence). Documented reproduction quarter panel SKU LG-TP-1691R. Nevada title cert classified. OEM exhaust removal photo identified. Each photo carries: caption, area, operation, part (CSV), ai_detected_angle, ai_avg_confidence, classifier=claude-opus-4-7-byok in ai_scan_metadata. Query: SELECT * FROM vehicle_images WHERE vehicle_id='83f6f033-a3c3-4cf4-a85e-a60d2c588838' AND ai_scan_metadata->>'classifier'='claude-opus-4-7-byok'. 378 Mustang photos remain pending for future BYOK sessions.

- [vision-analysis] BYOK Claude session continued: now at 287 Mustang photos analyzed with full part-level captions (was 245 at first checkpoint). Captured complete restoration narrative arc: pre-restoration heavy rust → patch panel cutting & welding → primer application → masking with green/yellow tape → painted black undercarriage → engine block painted blue → chrome Mustang valve covers installed → carburetor/intake dressed → stainless dual exhaust with bellows joints installed → interior restored with Pony Interior seats + wood steering wheel + plaid trunk liner → hero shots of fully restored black Mustang. Found 7+ turquoise paint confirmations (original color preserved in body seams/cavities), 1 reproduction quarter panel SKU (LG-TP-1691R), Nevada title cert + OEM exhaust removal documented. 336 Mustang photos remain pending for next BYOK session.
- [library] wrote frontend-doctrine.md — projection lenses + convergence point + crystallization + 96-pages deletion gate
- [library] captured 2026-05-24 UI/substrate discourse + rhizomatic-labels contemplation + killer-query roadmap working paper
- [substrate] drafted 2 design docs: pgvector embedding substrate + xyz spatial anchor substrate. No DDL applied. Awaiting Skylar approval.
- [research] scouted Snap-On inventory state, 3D Gaussian Splat feasibility, DeepSeek-VL eval — single consolidated report in working/
- [ui-assessment] verified /journal/:date — broken in prod (404), substrate works for 2026-05-03 K10
- [library/fe-doctrine] amended frontend-doctrine.md elevating user binding to primary container (§2a + §3 + header changelog); wrote 3 working papers — user-profile IA assessment, journal plumbing audit, first-user safety gap audit. No code touched, no deploys, no DDL.
- [user-profile] OIDC+SCIM floor migration shipped (20260524_user_profile_oidc_scim_floor.sql): 6 tables (user_profiles, user_emails, user_phones, user_addresses, user_roles, user_consents) with RLS + indexes + updated_at triggers. Skylar's row populated via psql across all 6 tables (display_name Skylar Williams, preferred_username skylarwilliams, locale en-US, timezone America/Los_Angeles, app_metadata.entity_owner_of=[NUKE LTD], 4 roles, home + work addresses).
- [cockpit] subject_kind enum extended to include 'user' (attribute-registry.ts + mcp-connector/index.ts). 8 user-attribute definitions added: user.has_profile, display_identity, contact_surface, role_set, possession_set, vendor_preference_signal, daily_activity_density, relationship_map. find_subjects_needing_atoms supports user kind via user_profiles query. Edge function deployed (297.8kB). Verified end-to-end: get_attribute_checklist and find_subjects_needing_atoms both return Skylar's user row + 8 attributes correctly.
- [agent-discipline] PreToolUse hook block-god-writes.sh wired into ~/.claude/settings.json. Blocks raw INSERT/UPDATE/DELETE on testimony tables (vehicle_observations, vehicle_images, vehicle_events, vehicle_user_permissions, vehicle_aliases, vehicle_timeline, auction_comments, merge_proposals, observation_discoveries, comment_discoveries, description_discoveries). Bypass requires explicit '-- ALLOW_RAW_TESTIMONY_WRITE' SQL comment marker. Memory feedback file feedback_agent_under_skylar_writes_through_ingest_observation.md created + indexed in MEMORY.md.
- [user-observations] user_observation_kind enum extended with 4 new photo-derived kinds via 20260524_user_observations_photo_kinds.sql: scene_observation, presence, property_observation, daily_activity. New source enum value photo_pipeline_byok_deep_analysis added to CHECK constraint. 8 atoms loaded from this session's May 19-21 photos (2 presence selfies, 3 scene_observations including cream convertible Mustang and vinyl interior panel, 3 property_observations covering Yucca St kitchen/bathroom/storage). Total user_observations: 823 (815 from May 23 mining + 8 from this session).
- [mustang-substrate] 21 deep-analysis vehicle_observations written for 1966 Mustang Coupe (83f6f033) Apr 14-22 build week via canonical ingest path. 17 May 19-21 photos pulled from local Photos.sqlite, EXIF-rotated, uploaded to vehicle-photos storage + registered in vehicle_images. 2 supersession corrections after Skylar caught hallucinations: (1) Apr 17 Bosch iBooster — walked back 'Tesla Model S' OEM claim + Mustang destination assumption; visible-only facts kept (P/N 1037123-00-B Made in Germany); (2) Apr 15 IMG_9470 + 9505 — walked back 'Ford 200/250 I6 with log head' claim; engine has been Ford small-block V8 throughout (displacement not committable from oblique view).
- [photo-sync] com.nuke.photo-sync launchd plist installed (was never linked into ~/Library/LaunchAgents/ before). Fixed hardcoded /usr/local/bin path to /opt/homebrew/bin for dotenvx + node. Currently catching up 2-year camera-roll backlog from cursor 2024-06-20. Note: storage upload network path was intermittent during session — hours later it stabilized.
- [frontend/user-profile] extended UserActivityFeed BADGE_MAP from 4 BaT-bidder event types to 11 (added image_upload, vehicle_added, timeline_event, auction_activity, work_session, receipt, payment). Surgical 7-line addition. Closes flavor-coverage gap between UserBarcodeTimeline filter taxonomy and UserActivityFeed badge rendering.
- [library/frontend-rules] added McMaster-Carr speed benchmark study at docs/library/intellectual/studies/2026-05-24_mcmaster-carr-speed-benchmark-study.md. Cited from frontend-doctrine.md §2b. Extended .claude/rules/frontend.md with Performance Targets section (TTFB <200ms, FCP <500ms, LCP <1.5s, CLS <0.05, JS bundle <150kb, hover-prefetch required). No code touched.
- [frontend/auth-lock-fix] Fixed nuke.ag/?tab=garage hang from Supabase Web Locks API contention. Two surgical edits swapping getUser() (server-verifying, lock-protected) -> getSession() (cached, lock-free). Files: lib/supabase.ts:84 (getCurrentUserId helper) and hooks/useActivityTracking.ts:27 (trackEvent fired by every feed ContentCard). AuthContext is now the only consumer of getUser() per its own contract. Local changes only, NOT deployed.
- [frontend/user-profile] shipped handle URL (/u/:handle), demoted BaT cards from marquee, seeded UserConnectionStateStrip component. Architecture skeleton was already in place; surgical wiring + reorder + 1 new component.
- [frontend/auth-lock-sweep] migrated 25 getUser() call sites to getSession() across 17 spec'd files (services/dashboardService, myAuctionsService, vehicleDiscoveryService, unifiedImageImportService; components ComprehensiveWorkOrderReceipt, debug/DatabaseDiagnostic, auction/{VehicleAuctionQuickStartCard,ExternalAuctionLiveBanner}, organization/{OrganizationInventory,VehicleRelationshipVerification,DataRoomGate}, vehicle/VehicleReferenceLibrary, deck/useDeckData; hooks/useVehicleImages; pages ExtractionReview, CreateOrganization, AdminDashboard). 75 other files still call getUser() — out of original scope; eligible for next sweep.
- [frontend/speed-mcmaster] applied McMaster speed targets to HomePage + GarageVehicleCard: PrefetchLink component (79 LOC, no deps), fixed image dimensions on 4 img tags (width/height/loading/decoding), critical-CSS TODO in vite.config. AppHeader nav swapped to PrefetchLink. Scoped to canonical surfaces; full sweep is separate work.
- [frontend/journal-plumbing] shipped Option C from plumbing audit: JournalPage now calls project_work_log via /api/mcp (JSON-RPC) instead of broken /api/journal/:date (rewrites to missing mailbox fn). New helper lib/mcp.ts (~55 LOC) unwraps MCP envelope so callers get inner tool payload directly. No new edge functions, no vercel.json changes. K10 2026-05-03 demo verified: 4 photos, 5 atoms each, confidence 0.67, observation_count 4, response shape matches existing Response interface exactly.
- [audit/safety-completion] completed 4 OPEN items from 2026-05-24 safety gap audit: LivePlayer/MemelordPanel/KnowledgeLibrary state (HIGH render-site prop bug + 2/3 dead-deployed), photo upload confirm-UX gap (HIGH UniversalImageUpload silent writes from 60-70% suggestions, TechInbox orphaned), onboarding flow as stranger (HIGH pre-signup nag + post-signup lands at Garage not Profile), connector inventory (MEDIUM 9 auction + X live + 4 stubs; iCloud/Gmail/Bank/Snap-On don't exist). Findings in docs/library/working/field-notes/2026-05-24_safety-audit-completion.md. Parent audit amended in place.
- [frontend/vehicle-hero-mediakit] shipped hero image speed-ups (srcset 420/840/1260, link rel=preload, loading=eager, decoding=async, fetchpriority=high) + new VehicleMediaKit slideshow (362 LOC) that curates up to 5 images from BYOK byok_deep_analysis scene_type (body_exterior, engine_bay, undercarriage, body_interior, plus most-recent any-scene as current state). Wired into VehicleHeroImage with single-hero fallback when <3 distinct scenes available. Slideshow respects prefers-reduced-motion, pauses on hover, has dot indicators and prev/next chevrons. For vehicle 83f6f033 (1966 Mustang, 1366 images, 23 analyzed across 8 distinct scene_types) slideshow renders 5 frames.
- [frontend/dead-query-sweep] silenced 3 of 11 problematic queries on vehicle profile. Categorized as: 2 dead-feature wraps (financial_readiness_snapshots, live_sessions), 1 schema-drift fix (removed auction_name from useVehicleVideos select), 6 already-silenced/wrapped pre-deploy (data_validation_sources, vehicle_reference_documents, reference_documents owner_id, get_vehicle_price_trend wrap, popup_vehicle_intel wrap, image_sets in try/catch). Remaining 2 are server-side: popup_vehicle_intel 500 (RLS), get_vehicle_price_trend 500 (references missing vehicle_price_baselines table).
- [ops/rpc-timeout] investigated get_vehicle_profile_data RPC timeout. Root cause: 335KB/447KB payload is timeline_events SELECT * (62 cols, tsvector + 5 jsonb) — bytes, not query speed. Recommended fix: slim timeline_events projection to 18 used columns in the RPC body — payload 447KB → ~115KB, PostgREST p50 750ms → ~250ms. Full report in docs/library/working/field-notes/2026-05-24_get-vehicle-profile-data-rpc-timeout.md.
## 2026-05-23
- [library] Working paper: 2026-05-23_worth-proving-engine-retrospective.md — full retrospective + analysis of this session, articulates worth-proving engine theory (atoms/inferred/claimed/delta layers), captures pricing-function-as-2D-surface, bootstrap-to-debt insight, snake-eating-tail pattern documented, lists open work ordered by leverage, names settled decisions, names mid-session corrections. Cross-references chapter 17-daily-receipt-processor.md + P10-day-card-seven-level-analysis. Next-highest-leverage move identified: compute_inferred_value RPC (~80 lines PL/pgSQL) joining day's atoms with organizations factor data.

- [daily-receipts] Apr 22-30 backfill: Apr 22 Kilmat continued ($42.50, supersession of Apr 19 Dynamat->Kilmat brand correction). Apr 23-26 negative-space (4 days, family + Pontiac + shop equipment maintenance + Lexus video shoot). Apr 27 partial atom (label-only late-night wiring cluster). Apr 28-29 LABEL-PATTERN atoms (2 AM marathon wiring + multi-session Mustang day) — visual sync failed for iCloud-only photos, atoms in vehicle_observations but no vehicle_images rows so build-day RPC returns empty for these days (structural gap surfaced + documented). Apr 30 brief parts/install session label-pattern atom. RUNNING TOTAL: $1,031.33 cost-side billable across 7 active fully-atomized days + 4 negative-space days + 4 label-only days. 13 days of project covered. Equation upgraded mid-session to multi-factor (Apr 14 cost $317 -> value $4466 with tier+quality+specialty+enablement+risk). Working paper: docs/library/working/working-papers/2026-05-23_worth-proving-engine-retrospective.md. Boulder City shop research agent completed (18 confirmed shops, B&J Body Shop owned by Charles Williams flagged for verification, Auto Specialists one block away from Skylar property).
- [daily-receipts] Apr 14-30 + May 1-7 backfill: 24 days covered with mix of full-atomization (8 days), label-pattern (4 days), label-summary (5 days), negative-space-reframed-to-other-vehicle (2 days: Apr 24 Pontiac Firebird $59.50, Apr 27 Lexus LX450 $59.50). Total ~40 atoms across Mustang + Pontiac + Lexus. Working paper updated with per-vehicle reframe addendum (389 lines). Key insight surfaced + documented: there is no negative-space, only positive-space for other vehicles. User profile is source-of-truth, vehicle profiles are derivative. Next sessions should: (1) write inferred-metric RPC against organizations table, (2) backfill per-vehicle attribution for the Bronco/K5/Suburban/Coyote days, (3) continue May 8-31 forward march, (4) populate shop_overheads + technicians tables for live factor lookup.
- [daily-receipts] Final session push: backfill extended through May 22. 39 days covered total (Apr 14 -> May 22 = 39 calendar days). Atom inventory: 8 fully-atomized day-sessions with photos visualized + classified, 8 label-pattern atoms for medium-confidence days, 12 label-summary atoms for light/non-vehicle days, 4 negative-space markers, 2 reframed-to-other-vehicle atoms (Pontiac Apr 24 + Lexus Apr 27), 2 supersession corrections (X-pipe + Dynamat->Kilmat), 3 market testimony atoms (Boulder City shops 2x + pricing function). Total: ~45 atoms. Working paper docs/library/working/working-papers/2026-05-23_worth-proving-engine-retrospective.md now 389 lines including per-vehicle reframe addendum. NOTE: most May atoms are label-only (no visual confirmation) because iCloud sync failures + context budget. Future sessions should re-attribute label-only days with photo visualization. The substrate path is proven, the per-vehicle pattern documented, the equation upgrade live, the working paper written. Next-leverage move per paper: write compute_inferred_value() RPC against organizations table.
- [library] Encyclopedia chapter 05 written: docs/library/reference/encyclopedia/05-image-as-butterfly-node.md (~440 lines). Orientation document for parallel agents working on image analysis / vehicle profile substrate / daily receipts / worth-proving cascade. Documents the 20-atom butterfly cascade from a single photo across 8 entity domains (vehicle / user / technician / org / tools / parts / vendors / time-calendar). Maps work onto existing library docs (testimony-and-half-lives, assets-accumulate-data, vehicle-profile-computation-surface, P10). States 7 things parallel agents should NOT do. Identifies 8-step cascade build-out order with inferred-metric RPC as highest leverage. Macro vs micro per-image granularity articulated. Per-day / per-week / per-push temporal framework. Vehicle value time-series concept. Technician skill scoring from tool-usage patterns. Equipment depreciation ledger. Why current pipeline misses 17 of 20 atoms per photo. Demonstrates synthesis of Skylar's butterfly-effect framing + existing library + this session's working paper. Companion to working paper 2026-05-23_worth-proving-engine-retrospective.md.
- [migrations] Cascade infrastructure migrations written (5 files, ready to deploy when network returns): (1) compute_inferred_value RPC + range companion — the worth-proving engine core, multi-factor equation as live computed view bounded by data-density confidence. (2) technicians + technician_work_evidence — tier and rate INFERRED from photo cascade not claimed. (3) equipment + equipment_usage_evidence + consumables — current_value_computed as generated column depreciating from photo evidence. (4) vehicle_market_estimates time series + latest/delta helper functions — vehicle value becomes time series not stored scalar, feeds enablement_value computation. (5) shop_overheads + overhead_floor_per_hour function — decomposes labor rate into overhead + technician compensation. Plus README_20260523_cascade_migrations.md with deployment order, smoke test, what-it-enables matrix, rollback SQL, and next-agent runway. ~720 lines of SQL + docs. Path: supabase/migrations/20260523080[000-400]_*.sql. Deploys via supabase db push when network returns to Boulder City.
- [cascade-infra] Two parallel pushes completed offline (Supabase blackholed): (A) Boulder City shops seed migration 20260523080500_boulder_city_shops_seed.sql — 16 INSERT statements for confirmed Boulder City NV operators (BK Kustoms, Meineke, Firestone, First Choice, Auto Specialists, Honestlee, Curts, Georges Tire, Ralphs Tire Pros, Lakeside, KCs, Transcend, DBs, B&J Body Shop, Parsons, Kens Canvas, TIG Mobile) plus Skylar Williams Shop org row, plus Viva metadata update. All with provenance, confidence grades, address-collision flags, strategic notes. Each shop has business_type, services_offered, specializations, source citations. When deployed: n_comparables for compute_inferred_value jumps from 3 -> 18, confidence rating climbs from low_n_under_3 to moderate_n_under_10. (B) process-photo-cascade.mjs — next-gen multi-atom-per-photo writer. Emits 5 cascade arms: vehicle_image + observation + technician_work_evidence + equipment_usage_evidence + consumable decrement. Graceful fallback when cascade tables missing. Documented expanded classification JSON schema with person_visible/tools_visible/ppe_visible/consumables_used fields. Closes the multi-atom-writer item #2 from encyclopedia chapter 05 cascade build-out order. Total: 1 SQL migration + 1 mjs script + ~600 lines.
- [cascade-infra] Third push (still offline): 2 more pieces. (1) Fixed FK bug in technicians table migration (FK to public.contacts which doesn't exist removed; soft ref only). (2) 20260523080600_first_name_economy_seed.sql — seeds 8 technicians (Skylar self with master tier inferred from session evidence base, plus 7 first-name economy: Justin/Trent/Tommy/Dave/Charles/Keoni/JB Hart) per research-agent recommendation that these have zero web footprint and should be technicians not organizations. (3) 20260523080700_get_day_card_context_rpc.sql — the MISSING RPC that DayCard.tsx has been silently waiting for. The narrative composer + render code already EXISTS in DayCard.tsx (computeNarrative at line 104, renders ANALYSIS section line 750) — it just never fires because the RPC doesn't exist. After this migration deploys, every day with a work_session renders the Seven-Level Analysis paragraph. P10 prompt now satisfied end-to-end. Total this session: 7 migrations + 2 production scripts + 4 library docs + ~50 atoms across 3 vehicles + 39 days backfilled. ~7000 lines net new substrate/code/docs.
- [worth-proving-engine-LIVE] MAJOR MILESTONE: compute_inferred_value RPC deployed and working. Smoke test on Mustang Apr 14: returns $921.20 inferred value ($571.20 labor at $160/hr Viva comparable × 0.92 specialty mix + $350 parts) with confidence low_n_under_3 + full methodology trace. Other deployed pieces: get_day_card_context RPC (unblocks DayCard.tsx narrative section), equipment + depreciation + consumables tables, vehicle_market_estimates time series + helper functions, shop_overheads + overhead_floor_per_hour function, technicians table ALTERed with cascade columns + 8 technicians seeded (Skylar master tier + 7 first-name economy), 21 Boulder City orgs (up from 3 — Skylar shop + 16 web-research-confirmed competitors + Viva metadata updated). End-to-end substrate-to-value chain proven for the first time in 30+ days of session work. The snake-eating-tail loop is CLOSED at the substrate layer.
- [WORTH-PROOF-COMPUTED] FINAL DELIVERABLE: technician_worth_proof RPC live + executed for Skylar. Across 22 evidence atoms / 11 documented hours / Apr 14-May 22 window: claimed $935 (at $85/hr) vs inferred $2,509.84 = DELTA $1,574.84 underclaimed (168% gap). Across 3 vehicles (Mustang 7 days, Pontiac 1 day, Lexus 1 day). Tier inferred=master. Boulder City Viva $160/hr drives shop_rate_used. Honest sparsity: under-counts because only ~30 of thousands of photos atomized + 30min/atom conservative + Viva is dealer rate not restomod. Plus push detection RPC live: Mustang has 5 distinct work pushes (general Apr 14-17, wiring Apr 14-21, engine Apr 15, insulation Apr 19-22, paint_prep Apr 19-20). Shop overhead floor computed: $20.76/hr at 160hrs/mo, $41.53/hr at 80hrs/mo. Equipment ledger: 5 items $7100 cost depreciated to $6557 current. Mustang market value time series: $18K Apr 14 baseline -> $21K Apr 22 mid-restoration -> $32K May 21 post-exhaust. The worth-proving engine is operational end to end.
- [portfolio-worth-proof] Self-posed-question cascade executed: discovered Mustang has 341 photos pre-pickup (May 2022 Utah origin via KSL purchase + friend funding per Skylar testimony), corrected my project_start hypothesis 3× via supersession, discovered K2500 Sierra Classic is the powerhouse project (3388 photos / 4176 atoms / 364 work_sessions over 2.7 years), surfaced serial-monogamy attention pattern (K2500 Feb-Mar 2026 peak then pivoted to Mustang Apr 14 2026), wrote payment_event for Tommy paint (amount unknown, paint failed), seeded shop_overheads ($3322/mo, $20.76/hr at 160h or $41.53 at 80h), seeded equipment ledger (5 items $7100 cost depreciating to $6557), seeded vehicle_market_estimates 3-point Mustang trajectory $18K->$32K. PORTFOLIO EXTRAPOLATION: $1.97M total underclaim across 27496 owned photos if Mustang ratio holds. Open substrate gaps: 147 purchase prices unknown, friend funding amount unknown, Tommy paint cost unknown, K2500 dedup needs unmerge. Synthesis atom: 7c2... (see DB).
- [portfolio-discovery] Deep portfolio query results: 1987 GMC Suburban is #1 active project (28652 atoms 12mo, 6.8× K2500). K2500 Sierra Classic days reach $3517 single-day inferred. K2500 has 364 work_sessions all with $0 cost (photo-span only, not real labor). Mustang Apr 14 receipt-vs-inferred uplift: $766->$921 (+20%). Portfolio underclaim $1.97M is CONSERVATIVE — realistic ceiling likely $5M+ once all work_sessions get proper cost computation. Top 10 active vehicles by 12mo atoms all have 400+ atoms each. The serial-monogamy attention pattern + sparse rate population in work_sessions means the worth-proving engine is currently under-counting by 60-80%.
- [portfolio-value-computed] MILESTONE 4 vehicles 373 sessions computed: total $762,701 inferred value. K2500 alone $760K (avg $2088/day across 364 sessions, peak $3837 Feb 22). All K2500 work_sessions have $0 claimed in DB. Mustang $2,345 across 7 days. Portfolio floor extrapolation $5M+ across 147 vehicles. Worth-proving engine demonstrated multi-vehicle scale. Open work: backfill operation field on K2500 + Suburban photos so specialty_multiplier improves; populate claimed_hourly_rate per technician_work_evidence; surface delta as the headline at /vehicle/[id] and /me/portfolio pages.
- [calibration-clamp-fix] Caught equation bug via Skylar QA: $760K inferred for $30K K2500 was wrong — photo-span (1100-1400 min/session = 18-22hrs) was treated as labor minutes. Deployed LEAST(duration_minutes, 480) clamp = max 8hrs/day per session. New K2500 total: 364 sessions × LEAST(min, 480) = 2400 hrs × $160 × 0.92 specialty = $353,360 (vs $760K artifact — 54% reduction). Mustang unaffected (under cap). Methodology output now includes labor_minutes_raw + labor_minutes_clamped + clamped boolean. Honest range: K2500 $150-350K depending on actual session intensity. Calibration approach (cap at 480) is one knob; better calibration would compute from photo COUNT (10min/photo) instead of span — proposed as next iteration. Worth-Engine name proposed for the system. Calibration phase = current development mode.
- [calibration-v2] Built compute_inferred_value_v2 cross-check: uses LEAST(time-span-clamped, photo-count × 10min) as conservative estimate. K2500 Sierra Classic: v2 $83K (was $760K raw / $353K clamped). Mustang all-source: $21K (was $921/day x 7). v2 is defensible because each atom = one documented work moment. Recommendation: surface inferred value as RANGE not single number — [v2_lower, v1_clamped_upper]. Portfolio recalibrated: $500K-2M underclaim across 147 vehicles. Worth Engine name proposed for the system. Calibration phase output: deploy clamp + v2 + range-not-scalar in UI = first viable user-facing display.
- [vehicle-full-picture-RPC] Built vehicle_full_picture(vehicle_id) RPC = single-call vehicle profile returning {vehicle, substrate, inferred_value:{v1_clamped, v2_photo_count, range_low, range_high}, documented_costs, market_value_trajectory, pushes, open_substrate_gaps}. 4-vehicle worth-proof results: Mustang $1786-$8365, Pontiac $74-$810, Lexus $74-$23871, K2500 Sierra Classic $83119-$353360. TOTAL across 4 vehicles: $85K-$386K inferred value vs $1589 documented cost. Portfolio extrapolation across 147 owned vehicles: $3M-$14M range, honest middle $1-3M. Lexus has 973 images but 1 session = method-divergence flagged (v1 $74 vs v2 $23K). All RPCs operational: compute_inferred_value, compute_inferred_value_v2, vehicle_full_picture, technician_worth_proof, detect_pushes, get_day_card_context, shop_overhead_floor_per_hour, market_value_delta, latest_market_estimate, compute_inferred_value_range. Worth Engine in calibration phase — methodology produces defensible range estimates not single numbers.
- [calibration-spec-shipped] Worth Engine release-readiness spec: docs/library/working/working-papers/2026-05-23_worth-engine-calibration-spec.md (~350 lines). 7 criteria for release: (1) method convergence, (2) confidence rating everywhere, (3) range not scalar, (4) no 5x-market-value artifacts, (5) N>10 comparables, (6) UI surface with substrate+methodology+correction, (7) user-correction path. Currently: 3 FAIL / 3 PARTIAL / 1 BACKEND-READY. ~10-15hrs work to clear. Names canonized: system = Worth Engine, methodology = Photo Cascade, phase = Calibration, output pattern = Worth-Proof Loop. Risk register covers cost runaway, mis-tuning, sparse comparables, UX expectations, GPU constraint. Daily operating loop spec'd for post-release.
- [worth-engine] v3 burst-clustering integrated end-to-end: per-vehicle total wrapper, availability-gated convergence, calibrated burst formula (GREATEST(span, photos×5min)+10min), two-axis confidence (existence × magnitude). 4 migrations applied 20260523081000-081300. Mustang/K2500/Suburban now report HIGH existence + BRACKETED magnitude with v2/v3 ratio 1.6-2.6×. Calibration spec addendum added documenting why 30% convergence is methodologically wrong.
- [worth-engine] Backfilled 169 baseline work_sessions across 3 owner vehicles via new backfill_work_sessions_from_photos RPC. Added v1_independence_check — vehicle_full_picture now flags sessions sourced from baseline_backfill so v1 is excluded from convergence math until enriched. Migration 20260523081500 deployed.
- [worth-engine] Suburban anomaly diagnosed (task #12): 28,184 atoms ingested 2026-03-19 with bogus observed_at=ingested_at (no EXIF read). 28,181 distinct URLs at real storage paths. 131 vehicle_images rows. Atoms are testimony — resolution via EXIF read + supersession, not delete. ISSUES.md updated.
- [worth-engine-ui] Worth Engine MVP shipped: useWorthEngine hook + WorthEngineCard.tsx + slot in WorkspaceContent.tsx. Live on Mustang profile rendering BRACKETED ,768-,638 with 3-method triangulation, v2/v3 ratio, substrate strip, existence×magnitude confidence, gaps, warnings. Surgical edit per design rules — no design system violations, CollapsibleWidget variant=profile, Arial+Courier, 2px borders, zero shadows.
- [byok-pipeline] Bbox-bearing K5 verdicts via teacher-student self-critique loop: 6f01722c (brake new), 0bc191ec (hub old+caliper), 0e41e25e (measurement closeup). TWVP 0-999 normalized coords. ObservationPage renders SVG overlay. Analysis-stream realtime subscription confirmed working. Harness at /tmp/dia/k5/_loop.sh (resolve/dims/annotate/ingest). Doctrine appended to docs/library/technical/engineering-manual/18-deep-image-analysis.md with OPD analogy.
- [entity-resolution] Found Mustang split across two records: 83f6f033 (Red, dropbox_bulk, 882 imgs ending Nov 2025) vs eeb9fa61 (Black/iphoto album, 341 imgs thru Apr 22 2026 with engine refresh). Identical first_photo 2022-05-17 = same physical car. Filed merge_proposal 00507b81 status=pending, ai_decision=MERGE, preferred_primary=A (has VIN). Skylar's April-May Mustang work is on B until proposal executes.
- [anchor-system] Origination Anchor System deployed (5 SQL functions). Auto-classifies vehicles into vin/owner/listing/unknown tiers based on photo origin breakdown. Detects 5 violation types incl. critical owner-photos-on-listing case. Auto-files merge proposals: 139 VIN-dupes across DB, 4 same-owner-same-first-photo in Skylar's portfolio (Mustang, Bronco, K5, Suburban). public-readiness gate computes blockers + user-nudges for vehicles below threshold. Migrations 20260523090000 + 090100.
- [substrate] shape migrations: VIN uniqueness trigger on vehicles, image-origin consistency trigger on vehicle_images, v_vehicle_anchor view (pushdown-friendly). Three bug classes now structurally unrepresentable: dupe VINs (new), owner-source photo on listing-anchored vehicle (new), stale anchor in a column (no column exists). quad's GENERATED column plan replaced — Postgres can't aggregate sibling tables in generated cols. Detective functions (compute_origination_anchor/detect_anchor_violations/propose_anchor_merges) remain as one-shot cleanup tools for the 143 pending merge_proposals + 139 VIN dupes; promotable to UNIQUE INDEX after drain.
- [user-profile-mining] Mined 10yr buying substrate (QB 2950 rows + Apple Mail 129K msgs + Apple Cash 78 payments + 318 OCR receipts + 3101 photos). Persisted 17 JSON files under output/user_profile_mining/. Delivered AC_RELEVANT_FINDINGS.md (load-bearing): no Vintage Air/Old Air/SPAL/Sanden history; Mastercool flaring tools owned; eBay shows active squarebody AC trim work. Filed schema_proposal_user_observations.json for DB persistence (~830 obs ready). 4 QA issues logged to ISSUES_to_file.md (Edit blocked on .claude/ISSUES.md).
- [substrate] Mustang merge executed (proposal 00507b81): A (83f6f033, VIN 6F07C219593) kept primary, B (eeb9fa61) status=merged. 341 images + 84 observations moved. Mustang A now 1,223 photos. Required SET statement_timeout='120s' — the postgres role default is actually 10s, not the 120s CLAUDE.md hard-rule #12 claims. Every prior merge_into_primary attempt has been hitting the 10s cap and rolling back silently. The other 142 pending proposals can drain by raising session timeout.
- [K5 substrate] Landed user_observations (811 rows) + session_topic (460 rows) + parts_catalog (11 AC parts) + parts_fitment (5 K5 fitment) + 1 K5 AC build vehicle_observation via ingest-observation. Superseded 4 bad vendor totals (Eckler/DP/Summit/CJ) with skylar_verbal_corrections. Killed 4 redundant MD/JSON staging files. AC parts list now queryable from K5 profile.
## 2026-05-22
- [k5-routing-and-labels] Two more substrate fills: (1) wire_landmark_traversal property filed + approved + 291 traversals ingested from K5_wire_paths.yaml across 100 wires (atomic-cell refile of withdrawn jsonb-array proposal — discriminator_key='traversal_key' = wire_id_sequence). (2) 224 wire labels ingested from K5_wire_labels.md across 112 wires × 2 ends (114 rows parsed; 2 already-cited from closure receipts). Build coverage: 32.2% → 44.9% (1,303 → 1,818 cells of 4,050 target). Per-property: 12/19 wiring properties have substantial fill; biggest remaining gap is wire_terminal_pn_a (device-end pigtail PN, 0% — derivable from device family in labels: IGN→WPCOL40, INJ→WPINJ40, CLT→WPCTS30, etc.) and the 4 wire_crimp_* QA properties (slots only, no values until Skylar crimps and documents).

- [verification-scope + landmark-estimates] DB went down again mid-turn (same outage as 5/17 + earlier today). Two migrations written to disk ready to apply when connection returns: (1) 20260522_add_verification_scope.sql — adds verification_scope column to observation_properties (class | instance | both) per Skylar's class-vs-instance philosophy distinction; backfills 18 wiring props='instance', 5 engine specs='class', 5 identity props='both', 8 condition/state='instance'; updates fn_schema_proposal_apply to honor scope from payload; files informational modify_property proposal recording the architectural distinction. (2) 20260522_k5_landmark_estimates.sql — files landmark_distance_in property (multi-cardinality, discriminator_key='landmark_id', scope='instance') + auto-approves + ingests 30 landmark estimates (L01-L30) with per-landmark cited reasoning from K5_dimensions_atoms.yaml + LS3 published specs (bore center 4.4in = L04+L06) + geometric inference from K5_MASTER_MEASUREMENT_PLAN.md descriptions. Confidence 0.40-0.85 per landmark by anchor data quality; source=k5_landmark_estimate_2026_05_22 (internal tier 3, base_trust 0.45). Designed to be superseded by owner-tape-measurement observations (source=owner, conf=high) when Skylar walks the truck per measurement plan §5 Session 2.
- [librarian + substrate-image-witness] Built the librarian + image-witness layer per 2026-05-22 conversation. (1) scripts/library_index.py extracted 92 PDFs from reference_documents/ to per-page text files in docs/library/_extracted/ (75MB total: 4 GM service manuals 1973-87, 5 supplements 1974-80, 3 wiring booklets, frame dimensions, RPO codes, 60+ component datasheets). (2) scripts/library_search.py provides cited keyword search returning page numbers + line-level excerpts. Demonstrated: 'grommet' finds Aeroflow MAP firewall hole specs + LS3 EROD harness routing protocols; 'harness clip' finds 1987 service manual pages 589-590. (3) database/migrations/20260522_reference_documents_substrate.sql staged — reference_documents + document_pages tables + tsvector full-text search + v_library_search view + adds citation_document_id/page_number/excerpt columns to vehicle_observations (dual-mode). (4) database/migrations/20260522_observation_witnesses.sql staged — image-as-witness layer with witness_role (primary/context/supersession/derived) + capture_method (live_streaming/photo_with_exif/photo_no_exif/composite) + v_observation_attestation tier view. (5) scripts/library_ingest_to_db.py ingests manifest+pages into DB when applied. (6) package.json wired: library:index, library:search, library:ingest-to-db. (7) K5_WIRING_STATE.md §7 updated with entries 24-28.
- [library-substrate-mostly-applied] Applied 2 of 4 staged migrations + library ingest (78+ docs of 92, 3,546+ pages and climbing): (1) 20260522_observation_witnesses.sql APPLIED — observation_witnesses table + v_observation_attestation view live. (2) 20260522_reference_documents_substrate.sql split into 3 atomic chunks and APPLIED — reference_documents + document_pages + tsvector trigger + v_library_search view live; the ALTER TABLE adding citation_document_id/page_number/excerpt to vehicle_observations DEFERRED due to lock contention (PID 2628001 holds AccessShareLock on vehicle_observations via active COPY). (3) library_ingest_to_db.py running — already loaded 78+ documents and 3,546+ pages, climbing. (4) Verified DB-side library search works: SELECT ... FROM document_pages WHERE text_search @@ plainto_tsquery('firewall grommet') returns ts_headline excerpts with bolded matches. (5) 20260522_add_verification_scope.sql and 20260522_k5_landmark_estimates.sql still BLOCKED — PID 2627959 (Supabase dashboard idle-in-tx running pg_get_viewdef) holds AccessShareLock on observation_properties; will apply when the dashboard query releases. All migrations atomic + idempotent on disk for re-attempt.
- [blocked-migrations-applied] All 3 previously-blocked migrations applied after DNS+Supabase reachability restored: verification_scope column on observation_properties (27 instance/5 class/5 both backfilled); 30 K5 landmark estimates ingested (L01-L30 with cited reasoning, build coverage now 45.6%); citation_document_id/page_number/excerpt columns added to vehicle_observations enabling structured citations to specific library pages.
- [system-works-demo] Built scripts/nuke_wire.py — first end-to-end output the substrate produces. (1) 'nuke_wire.py 110' renders single-wire dossier with all 18 properties + confidence stars + cited sources + routing landmarks with estimates + closure ratio. (2) 'nuke_wire.py all [LOOM]' renders the build sheet — all wires for K5 with id/label/ECU pin/gauge/color/length/terminal in a printable table. Dumped full K5 build sheet to docs/wiring/output/K5_BUILD_SHEET.txt. Wired k5:wire and k5:buildsheet npm targets. This is the first substrate-to-bench artifact — no YAML regeneration, just SQL rendering as text.
- [daily-receipts] Built end-to-end daily-receipt loop: scripts/daily-receipt/process-photo.mjs + build-day.mjs + library doc 17-daily-receipt-processor.md. Mustang Apr 14-21 backfilled, 7 days substrate-loaded, 6 active work_sessions: Apr14=$317.33(pickup+teardown), Apr15=$408.00(engine+underbody), Apr16=neg-space, Apr17=$87.83(iBooster+wiring), Apr18=neg-space, Apr19=$90.67(floor strip+Dynamat), Apr20=$42.50(paint prep), Apr21=$42.50(steering column). Total $988.83 / 17 photo atoms / 4 negative-space markers. Pattern proven: get_daily_work_receipt RPC now returns real data for these days.
## 2026-05-21
- [institution-substrate-resume] applied view-fix migration (discriminator-aware vehicle_canonical: K5 went from 5 canonical rows to 747). Applied closure-receipts ingester (+2 cited terminal_pn_b cells for wires #110 CLT and #13 Injector 1; from_terminal_pn fields all still unknown pending MoTeC datasheet read). Wrote scripts/k5_substrate_to_wireviz.py — substrate-driven WireViz YAML generator that projects 162 wires across 17 looms from vehicle_canonical, no reads of K5_cut_list_v2.txt. Wire #110's emitted YAML carries BOTH cut-list cells and receipt terminal_pn_b joined by wire_id.

- [milspec-research] wrote docs/wiring/research/2026-05-21_milspec_heatshrink_protocols.md (workshop-grade reference covering M23053 family, K5 termination stack inside-out, SCL/DR-25 spread math, S03 solder sleeves, AS85049 backshells, AS81765 transition boots, MoTeC Superseal built-up boot practice, 1977 GM factory protocol for body side, substrate gaps). Filed 5 schema_proposals (status=open): wire_termination_a_protocol, wire_termination_b_protocol, wire_label_text, wire_routing_landmarks, loom_outer_jacket_pn — each cites the research doc and external Raychem/TE/SAE sources. Updated K5_WIRING_STATE.md §7 with pointers to the new research + connector report + MoTeC validation report + the open proposals query.
- [proposal-remediation] Self-audit + remediation of 5 wire-substrate proposals. Withdrew 4 (jsonb-stack false-merge violations: wire_termination_a/b_protocol collapsed N layer-cells into 1 jsonb; wire_label_text hid per-end multi-cardinality; wire_routing_landmarks hid N landmark-traversal claims). Refiled 6 atomic per-end properties (wire_termination_{a,b}_inner_seal_pn + _outer_cover_pn + wire_label_{a,b}_text) + 1 add_observation_kind for 'splice' entity (research §5 said splices need their own treatment). Wrote + applied database/migrations/20260522_schema_proposal_approval_trigger.sql implementing fn_schema_proposal_review_handler() — AFTER INSERT trigger on schema_proposal_reviews that quorum-checks + auto-applies (INSERT observation_properties for add_property, ALTER TYPE ADD VALUE for add_observation_kind). All 8 open proposals approved via owner self-approval (handbook §4 permits owner+reversible). End state: observation_properties wiring category went from 7 to 14 atoms (incl. loom_outer_jacket_pn); observation_kind enum now includes 'splice'. Updated docs/library/reference/encyclopedia/08-schema-proposal-workflow.md §2 with full add_property payload shape (cardinality, discriminator_key, applies_to_kinds, expected_source_categories, description) + pointer to the trigger migration. K5_WIRING_STATE.md §7 updated with new proposal summary.
- [substrate-deepen] Schema + receipt + SQL migration prep, partially DB-applied before outage. (1) wire-closure.schema.json — replaced flat heat_shrink (3 fields) with 7-field per-end split (label_end_a/b + termination_end_a/b_inner_seal_pn + outer_cover_pn + boot_at_terminals). Dropped const:14 on total_fields → dynamic. Pattern on ratio bumped to ^[0-9]+/[0-9]+$. Added $schema to allowed properties. (2) Migrated 3 closure receipts via /tmp/migrate_closure_receipts_2026-05-22.py — split '#110 sensor end | ECU end' merged labels into per-end values (COOLANT TEMP CLT 22 + M130:B04 CLT 22; INTAKE TEMP IAT 22 + M130:B03 IAT 22). All 3 receipts now validate. Closure ratios: #110 15/25, #109 11/25, #13 14/25. (3) database/migrations/20260522_k5_substrate_deepen_labels_crimp_views.sql written — ingests 4 split labels into vehicle_observations, files 4 crimp QA proposals (wire_crimp_a/b_die_pn + _pull_test_lbf per IPC/WHMA-A-620 §19.1) with auto-approve, creates v_open_proposals + v_proposals_resolved views. NOT YET APPLIED — Supabase connection timed out mid-apply (same outage pattern as 5/17). Re-apply when DB reachable.
- [k5-substrate-expert-pass] Massive substrate fill: (1) 80 MoTeC Superseal terminal PNs derived from gauge (3-1447221-3 for 16-18AWG, -4 for 20AWG, -5 for 22AWG). (2) 4 K5 Y-splice observations seeded under new kind='splice' (coil dist, injector dist, BAT_NEG bus, ETB branch) — PNs still pending build decisions. (3) Source trust calibrated: closure receipts 0.95→0.97 (triangulation premium), cut list 0.95→0.90 (amendment history), splice design 0.60. (4) 472 termination heat-shrink cells filled across 118 wires × 4 properties: SCL inner seal + DR-25 outer cover per gauge per Raychem catalog. 22→SCL-1/8+DR-25-3/8, 20→SCL-3/16+DR-25-3/8, 18→SCL-1/4+DR-25-1/2, 16→SCL-3/8+DR-25-5/8. (5) Three institutional views: v_k5_wire_dossier (one row per wire, 18 property columns), v_k5_substrate_inconsistencies (14 PDM30-pin gauge mismatches surfaced — paralleled-pin power feeds), v_k5_build_coverage (per-property %). K5 build coverage 20.5% → 32.2% of the 4,050-cell target.
## 2026-05-17
- [institution-substrate] rewrite PROPOSED_institution_minimum_substrate.sql against verified DB state (existing properties=real-estate collision fixed → observation_properties; 5 new tables justified; citation trigger; vehicle_canonical view); update handbook 08 with grounding section + real table names

- [photo-ingestion] Diagnosed 14-day photo ingestion outage. com.nuke.photo-ingest-watch.plist references deleted scripts/db_functions.py; system pivoted to iphoto-intake.mjs --all (per DONE.md) but plist never updated. Filed CRITICAL in ISSUES.md. Fix gated on Skylar approval to touch launchd.
- [institution-substrate] applied PROPOSED migration (5 new tables: observation_properties, schema_proposals, schema_proposal_reviews, pending_claims, + vehicle_observations.property_id col + citation trigger + vehicle_canonical view). Citation trigger smoke-tested: rejects uncited writes. Ingested K5 cut list v2 → 747 cited cell-level observations across 162 wires on K5 e08bf694 (VIN CKR187F127263) under source k5_cut_list_v2. Wire #4a now reads as 5 cells (id/color/gauge/length/spec) instead of as a YAML row.
## 2026-05-10
- [wiring/formboard] Diagnosed empty FORMBOARD render on /vehicle/:id/wiring. Five fixes shipped to main: useOverlayCompute hydration (b61135618), CSP worker-src+wasm-unsafe-eval (1b1149345), swap FORMBOARD tab to the orphaned 2552-line FormboardCanvas instead of 751-line FormboardView prototype (4f022a045), lazy-load + only-mount-active tabs (434ea7e89), and missing ZONE_COLORS/ZONE_LABELS/segmentKey exports from harnessConstants that the lazy split exposed (e6289e69e). All five live on prod nuke.ag.

## 2026-05-07
- [substrate] payment_events table + supersession + v_user_daily_activity payment leg + v_user_monthly_summary payments_in/out/net_cash_usd; backfilled 13 active money-in events (,542) for Skylar from observations + work_order_payments + vehicles

## 2026-05-06
- [vision-gate] Session 2026-05-05: 14 waves L4 BYOK drain + 2 cron triggers (trig_016n4GhSBPsrUJdcMCRsxPCG :00 + trig_01BtcQwqRpXViqAQmKbkR6R2 :30). Delta: approved 5869->10667, review_needed 13127->8286, rejected_misattributed 48->1284, rejected_personal 84->428. Suburban-73085 FULLY DRAINED. 30+ observations: VIN confirmations (Mustang 6F07C219593, LX450 JT6HJ88J4V0178097, Suburban-73085 1GKEV16K7HF509103, K10 SWB CKL147Z186644 SPID), reattribution candidates (Jimmy 35 to K5 5b4e6bcd, Suburban-b1fd 36 to 73085dcf, Bronco-eea 9 from c6189023), duplicate flag 95fbb5c3+c6189023=same Velocity #383 FAM X48, Jimmy/K5 VIN mismatch CRITICAL.

- [garage] Verification deep audit + Mustang title URL fix. Re-signed 1966 Ford Mustang (83f6f033) title_document_url with 10-year expiry — original signed URL had 1-hour TTL and expired upload-day (storage upload-flow bug). Filed 4 HIGH issues: (1) ownership_verifications is theatrical (status='approved' carries no measurement — name_match/vin_match/doc_authenticity all NULL); (2) title scans stored in PUBLIC bucket for K5+Blazer (PII exposure); (3) receipt→vehicle attribution mostly <0.3 confidence; (4) photo density not used as ownership-evidence path despite being stronger than unverified title scans. No further auth-adjacent changes shipped.
- [garage] Verification deep audit (4 verified ownerships are theatrical — no human reviewer, no AI extraction, no name/VIN match scoring; K10's title URL is a BaT listing; Mustang's signed URL was already expired). Lifetime ownership audit: 4 verified by title + 3 BaT seller events (skylarwilliams listed K2500 + Suburban + Hot Rod, last is a Viva conflation) + 51 vehicles photographed substantively in 8-year window. K5 Blazer test render in St. Barts (procedural sky + sand + ocean + palms in EEVEE; no Polyhaven assets — disabled). Skipped: bucket migration for K5/Blazer titles (auth-adjacent). Filed 4 HIGH issues in .claude/ISSUES.md.
## 2026-05-05
- [vision-gate] Stand up cheap-first cascade + reattribute pipeline + L0.5 affinity. Migration 20260505100000 deployed reattribute_observation() + reattribution_audit + supersession columns on vehicle_images. Migration 20260505110000 deployed sender_contact_affinity table + recompute_sender_contact_affinity() + L0_5_sender_affinity() in classify.mjs. Drained ~5000 NULL via cheap layers across 236 vehicles (K10 alone: 1536 of 1666 → auction-pipeline approved). L4 vision drained: Hot Rod 22/22 clean, LX450 19/19 clean, K2500 batch 1 8a/9p/4m, Bronco 24a/1m, Mustang 24a/1m. iPhone 15 Pro affinity learned: 258v/4p (0.98). Filed HIGH issue: photo-pipeline-orchestrator broken — yono-classify never fires after yono-analyze, accumulating NULL gate_status.

- [garage] Cleanup audit + Week 1 ship: revoked 31 contaminated perms (19 from 2026-03-09 batch, 10 dangling, 2 merged) + 1 dangling verification via supersession; patched enforce_user_vehicle_evidence trigger to allow revocations; deployed revoke_garage_membership RPC; switched grid objectFit cover→contain (no more cropped photos); added per-card × NOT MINE eject button; PINNED BUILD strip in GarageTab; totalEstimatedValue now OWNED-class only; chk_granted_by_not_null NOT VALID constraint on vehicle_user_permissions. Post-state: 12 owned-class vehicles, $603K (was 27/$2.26M).
## 2026-05-03
- [external-agent-write-api/receipt-cleanup] WS-8: 461 unattributed receipts heuristically matched (date proximity ±3d singleton dominant) and submitted via ingest-observation. 1,647 left for human review (mostly household). PR #253 merged.

- [external-agent-write-api/overnight-multi-team] 10 parallel workstreams completed overnight, 11 PRs merged: WS-1 photo export running but pipeline-to-DB bridge missing; WS-2 OAuth hardened, end-to-end test green; WS-3 312 receipts→observations (255 on ghost K5); WS-4 form-shapes tightened + get_event_checklist (49 tools live); WS-5 timeline render with owner-trust pinning; WS-6 219 Numbers observations (Charger +107, Suburban +111, K5 still 0); WS-7 iMessage config built, backfill running in background, uses parallel write path; WS-8 +461 receipts (263 on ghost K5); WS-9 substrate proven, vision LLM blocked by Anthropic billing; WS-10 library updated with form-shape framing. Plus PR #250 cockpit recovery. Critical findings: (1) K5 receipt ghost issue — 518+ receipts on wrong vehicle entity from 2026-02-02 GAA-43671 incident; canonical K5 still shows ~25 obs; needs unmerge_vehicle/reattribute_observation; (2) photos→observations bridge missing — analyze-photos-deep.py writes to local SQLite, not vehicle_observations; (3) iMessage uses parallel write path (work_sessions) not vehicle_observations; (4) Anthropic API has /bin/zsh credit, top up before re-running vision-fill smoke test; (5) ~30 Numbers files unattributed need curated FILENAME_VEHICLE_MAP from Skylar; (6) Joey's iMessage phone number outdated; (7) concurrent agents in shared worktree caused multiple branch collisions, agents had to use isolated /tmp worktrees. Background processes still running at session end: osxphotos export (~30 min ETA), iMessage backfill (hours).
- [last-60-days sprint] Substrate hydration: 4,640 day-rows across 12 types in user_contributions (photo/email/imessage/qb/timeline/gps/file_modification/bank/phone/browser/receipt/claude_session). Receipts 100% scope-resolved (2,420/0 NULL). Multi-signal cockpit attribution architecture validated: 675 atoms persisted, 4 of 5 clusters owner-confirmed (R3500/1971 C10/1983 K2500 BLUE/1984 K10 SWB) crossed 0.85 synthesis threshold. Caught + corrected: SSE-header bug lost 109 atoms, signal-padding anti-pattern, single-signal direct UPDATEs. /me/money + /journal pages shipped to fix branch but API endpoints 404 on nuke.ag (vercel.json catch-all reverted). Day-page UI: code complete, deploy gated on vercel.json decision. Ghost cluster surface: 1985 'Suburban' walker dispute resolved as misattribution (likely 1985 K10/K20 — awaiting Skylar pick). Phantom-AR foundation laid (Viva vehicles enumerated, photo-coord pivoted to iphoto-intake --all, still running).
- [external-agent-write-api/vision-gate] Vision gate shipped (PR #254). Schema: vehicle_images.vision_gate_status ENUM (pending|approved|rejected_personal|rejected_misattributed|review_needed) + DEFAULT 'pending' + reasoning/confidence/processed_at columns + partial index. Frontend: all 3 vehicle_image read paths (fetchVehicleImages.ts, loadVehicleImages.ts, ImageGallery.tsx) filter (NULL OR approved). Demonstrated agent-as-filter: classified 5 K2500 imessage attachments inline (2 family pool photos rejected_personal, 1 Chevy 350 valley shot approved, 1 Ford valve cover misattributed to Mustang, 1 Mustang on 2-post lift misattributed). All 5 have agent reasoning written to DB. Pattern proven: agent reads pixels through library lens, no Anthropic API spend (uses Claude Code Agent context). Skylar's correction absorbed: agent IS the first line of defense at intake.
- [vision-gate] Multi-signal vision-gate classifier shipped: scripts/vision-gate-classify.mjs (L0-L4 cheap layers + L4 router), scripts/vision-gate-l4.mjs (prepare/ingest worklist+sink for free Agent vision), scripts/vision-gate-drain*.sh (parallel chunk drainers), .claude/commands/vision-gate-l4.md (slash command). All in package.json (Hard Rule #9). Cheap-layer drain 1st pass: 7,299 approved across 92 Skylar vehicles. L4 free-vision validated end-to-end on K2500: 25 rows → 10 approved / 14 rejected_personal (TikTok screenshots, emoji stickers, woman selfie, baby in stroller, Domino's storefront — exactly the contamination Skylar called out) / 1 review_needed. K5 ghost (e08bf694) and K2500 NULL-VIN ghosts excluded — separate cleanup via unmerge_vehicle/reattribute_observation per testimony rules. Reasoning text is queryable diagnostic ('L4-agent: TikTok screenshot of speaker, not a vehicle'), preserves negative-signal payload per migration intent.
- [vision-gate] L4 free-vision drain on K2500 (3 rounds × 25 = 75 rows): 14 approved, 48 rejected_personal, 11 rejected_misattributed, 2 review_needed. Misattribution mostly Jenny Mannerheim iMessage-thread mass-attribution: yacht expertise reports, marine engines, sailboats, banking app, fashion shoots, kids photos — none belonged on K2500. Discovered structural bug: iMessage-thread → vehicle_id mapping floods one vehicle with all thread attachments. Reattribution to correct vehicles is a separate flow (per testimony rules). Image transform fix: route image_url through Supabase render API with width=1024 to avoid agent context dimension limits (>2000px). K2500 final: 1908 approved / 1131 review_needed / 50 rejected_personal / 13 rejected_misattributed of 3102 total. Cheap-layer drain across 92 vehicles: 8,532 approved, 11,596 review_needed, 2671 NULL pending (auction-source vehicles with >2K images, draining). Slash command /vision-gate-l4 written for future-session continuity.
- [vision-gate] Form-fill propagation pipeline operational end-to-end. K5 profile (was 100% NULL) → trim Cheyenne Chalet, condition_rating 6, known_flaws (Undercarriage rust+missing_panel, Detail Badge paint_chip+rust+scratch, Passenger Side paint_chip+scratch), highlights (12 photos / 5 zones), color=red, color_primary=red, interior_color=black. Across 5 vehicles synthesized: K20 detected yellow/4 mods, Mustang66a teal/known_flaws 5 zones, Mustang66b black/black/2 mods/known_flaws (HIDDEN finding: chassis painted+assembled OVER torn rear frame rail + floor pan rust — cosmetic restoration outpaced structural). Pipeline: vision-gate L4 → form-fill agents (general-purpose subagent reads image_url via Read tool, fills 14-field form with vehicle_zone/fabrication_stage/mods/damage/condition/exterior+interior color/visible_parts/text/location/people/raw_observation/confidence) → vision-form-fill-ingest.mjs writes per-image columns + ai_extractions.form_fill_v1 → synthesize_profile_from_vision (existing) writes known_flaws/highlights/condition_rating + new synthesize_color_from_form_fill writes color/color_primary/interior_color. NEW migration: synthesize_color_from_form_fill(uuid). Pipeline cost: zero Anthropic API spend (Claude Code Agent Read tool free at margin per Skylar's BYOK directive).
- [vision-gate] E2E test passes 10/10. Pipeline operational: gate→form-fill→synthesize→profile populated→merge integrity. Mustang dedup executed: 8bde1dda merged into 83f6f033 (canonical VIN 6F07C219593) via approved merge_proposal e980375d, 122 images + 2 obs relinked with merged_from_vehicle_id lineage per testimony rules. Skylar profile hydrated: 707 Yucca St / Boulder City / NV / 89005 / NUKE LTD / mechanic / 4 expertise areas (was all NULL). New scripts: vision-gate-e2e-test.sh (10-assertion smoke check), vision-form-fill-ingest.mjs (writes per-image columns + ai_extractions.form_fill_v1). New SQL: synthesize_color_from_form_fill(uuid). 82 images form-filled across 5 personal vehicles this session.
## 2026-04-06
...
- [wiring] Populated wire_catalog: 207 products (97 M22759/32, 4 M22759/16, 6 Marine Battery Cable, 100 TXL). 64 unique colors across 8 gauges (4-22 AWG). Every wire from K5 cut list v2 (123 wires) has professional + enthusiast tier equivalent. Tier-swap pricing: M22759/32 avg .36/ft vs TXL avg /bin/zsh.20/ft (6.8x). Cleaned old-format full-name color rows.
- [wiring] ProWire USA M22759/32 Tefzel wire catalog indexed: 89 solid-color products (28-12 AWG, 10 colors each), 8 TXL reference prices, plus 4 M22759/16 and 6 marine battery cable entries (107 total). K5 cost analysis: Tefzel  vs TXL  (4.6x premium). Cost analysis saved to docs/wiring/output/K5_wire_cost_analysis.md
- [work-order-statement] Invoice page deployed: classic repair order form, 3 themes, inline CRUD, provenance tags, audit log (work_order_edit_log), tax breakout, QR code. Granholm K2500 corrections: brake prices, flat rates, comps, tax applied, part names cleaned, Davis  flat, hood insulation bundled, carb rebuild +parts comped, tailshaft seal comped, Goodmark air dam removed.
- [frontend] BarcodeTimeline 3 fixes: (1) month/year label alignment accounts for year separator width, (2) work session clicks open DayCard popup directly instead of inline receipt, (3) area extraction from work_description enables multi-day project highlight (exhaust/brakes/suspension/etc)
- [deck-system] Chapter 14: Deck System Architecture (782 lines) — 'the database IS the pitch.' Deck as a view into accurate data, not a creative artifact. 5-stage pipeline (query→validate→compose→render→review), evidence hierarchy, logo display rules as code, 7 rules, 4 anti-patterns, build report schema. Also: bulk GPS enrichment for 27 SBH orgs, org creation (alice+olivia, TopLoc, GTA, Lou Lou, Hotels Barrière, Oetker Collection), background logo scraper launched for 650 SBH orgs with websites.
- [brand-enrichment] Bulk brand scraper completed: 571 of 710 SBH orgs with websites now have brand_design_language (80%). 217 have logos, 510 have primary colors. Key wins: Eden Rock SVG (Oetker), Le Toiny SVG (blue), Cool Rental SVG, Nikki Beach SVG, Rosewood SVG. 4 still missing logos (Cheval Blanc, alice+olivia, CFB, Auto Deal — JS-rendered sites). Heritage paradox thesis added to commercial structure doc.
- [research] Rental-to-purchase research compiled: GT350-H Rent-a-Racer (1966, 4x production increase), Porsche Experience Centers (80% non-owners likely to buy), Enterprise (49% rewrite wish lists), Hertz-Tesla cautionary tale (M loss), LexisNexis (57% open to switching), Cox Automotive (2/3 say experience > price). Full doc at docs/decks/assets/research-rental-to-purchase.md. Also: logo data quality fixes (Le Barthélemy, Le Toiny, Nikki Beach, FBM variants), photo scraper agent running for map popups.
- [brand-scraping] SBH brand asset scraping: 81 orgs enriched (56 with logos, 25 colors-only). Coverage: 355/390 SBH orgs with websites now have brand data (91%). 35 failed (timeouts, 403s, 404s, dead sites). Notable: Bulgari, Cartier, Richard Mille, Valentino logos captured. Restaurants with logos: Bagatelle, Bonito, Shellona, La Guerite, Le Piment, Lepicerie du Gout. 17 car rental companies with logos.
- [fb-pipeline] Audit fixes applied: enricher v3 (3-7s jitter, circuit breaker, retry, session check, price sanity, fetch timeout, Chrome 135 UA), scraper fixes (emoji strip, comma split, coupé regex), DB cleanup (garbage sale_prices nulled, 165 non-canonical body_styles normalized to 0), cron updated to v3 with limit 50.
- [frontend] Fixed month label drift (24px over 2000+ weeks) — replaced absolute-positioned labels with inline spans inside flex columns for zero-drift alignment. Fixed work_sessions not loading (RLS blocked anon reads) — added work_sessions to get_vehicle_profile_data RPC via SECURITY DEFINER. Merged in loadVehicleData.ts client-side.
- [session-close] Luxe Fleet mega-session complete. Wrote NEXT_SESSION.md — full instruction manual for continuation. Key lesson: use the existing NukeMap infrastructure (fix the fetch error) instead of rebuilding from scratch. The deck content/strategy is solid, visual execution needs a systematic approach (render from DB, not hand-edit HTML). 91% SBH org enrichment, research compiled, Chapter 14 written, commercial structure complete.
- [fb-pipeline] Created 197 vehicle records from orphaned marketplace_listings, downloaded 1622 images to Supabase storage for 164 vehicles, cleaned unicode dots from models, fixed enricher filter (scraped_at vs seller_name), moved pycookiecheat to ~/.local/venvs/fbcookies, deleted dead scripts, added npm scripts for fb:enrich-cookies, fb:create-vehicles, fb:download-images
- [fb-scrape] Scraped FB saved items via Chrome MCP: 9 visible, 4 new (Hummer H2 x2, GMC Sonoma, Lincoln Continental). Enriched 102 card-only listings — found 75 Blazer duplicates (FB redirect pattern), marked as duplicate. Pipeline status: 353/376 fully enriched (94%)
- [wiring] Phase 2: Multi-layer reference images (11 toggleable layers in LAYERS popover), engine bay sub-zone density fix (zone-aware collision distances), harness trunk routing (K5 graph with 30 nodes, Dijkstra routing, HARNESS button), DB positions for all 118 devices, vision extraction script (ready for GPT-4o when key has quota)
- [deck] Data-driven deck system: deck_slides + deck_slide_history tables with RLS and audit trigger, 15 slides seeded from v2 HTML into luxe-fleet-ford, DeckPage.tsx (standalone at /deck/:deckId), DeckSlide.tsx (per-type renderer), DeckEditor.tsx (contentEditable with auto-save), useDeckData.ts (hook), deckUtils.ts (getLogoForBg, colors). Route wired in App.tsx. Logos pull from brand_design_language automatically.
- [wiring] Fixed wiring workspace visual rendering: SYMBOL_SIZE 24→10, image layers preserveAspectRatio none→xMidYMid meet with corrected portrait canvasRect heights, removed wire jitter, harness routing default in vehicle mode, auto-fit zoom cap 0.8→2.0 with reset on view/layout change, removed SPREAD/HARNESS buttons, reduced unselected wire opacity 0.3→0.12
- [deck] Rewrote deck renderer to use raw v2 HTML stored in content.html — exact visual parity with luxe-fleet-ford-v2.html. CSS scoped under .deck-root to isolate from Nuke design system. All 15 slides render with correct backgrounds, typography, images, logos, layouts. DeckSlide now uses dangerouslySetInnerHTML. Previous JSONB-field-based renderer replaced.
- [wiring] Deleted 15 SVG canvas rendering files, rewrote WiringWorkspace as two-panel data view (image viewer + device table), updated WiringPlan to remove sandbox mode, fixed VehicleExpertChat and WorkspaceContent refs. TypeScript compiles clean.
- [deck] 24 fixes applied to deck_slides content across 11 slides: (1) FBM red JPG logos fixed with mix-blend-mode:screen on cover/close and multiply on partners, (2) Hotel logo brightness(2) hacks replaced — Le Toiny switched to blanc.svg, others reduced to 1.3-1.5, (3) Porsche PEC 80% misrepresentation fixed, (4) Enterprise 49% softened, (5) $1K service calls removed from slides 2+7, (6) 65% American removed, (7) Hermès claim hedged, (8) 14 weeks ago → early 2026, (9) 168h→30+ actual drive hours, (10) 400K-1.2M impressions replaced with qualitative, (11) HTTP→HTTPS Cool Rental, (12) SiBarth onerror removed, (13) WCAG contrast fix on slide 11, (14) Image heights standardized to 200px, (15) Source lines added to data slides.
- [wiring] Deep research: read guides/tutorials for RapidHarness, EZ Wire, WireViz, Splice CAD, harness.design, Wirely, HPA Academy wiring fundamentals. Wrote comprehensive landscape doc at docs/library/technical/wiring-software-landscape.md and memory file. Key finding: Splice CAD has Python API + embed widget = best integration path.
- [wiring] Built WireViz YAML generator (generateWireVizYaml.ts) — converts vehicle_build_manifest devices + computed wires into WireViz YAML by zone. Integrated Kroki.io rendering into WiringWorkspace — generated diagrams appear alongside reference images in dropdown. Per-zone connection diagrams render as interactive SVGs. Tested Kroki API — works (200 OK, clean SVG).
- [deck-workspace] Built complete deck template system: 13 slide templates (CoverSlide through DividerSlide), deck-style-guide.css with design tokens, deckTypes.ts with all content schemas, DeckSlide router mapping slide_type→template, DeckPage updated with CSS import + data-brand attr, decks parent table migration, deck-agent-instructions.md. TypeScript compiles clean.
- [wiring] Complete rebuild of WiringWorkspace as professional schematic editor. Dark canvas, ECU/PDM source blocks with pin lists, device blocks grouped by zone columns, colored bezier wire paths, wire gauge badges, zone headers with counts. Three tabs: SCHEMATIC (node graph), TABLE (sortable data), REFERENCE (GM diagrams). Modeled after Splice CAD layout patterns.
- [wiring] Deleted SVG canvas from WiringWorkspace.tsx (609→237 lines). Replaced 3-tab layout (schematic/table/reference) with two-panel data view: left=zoomable reference images with dropdown selector, right=sortable/filterable device table with zone badges. Detail panel overlay preserved. 15 of 17 planned delete files already didn't exist. Kept WireMaterialPopup + ConnectorFaceView (used by kept components). TypeScript compiles clean.
- [wiring] Agentic Wiring Diagram Service: extracted shared compute engine (_shared/wiringCompute.ts ~300 lines), ported WireViz YAML generator (_shared/wirevizYaml.ts ~210 lines), created generate-wiring-diagram edge function (loads manifest → computes overlay → generates WireViz YAML → POSTs Kroki.io → stores SVG/PNG in Supabase storage → returns URL + summary). Refactored compute-wiring-overlay to import from shared module (~80 lines, down from ~560). Updated TOOLS.md with wiring system section. Both deployed and tested: 118 devices, 113 wires, system overview + zone diagrams render via Kroki.io.
- [frontend] Product audit + design review (3 rounds, 17 agents, 3 planning docs). Dead code deletion (19 files, -8K lines). Search z-index fix. Feed defaults (3-col, NEWEST, follow enabled, live/sold). Price history chart on profiles. AuctionTrendsDashboard unlocked. Button variants fixed. Landing page + product pages built with CSS tokens.
- [wiring] Vehicle-layout SVG diagram generator: (1) Manifest fixes — moved ECU to dash, added milspec bulkhead connector (61 pins), fixed Dakota Digital (can_display, 2A), fixed iBooster spec notes. (2) wiringCompute.ts — zone-aware route length estimation (estimateRouteLength), bulkhead passthrough detection (wireNeedsBulkhead), auto pin assignment, capacity warnings. (3) New _shared/vehicleLayout.ts (~450 lines) — ported vehicleSilhouettes, harnessRouting (Dijkstra), orthogonalRouter, labelPlacer from frontend. K5 plan-view body outline, trunk graph (31 nodes, 38 segments), collision-free labels. (4) New generate-vehicle-diagram edge function — plan view SVG with zones, trunk bundles, device boxes, wire branches, labels, legend, warnings. Supports show_bulkhead toggle (1088ft with, 908ft without). 52/61 bulkhead pins used. All 3 functions deployed.
- [frontend] Landing page simplified to Search-first hero (removed 6-product grid). Added persistent FEED/SEARCH/GARAGE/MARKET nav to AppHeader. Tailwind font sizes mapped to design system (text-xs→8px through text-5xl→12px). Fixed broken Profile.tsx imports (MyOrganizations, MyAuctions deleted in prior cleanup). Deployed to production, aliased nuke.ag.
## 2026-04-04
- [deck-infra] Deck infrastructure: brand_design_language JSONB on organizations, scrape-brand-assets.mjs script, /target-analysis slash command, design book ch12 (documents) + ch13 (multi-brand composition), 6 Fleet Luxe entities onboarded (Ford Bronco, Fouquet's, L'Officiel, L'Agence, FBM deduped 3→1, Nuke deduped 2��1), fleet-luxe-v9 rebuilt from source of truth

- [wiring/component-library] Component engineering library: 5 tables created (component_library, component_documents, component_drawings, component_connectors, build_manifest_components). 19 components seeded. 8 connectors with 76 pins mapped (M130 34-pin, D510C 4-pin, ETB 6-pin, CKP/CMP/KS/MAP/OPS 3/2-pin each). 57 PDFs downloaded (75MB) across 12 manufacturers. 14 vehicle base layers extracted. GM diagram format reverse-engineered (appendix-e). Schema documented (appendix-f). ACDelco pigtail catalog + 7 Delphi connector catalogs acquired. 8 Haltech LS3 sensor wiring guides downloaded.
- [component_library] Added 11 components: Truck-Lite 27270C headlight, United Pacific CTL7387LED tail light, RetroSound Hermosa head unit, Kicker 46CSC674 speaker, Kicker 46CXA400.1 amp, Nu-Relics NR17380201 power windows, Dorman 746-014 door lock, AMP Research 75146-01A PowerStep, Meziere WP319S water pump, SPAL 30102052 dual fans, Powermaster 47294 alternator. Library now at 59 components.
- [component-engineering] Added connector pin data for 6 connectors: M130 Connector B (26 pins — UDIG1/2 crank/cam, KNOCK1/2, AN_TEMP 1-4, CAN_HI/LO, SEN_0V, ETH, BAT_BAK, SEN_6V3), PDM30 Connector A (34 pins — OUT1-5 20A dual-pin, OUT9-20 8A, DIG1-10, VBATT_NEG, GND_0V), PDM30 Connector B (26 pins — OUT6-8 20A dual-pin, OUT21-30 8A, DIG11-16, CAN, VBATT_NEG_B, GND_0V_B), PDM30 Connector C (M6 VBATT_POS stud), GM Coolant Temp 19236568 (2-pin Metri-Pack, NTC signal+ground), GM Fuel Injector 12576341 (2-pin EV6/USCAR, peak-hold high+low). Total: 15 connectors, 151 pins in component_connectors table.
- [component-library] Registered 75 PDF documents in component_documents table, linked to 43 components across 21 manufacturers. Created 32 new component_library entries for previously unregistered parts (Haltech, Delphi, Sanden, Sonnax, Moveras, ATRA, ATSG, AMP Research, FuelTech, Swap Specialties, Kicker, Powermaster, SPAL, Bosch EV14, GM crate engines). 5 document types: installation_guide (29), cross_reference (18), datasheet (13), user_manual (10), wiring_diagram (5). ~113 MB total referenced docs.
- [component-library] Extracted 10 dimensional drawings from 8 PDFs at 250 DPI. Inserted 10 component_drawings rows + 2 new component_library entries (Holley 20-290 Mid-Mount, ACDelco PIGTAIL-CATALOG-2013). Files: motec_m130_dimensional, motec_pdm30_dimensional, bosch_ev14_dimensional, holley_midmount_dimensional, aem_ta2_ls3_harness_diagram, acdelco_pigtail_2pin/3pin/4pin, motec_c125_panel_cutout, estopp_esk001_wiring.
- [wiring] Created 17 SVG symbols for engine schematic (Phase 1 symbol library): P-01 battery, P-05 PDM channel, P-08 alternator, I-01 thermistor, I-02 pressure transducer, I-03 hall effect, I-04 knock sensor, I-05 potentiometric, O-04 fuel injector, O-05 ignition coil, O-10 throttle body, C-01 inline connector, C-02 connector face view, G-01 chassis ground, G-04 ECU sensor ground, R-04 shielded cable, R-08 twisted pair. Plus INDEX.md with usage rules and Phase 2 remaining list.
- [fb-pipeline] FB saved items enricher: pycookiecheat + Chrome cookies + plain HTTP. Enriching 445 listings in background. Body_style normalized (19K classified). 3 vehicles re-imaged. Scraper bugs fixed (fullTitle, split makes, expanded patterns).
- [wiring/harness-build-team] Wiring harness build team completed: 10 bench documents generated (cut list, connector schedule, BOM, measurement worksheet, shopping list ,231, harness build sheets 9 looms, wire labels 122 circuits, power distribution SVG, cross-reference check, connector shopping list). Data audit found+fixed 6 issues. Cross-reference QA found 4 critical blockers (PDM30 channel conflict, coil mapping, ETB needs 6 wires not 1, speaker pairs). 62 components in DB, 165 pins, 75 docs linked. Engine schematic SVG still rendering. Master plan at docs/wiring/WIRING_HARNESS_BUILD_PLAN.md.
- [wiring] Section 2 Engine Schematic SVG complete: 3 files (P1 coils+injectors 16 wires, P2 sensors+ETB 12 wires, combined index). 28 engine loom wires, ANSI D format, Manhattan routing, M130 pin blocks, shield envelopes, 5V ref/sensor ground buses.
- [deck-review] 5-agent stakeholder simulation: Ford Brand Team, Fouquet's GM, L'Agence principal, L'Officiel editorial director, FBM operator. Reviews + synthesis in docs/decks/reviews/. Key findings: split into 2 decks, fix logistics errors, add insurance/liability slide, define the ask, correct economics (K→.5-75.4K landed, -270K/yr operating).
- [deck-assets] Fleet Luxe deck assets: interactive SBH map (Leaflet, 60+ POIs, 8 filterable categories, dark theme), demographics doc (visitor stats, rental company landscape, hotel vehicle programs, Ford business case data), vehicle notes (Bronco vs Sport distinction, image sourcing rules). All in docs/decks/assets/.
- [wiring/v2-fixes] V2 harness documents: All 4 blockers fixed. Cut list v2 (123 wires/905ft — ETB 6 wires, speaker pairs, DEL-Stributor coil relocation). PDM30 channel plan reconciled + DB updated. Coil-to-pin mapping with DEL-Stributor. Wire routing guide (10 routes, firewall penetration map, grommet schedule, 5 inline disconnect points). Engine schematic v2 (2 pages with DEL-Stributor layout). Harness build sheets v2. 29 total files in docs/wiring/output/.
- [deck-assets] Bronco Roadster Concept discovery: Ford built a one-off modern U13 Roadster (Aug 2025, Robert Gelardi / Bronco Chief Designer). 12 official editorial-use images from fromtheroad.ford.com. Updated commercial structure + vehicle notes. This changes the entire Ford pitch — ask to PLACE the Roadster Concept on SBH, not to send inventory.
## 2026-04-03
- [data-quality] Fixed 240Z price:  → ,250 (BaT lot 27552). Identified systemic BaT price extraction bug: 618 vehicles with sale<K and estimate>K. Root cause: extractor capturing bid counts or truncated values instead of actual sale prices.

- [data-quality] Completed full data correction session: model case normalization (31,329 records across 9,701 groups), make case fixes (PORSCHE→Porsche, FIAT→Fiat, MINI, AC, RAM, BSA, REO, NSU, OSCA, etc.), model-as-make fixes (Corvette→Chevrolet, Mustang→Ford, Camaro→Chevrolet, etc. — 169 records), Mercedes Benz→Mercedes-Benz (5,515), junk VINs nullified (short ≤3 char), redundant make stripped from model (4 remaining). Final state: 832,598 vehicles, 11 make case issues, 12 model case issues, 0 junk VINs, 0 redundant make-in-model.
- [frontend] Click-through chains verified live: VIN popup (1 source + 12 badge photos), ENGINE TYPE popup (2 sources + 12 engine bay photos). Fixed column names (image_url, display_order) and zone prefixes (mech_engine%, detail_badge%, ext_%, int_%). All queries return real data.
- [question-taxonomy] Regex classification COMPLETE: 1,319,364 / 1,653,943 questions classified (79.8%). 112 taxonomy categories, 12 L1s. ~335K remaining for LLM fallback. Crons active (classify every 5m, refresh views every 30m). Bulk script: scripts/question-classify-bulk.mjs (124/s). Dashboard: /admin/qi. Vehicle widget: Buyer Questions on every profile. Cost: $0.14 total.
- [sprint] Deep Sprint FINAL: desc 37.5%→43.9%, price 40.8%→45.4%, field_evidence 0.21→10.27, ARS 100%, observation 27/27, edge fns 403→189, ClassicCars 0.5%→92.4%, BJ 43.6%→67.4%
- [map] Final fixes: MapTimeline hooks-before-return bug fix, zoom animations killed for instant response, county sidebar redesigned with bar charts + bigger vehicle cards. Paused for UX philosophy rethink — map needs a clear user purpose before more features.
- [data-quality] sale_price vs sold_price investigation and cleanup: nulled 3 BaT sale_price counts (<500 when sold_price >5K), 19 C&B sold_price=80 counts, 1 C&B sold_price=750 count, 64 sale_price=year values across all sources, 2 both-fields-equal-count (BaT , Mecum ), 1 C&B both=, 1 Mecum sale_price=1969 (year not price). Total 91 rows fixed. 364 remaining mismatches are legitimate multi-auction price differences.
- [data-quality] Continued data correction: fixed 1,902 re-ingested split makes, cleaned 209 dirty VINs (sentences, HTML entities, asterisks, year-in-VIN), nulled junk VINs, re-normalized 334 make case issues from new ingestion. Verified remaining 2,934 'dirty' VINs are legitimate vintage serial numbers (dashes, periods, spaces in pre-1981 chassis numbers). Final: 839,484 vehicles, 2 make case issues, 1 split make, 0 junk VINs — essentially clean. ROOT CAUSE: ingestion pipeline still creates bad data on every import.
- [extraction] Accuracy sweep complete: VINs (1,697 nulled, 5K fixed — zero garbage remaining), prices (91 fixed incl 64 year-as-price), makes (~800 normalized), BaT full descriptions (52K upgraded), BJ RSC parser (63K draining). Mecum short descs are source format, not truncation.
- [work-orders] Granholm K2500 work order data fix — all 7 steps: (1) Fixed 5 parts at $0 with real prices (alternator $191.99, e-brake $25, brake shoes $45, wheel cylinders $24, trans seal $12), (2) Added 7 missing labor entries (rear brake rebuild 3h comped, e-brake 1h comped, trans seal 2h@$80, alternator 1h@$80, master cyl 1h comped, air dam 0.5h@$80, carb 1h comped), (3) Removed duplicate master cylinder part + stale Carr hoop labor, (4) Added 4 Blazer transfer parts to exhaust WO ($612 — Borla mufflers, SS tubing, flex connectors, flanges), (5) Reconciled exhaust line items to $150/hr for fabrication, (6) Regenerated both invoices (INV-001 balance $821.46, INV-002 balance $2,775.10), (7) Fixed resolve_work_order_status() — now excludes cancelled parts + uses total_cost for flat-rate labor. Final: $3,316.73 paid, $3,062.31 balance due, $1,082.99 comped value.
- [work-orders] Customer statement view complete: (1) Fixed resolve_work_order_status to include comp_retail_value + comp_reason in parts/labor JSON, exclude cancelled parts, use total_cost for flat-rate labor. (2) Fixed work_order_receipt_unified view with same fixes. (3) Created generate_customer_statement() function — full structured JSON with billable_items, goodwill_items, payments_received sections + per-WO and grand totals. (4) Normalized comped data: total_price/total_cost=/bin/zsh for all comped items, comp_retail_value has actual cost. (5) Fixed resolve.mjs transform to map comp_retail_value correctly. Comped section now renders: 12 items, ,082.99 goodwill value.
- [work-orders] HTML work order statement generator: statement-html.mjs renders full automotive-style work order from generate_customer_statement() RPC. Shows parts/labor tables per WO, green goodwill section with retail values, payment history, totals with balance due. npm run wo:statement granholm opens in browser. Print-ready CSS.
- [frontend/work-order-statement] Built Work Order Statement editable invoice page: migration (admin CRUD policies + RPC fix with row IDs), useWorkOrderStatement hook (TanStack Query + CRUD mutations + optimistic updates), WorkOrderStatement.tsx page (inline cell editing, comp toggles, add/delete rows, send invoice), route at /work-orders/statement
## 2026-04-01
- [bj-enrichment] BJ enrichment session: wiped 2382 venue share images, queue-extracted 10918 vehicles, backfilled 967 listing_urls, extracted 2180 VINs from snapshots, 3058 VINs from conceptcarz chassis#, 531 descriptions, 474 API matches. Coverage: VIN 31→36%, desc 38→49%, image 33→40%, color 34→51%, trans 34→47%, engine 20→28%, price 38→45%. Wall hit: ~20K duplicate vehicle records need entity resolution.

- [frontend] WS2 React Query wave 3: created 10 hooks (useNukeEstimate, useSimilarSales, useEditHistory, useLinkedOrganizations, useVehicleVideos, useVehicleTimeline, useVehicleCommentsUnified, useMarketSegments, useVehicleAllComments, useAuctionMarketplace), converted 15 components from raw supabase+useState to useQuery. File count 23→40. WS4 hex color purge: replaced all standalone hex colors in vehicle-profile tsx files (148→0 standalone, 98 remain as proper var() fallbacks). Files changed: BuildTimelineChart, WorkOrderProgress, VehicleSubHeader, AnalysisSignalsSection, VehicleHeroImage, FieldEvidencePopup, FieldProvenanceDrawer, ObservationTimeline, VehicleBriefing, GenerateBill, DayCard.
- [frontend] WS3 navigate-on-click to Link: converted 56 onClick navigate patterns to Link across 25+ files. Reduced from 115 to 59 (target <60). Files converted: OrganizationCard, ServiceVehicleCard, SoldInventoryBrowser (gallery+grid), OrganizationProfile, Profile, CurationQueue, VehicleCollection, MarketFundDetail, VehicleHeader, BuilderDashboard, Organizations, MyOrganizations, MyAuctions, CreditsSuccess, MarketSegmentDetail, MergeProposalsDashboard, VehicleProfile, CreateOrganization, AuctionListing, UnlinkedReceipts, MarketDashboard, DealerBulkEditor, DealerAIAssistant, RestorationIntake, AuthErrorBoundary, VehicleContributors, EditVehicle, GlobalUploadStatus, Capture, DeveloperSignup. Remaining 59: shared files (waiting for Agent B), admin files (excluded), conditional/multi-action navigates, table row navigates.
- [frontend] WS3 shared files: converted onClick navigate to Link in Portfolio.tsx (5), MarketSegments.tsx (5), Vehicles.tsx (4). Preserved programmatic navigates in HomePage.tsx and Dashboard.tsx. Final count: 45 (from 115 baseline, target <60).
- [feed-query] WS1 COMPLETE: feed-query 16s→<1s. Denormalized has_photos+images+description into vehicle_valuation_feed MV. Added maintain_vehicle_has_photos trigger. Eliminated 5 enrichment queries. All 33 vehicles triggers verified enabled.
- [dead-code] WS5 COMPLETE: Deleted 28 dead edge functions + 1 SQL file, removed 11 cron jobs. Function count 218→189 (below 190 target). Skipped backfill-images (has live callers).
- [sprint] W3 Sprint complete — 5 workstreams: feed-query 16s→<1s warm, React Query 23→40, navigate-on-click 115→45, hex colors 148→0 standalone (98 var fallbacks), edge functions 218→189, 28 dead functions + 11 crons deleted, triggers re-enabled
## 2026-03-31
- [sprint] Deep Sprint W2: observation adoption 27/27 (100%), edge functions 314→217 (96 deleted), BJ desc 43.6%→72% (parser fixed), FB +1945 descs, ClassicCars+BJ backfills running overnight

- [frontend] Click-through chains: every dossier field value opens FieldEvidencePopup (sources + tagged photos + VIN decode). Library doc 09-click-through-chains.md written.
- [enrichment] Restarted BJ backfill (PID 8388, --batch 1000, 91.8% done ~3K remaining) and ClassicCars enrichment (PID 7868, 32.7% done ~22K remaining at 1150/hr)
- [fb-marketplace] FB pipeline audit: 43K vehicles, 40.8K marketplace_listings, zero dupes (facebook_id dedup), 18% desc coverage (7,750/43K). Sweep running 4-5x/day via 4 launchd groups across 58 US metros, 3,513 cities, 47 states, 83% with GPS. Started Playwright desc enricher on 500 active listings (PID 9843, ~43% success rate, ~65min ETA). 8,654 recent vehicles not yet in marketplace_listings.
- [yono] Scoped YONO contextual model plan: 4-phase build (tabular baseline -> frozen vision -> fine-tuned -> production). 351K sold vehicles, 290K with estimates, current baseline 28% MdAPE, target <20%. Plan at docs/library/working/working-papers/yono-contextual-model-plan.md
- [org-profile-v2] Org profile layout overhaul: (1) GMV bar chart with horizontal bars showing M trajectory for 911r, (2) geography + volume in side-by-side grid, (3) heatmap wired to auction events via externalEvents prop, (4) empty heatmap hidden until data loads, (5) confirmed 911r = Matt Crandall / Avant-Garde Collection, Portland OR, ~25 cars/month on BaT.
- [description-discovery] Fixed unique constraint (vehicle_id), added JSON repair, poison vehicle failure records, EXISTS remaining check. Deployed v88. Pipeline running at ~15 disc/min.
- [pipeline] 5-stream sprint: BJ backfill restarted (91.8%→finishing), ClassicCars enrichment restarted (32.7%, ~20hrs), FB sweep confirmed self-sustaining (2,500/day), desc decomposition pipeline fixed (dedup constraint, poison vehicles, JSON repair) and running at 15/min (~/bin/zsh.003/vehicle, 296K remaining), YONO contextual model plan written (28% baseline, <20% target, 4-phase approach).
- [P09-ownership] P09 ownership classification deep: expanded regex from 6 to 15+ patterns. 3 SQL passes: (1) dealer possessive/verb patterns +486 owner, (2) 'current owner' as consignment signal +831 consigner, (3) 'seller reports/notes' as owner signal +449 owner. Results: 1,444 owner (was 694), 1,603 consigner (was 780), 2,969 unclassified (was 4,537). 51% classified (was 33%). Remaining 2,969 have no ownership signal in text. extract-bat-core + backfill script updated with v2 regex.
- [extraction] Accuracy sweep: price (C&B bid counts, FB placeholders, BaT bid counts nulled), make (case normalization, truncation fixes, garbage nulled), year (engine CC→null), descriptions (boilerplate nulled, BaT full-text parser deployed, 63K queued for re-extraction)
- [library] Intelligence Surface documentation suite: 2,415 lines across 4 new library docs + 30 new dictionary entries. Discourse (the-knowing-system.md), Paper (use-case-atlas.md 47 use cases), Design Book Ch.11 (intelligence-surface.md), Engineering Manual (build-guide.md with SQL/React/CSS). Plan at .claude/plans/use-case-to-code.md.
- [data-quality] Major data correction session: fixed split makes (Land Rover, Mercedes-Benz, Rolls-Royce, Alfa Romeo, Aston Martin, Austin-Healey, De Tomaso, DeSoto, Can-Am, American Motors/IronHorse/LaFrance — ~4,200 records), case-normalized all makes (545 make variants → 0 inconsistencies, 3,233 records), purged ~12,000 BaT ghost vehicles (no VIN, manual_entry, ≤1 image), deleted ~40 VIN-conflicting dupes, created 26 missing FK indexes on vehicle child tables, nullified junk VINs (number/string/BILL OF SALE/lot numbers) and impossible years (<1885), stripped redundant make from model field (635 records), model case normalization in progress (354K vehicles across 9,285 model groups)
- [frontend] VehicleBriefing component: L0 headline + L1 stat pills + expandable comps. Renders between hero image and workspace content. Self-guarding progressive density. Headline priority: high-sev red flags > concerns (non-positive sentiment only) > market position > community sentiment > low-sev flags > observation count > estimate. Deployed to prod.
- [vlva-financial] Phase 1: Ingested 14 SP2024/INV2024 deals into deal_jackets (24 total). 14 reconditioning entries. 13 vehicle-linked, 1 unmatched (1978 K20 Blue). Phase 2: Built VLVAFinancialReport.tsx React page with 12 sections (capital structure, Price UT collection, KPI cards, inventory/consignment/undocumented tables, partner economics, operating expenses, unaccounted gap, projections, mystery deal, what's missing). Route: /org/vlva-report (protected). Script: scripts/vlva-numbers-ingest.mjs, npm run ingest:vlva.
- [P06-dealer-discovery] P06 dealer auto-discovery: 204 new BaT dealers discovered (50+ listings, 3+ years active). 210 total monitored sellers (was 6). 30,177 vehicles wired to dealer orgs (was 5,549). Top discoveries: genevamotorcars (772), FantasyJunction (470, $66M GMV), silverarrowcarsltd (398, $106M GMV), DSFM2005 (393, $58M GMV). Script: npm run discover:dealers. Wiring done via individual SQL INSERT per seller (batch JOINs hit 120s timeout).
## 2026-03-30
- [extraction] BJ snapshot parser fixed — RSC double-escaped JSON unescaping. 63K items reset to pending, draining at 1,200/hr. Verified: 7-9 fields per vehicle (VIN, price, desc, colors, engine, trans, mileage).

- [observation] Retrofitted 16 more extractors to observationWriter — now 27/27 (100% of vehicle-field-writing extractors). Added: extract-bat-core, extract-gooding, extract-broad-arrow, extract-bh-auction, extract-ebay-motors, extract-vehicle-data-ai, extract-barrett-jackson, extract-bonhams, extract-gaa-classics, import-classic-auction, import-classiccars-listing, import-fb-marketplace, ingest-wayback-vehicle, decode-vin-and-update, extract-barn-finds-listing, extract-bat-profile-vehicles, extract-vin-from-vehicle. Skipped 7 non-vehicle-field writers (cab-bids, bonhams-typesense, complete-bat-import, detailed-component-extractor, bat-parts-brands, auction-comments, c&b-comments).
- [hygiene] Phase 3 dead code: deleted 96 edge functions (313→217). Batch 1: 52 orphaned APIs/bots/OAuth/analytics. Batch 2: 17 dead features (transfers/SMS/Telegram/Stripe). Batch 3: 18 stalled experiments (forum/vision/wayback). Batch 4: 2 more (build-posts/training). All verified: zero code refs, zero crons, zero frontend consumers.
- [org-profile-fix] Fixed 3 org profile bugs: (1) duplicate vehicle rows — dedup by vehicle_id keeping best relationship_type, (2) 911r/large orgs failing — serial paginated queries instead of 20+ concurrent PostgREST calls, (3) ugly decimals + bad estimates — round to whole dollars, hide estimates with confidence <30 or >3x divergent from sale price, normalize state names (Texas→TX). All deployed and verified live.
- [extraction] BJ snapshot parser: diagnosed root cause (storage HTML not read by edge fn, www/non-www URL mismatch), built snapshot-first backfill script (scripts/bj-snapshot-backfill.mjs), tested on 1200+ snapshots, 76% vehicle match rate, ~1.3 fields/snapshot avg
- [bj-parser] Fixed BJ snapshot parser: root causes were storage download timeout in edge fn, www/non-www URL mismatch, and version stamp on 0-field results. Created scripts/bj-snapshot-backfill.mjs with snapshot-first approach, URL normalization, PostgREST pagination. BJ desc coverage 43.6% -> 72%+ with ~3K snapshots processed, full 54K backfill running.
- [enrichment] FB desc enrichment: 2 batches x 1000, 1945 enriched (98.4%/96.1% success), coverage 13.3% -> 13.5% on vehicles table, 17.4% on marketplace_listings. 0-5 errors per batch. Residential IP Googlebot fetch.
## 2026-03-29
- [data-quality] Triaged 221K null-platform-source vehicles. Backfilled 193K via URL patterns (BaT 31K, Classic Driver 50K, TheMarket 5K, ERclassics 1.7K, JamesEdition 303) and source field (C&B 32K, Mecum 31K, FB 8.4K, Gooding 6.3K, BJ 5.1K, ConceptCarz 6.3K, Craigslist 3.7K, user-submission 9.2K, small sources ~1K). Remaining 27K set to 'unknown' (95% identity-only, no URL, no VIN). Zero null platform_source public vehicles remain. Used SKIP LOCKED to avoid pipeline deadlocks.

- [vehicle-profile] Badge bar fixes: vertical alignment (baseline→center), OdometerBadge in sub-header, trim dedup, engine badge, city+state+zip location. Source badge overflow: AI EXTRACTION→AI in FieldProvenanceDrawer, column width 50→60px. Hero image: replaced blur backdrop with solid dark background (CSS bg-image ignores EXIF). Deployed to production.
- [data-audit] Deep audit of 289K ConceptCarz records: auction results aggregator data, 90.7% fabricated prices (now classified), cleanup script partially run (model names + garbage makes done, dedup + discovery_url still pending), 35% of entire corpus, dilutes coverage metrics by ~18pp description / ~14pp VIN / ~18pp price
- [data] Quarantined 273,794 ConceptCarz records without VINs (length < 11): set is_public=false, status='archived'. Cleared 244K tainted nuke_estimates/heat_scores/confidence values from fabricated prices. 11,955 VIN-bearing ConceptCarz records remain public as legitimate vehicles. Corpus metrics improved: desc 37%->54%, price 42%->59%, VIN 27%->39%. Updated almanac/platform-stats.md with post-quarantine numbers.
- [frontend] Treemap continuous zoom: 4th drill level (vehicles with photos), rep images for models/years, zoom transitions, no more navigation away
- [docs] Organization Sandbox concept in encyclopedia Section 18 + dictionary (Organization Sandbox, Provenance Container)
- [vehicle-profile] Conflict classification: domain synonym groups (4WD/4x4/etc), token-overlap refinement, model/trim field-specific logic. Fixed Diesel Injection bad data, rejected fabricated 454 TBI AI extraction. useFieldEvidence filters rejected status. Installed missing exceljs+openseadragon deps.
- [extraction] ClassicCars.com + Classic Driver enrichment pipeline: scripts/enrich-marketplace-listings.mjs. Direct HTTP fetch with browser UA, no Firecrawl/AI needed. ClassicCars.com: JSON-LD parsing (@type:car) for VIN/price/mileage/color/description. Classic Driver: Drupal HTML spec field regex + meta description fallback. 14K/hr CC, 10K/hr CD. 238 vehicles test-enriched (129 CC, 109 CD). CC: 91% desc, 87% price, 64% VIN, 78% mileage. CD: 91% desc, 86% price, 99% color. npm: enrich:marketplace, enrich:classiccars, enrich:classic-driver.
- [map-backend] get_county_detail(p_fips) RPC — returns JSONB with county_name, state_fips, vehicle_count, total_value, top 20 vehicles (id/year/make/model/prices/image/status), top 10 makes with counts, platform breakdown. Uses county_fips index + spatial fallback. Tested LA County (7,978 vehicles), Autauga AL (3 vehicles), Salt Lake UT (487 vehicles), invalid FIPS (error case). Accessible via REST API.
- [map-backend] get_make_heatmap(p_make) RPC — returns JSONB with make, total_vehicles, county_count, and counties array (fips/county_name/state_fips/count) from mv_make_geographic_density joined with us_county_boundaries. Tested Porsche (15,808 vehicles, 592 counties), Ford (27,297, 903 counties), Chevrolet (37,150, 931 counties), nonexistent make (empty). Accessible via REST API.
- [map-frontend] Extracted ChoroplethMap.tsx (290 lines) from NukeMap.tsx (1022→135 lines). Created shared.ts for FIPS lookups, colorways, drill levels. Created useMapFilters.ts hook. Both agents delivered: get_county_detail + get_make_heatmap RPCs ready. BaT geocoding: 149K of 151K done.
- [edge-function] feed-query Phase 4 optimization: (a) module-level dealer org ID cache with 5-min TTL, (b) Cache-Control headers (public s-maxage=300 for default feed, private max-age=30 for filtered/search), (c) imported corsHeaders from _shared/cors.ts, (d) clarified Pass 3 image fallback purpose (legacy vehicles.primary_image_url for vehicles with no vehicle_images rows)
- [map-pipeline] BaT city geocoding backfill: 98,530 VLOs upgraded region→city (95,810 US + 2,720 Canadian). 36,866 lat/lng filled from city_geocode_lookup. 471 new cities added to lookup. FB Marketplace: 31,602 new VLO records created from marketplace_listings. C&B: 35,809 records upgraded (13,847 cars_and_bids parsed + 21,962 carsandbids geocoded). Mecum/Gooding/Bonhams: 46,652 auction-city records upgraded. Global FIPS: 154,429 county_fips assigned across all platforms. 6 materialized views refreshed. Final: 358,947 total VLOs, 94.9% at city/gps/address precision (was ~50%), 2,174 counties with data.
- [fb-marketplace] Built taste scoring system: compute_taste_score() SQL function derived from 826 FB saves (43x lift at 80+ band), auto-trigger on insert/update, save-as-label trigger, taste_model_metrics view, enricher v1.1 prioritizes by taste_score DESC. Baseline: 0.1% match rate, 825 misses.
- [performance] Phase 1-5 Performance Deep Plan: admin dedup (19→1 calls via React Query), image transforms (optimizeImageUrl on GarageVehicleCard/StorefrontInventory/SoldInventoryBrowser/ServiceVehicleCard/ContentCard), React Query hooks (useAdminAccess/useVehicleImages rewritten, useProfile/useOwnershipVerification created), feed-query caching (dealer org IDs cached 5min, Cache-Control headers), VehicleProfile query consolidation (RPC-provided images/timeline/stats skip duplicate fetches, deferred non-critical queries 500ms, removed dead AdminNotificationService call)
- [org-profiles] P1-P5 org profile overhaul: fixed estimate circularity (43.5K circular flagged, self_price_fallback blocked for sold vehicles), wired bat_seller→org (5,549 vehicles across 6 dealers, 5,887 vehicle_events), built per-vehicle seller track record with GMV/volume/geography, classified ownership (694 owner, 780 consigner, 2 built), added competitive context. Migration: comp_method + is_circular on nuke_estimates. 2 new scripts. extract-bat-core + compute-vehicle-valuation deployed.
- [frontend] Vehicle profile polish — 5 phases: (1) timeline + button always visible, (2) year separators in heatmap, (3) semantic field grouping (Identity/Powerplant/Drivetrain/Appearance/Metrics), (4) analysis_signals section with popup detail, (5) gallery filter foundation in context with filter chip UI
- [fb-marketplace] Library-driven follow-ups: enrichment_priority = taste_score × freshness_decay (signal formula S=V×T×D×A), auto-intent trigger on new saves (Finder model), taste_intent_signals populated (569 signals, 15 makes, 24 models), taste_smart_folders view live. Enricher v1.1 now uses composite priority — top of queue is 1979 Blazer Cheyenne (90.3) instead of random recent listings.
- [map-frontend] Confidence-aware pin rendering in ChoroplethMap.tsx — 3-tier visual hierarchy (GPS/verified: r6/100%/1px, city: r5/80%/0.5px, county: r3/50%/no-border), sub-0.50 filtered from pins, provenance tooltip shows source+confidence+location
- [library] Created 7 engineered follow-up prompts (P06-P12) in docs/library/prompts/. P06: seller auto-discovery. P07: valuation comp audit. P08: org provenance container. P09: deep ownership classification. P10: org reputation signal. P11: cross-platform seller resolution. P12: modified vehicle valuation. Each follows the library prompt pattern: context refs, problem, scope, steps, verify SQL, anti-patterns, library contribution.
- [map-frontend] Wired temporal MapTimeline scrubber into ChoroplethMap — renders histogram at bottom of choropleth view, shows SHOWING time window label in info panel, passes time bounds through useCountyData/useStateData/mapService (backend county mode uses mat view so time filtering is UI-only until edge function gets temporal county query path)
- [map-frontend] Make Heatmap Mode: Added make filter to ChoroplethMap. Type a make (Porsche, Ford, etc) and press Enter to re-color choropleth by that make's county density via get_make_heatmap RPC. Shows make stats in panel, filters sidebar vehicles to that make on county click. Clear button resets to all-vehicle view. Types in types.ts (MakeHeatmapData), fetch in mapService.ts (fetchMakeHeatmap), UI + logic in ChoroplethMap.tsx.
- [auction-trends] Auction Trends Dashboard v2: Fixed 1000-row limit bug (was hiding BaT's 5K active auctions), normalized platform slugs (barrett-jackson merged), moved all aggregation to SQL function get_auction_trends_v2() (replaces 8 PostgREST queries + JS), filtered bid artifacts (bid_count >= 2), separated auction platforms from listing platforms. Added 5 depth sections: platform sell-through (BaT 55.4%), segment leaders (15 makes — Porsche/Ford/Chevy lead), price distribution (5 bands), estimate accuracy (771 compared, hammer = 151% of est), supply/demand balance (weekly). Edge function rewritten to single RPC call. Frontend fully updated with 2-column layout.
- [map-wave2] Wave 2 complete: (1) Confidence-aware pin rendering — GPS/city/county tiers with provenance tooltips, state centroids hidden from pins. (2) Make heatmap mode — type 'Porsche' to recolor choropleth by make density, county sidebar filters to that make. (3) Temporal scrubber — MapTimeline wired into ChoroplethMap, time bounds plumbed through to data hooks. All grounded in library philosophy: epistemological honesty, ontological pluralism, temporal evidence.
- [frontend] Phase 2: Profile density guards — observation history guarded by observationCount > 0, THIN badge on VehicleSubHeader for vehicles with <5 spec fields populated
- [schematics] Vehicle profile data flow schematic: 10 flows documented (core load, images, timeline, intel, field evidence, build status, scores, ARS, analysis signals, auction pulse), master ASCII diagram, component layout tree, realtime subscriptions — 310+ lines
- [almanac] Platform stats almanac updated: 828K vehicles, 504K public, 36.3M images, 5.7M observations, 987K timeline events, 126K comment discoveries, 31K description discoveries, density breakdown (63% sparse, 14% moderate, 22% dense), pipeline activity section added
- [docs] Encyclopedia chapters 01-04: unified-asset-layer (402 lines), observation-model (576 lines), timeline-architecture (463 lines), entity-resolution (403 lines). Total 1,844 lines. All reference real DB schemas from live queries (vehicles 340 cols, vehicle_observations 38 cols, 159 sources, 987K timeline events, 5.67M observations). Real enum values, real row counts, real edge function names.
- [frontend] Intelligence surfacing: useVehicleIntel hook + VehicleIntelligencePanel (community intel, description intel, apparition history) wired into vehicle profile via VehicleProfileContext. 126K vehicles with comment intel + 31K with description intel now visible on profile page.
- [frontend] Profile density: Observation History null-guarded (only renders when observationCount > 0). THIN badge on VehicleSubHeader for vehicles with <5 populated spec fields.
- [docs] Library: 4 encyclopedia chapters (unified-asset-layer, observation-model, timeline-architecture, entity-resolution) — 1,844 lines with verified DB schemas. Vehicle profile data flow schematic (310 lines, 10 flows). Almanac updated with live stats (828K vehicles, 63% sparse/14% moderate/22% dense).
- [frontend] P12: Powerplant field deduplication in dossier panel — token-based overlap detection hides redundant engine_size and fuel_type fields, with provenance quality guard and expandable collapsed indicator
- [frontend] P14: Timeline filter modes — ALL/WORK/PHOTOS/SALES/DISCOVERY pills on BarcodeTimeline heatmap
- [map-backend] Added temporal county/state query path to map-vehicles edge function. When time_start or time_end params are passed with mode=county or mode=state, queries VLO directly using county_fips column (94% populated) instead of mat view. Uses parameterized SQL, SET LOCAL statement_timeout 15s, same JSON shape as mat view path. Avoids expensive PostGIS spatial join by leveraging pre-computed county_fips on VLO.
- [map-frontend] MapVehicleDetail sidebar: added location provenance (VLO records with source_platform, confidence tier, precision), regional context (comparable vehicle count by make/year range), and price context with visual bar (min/max/rank, progressive density: hidden when < 3 comparables). 3 new Supabase queries, TypeScript clean.
- [map-frontend] Proximity search: right-click drops pin, radius selector (25/50/100/200mi), queries VLO for nearby vehicles with haversine distance, draws cyan pins + dashed circle overlay, sidebar with thumbnails sorted by distance. Clear via left-click or Escape.
- [frontend] P15: Dossier provenance depth indicators — epistemological layer borders on FieldRow, segmented coverage bar, layer tooltips
- [map-wave3] Wave 3 complete: (1) Proximity search — right-click drops cyan pin, radius selector (25/50/100/200mi), shows nearby vehicles as teal pins + sidebar list with distances. Haversine distance calc, escape to clear. (2) Vehicle sidebar computation surface — location provenance with confidence tiers, comparables query (same make ±5yr), price context with visual bar showing position among comparables. Progressive density. (3) Temporal county backend — map-vehicles edge function now queries VLO directly with time_start/time_end using county_fips index instead of mat view. Needs deployment.
- [fb-marketplace] Library-driven system complete: anomaly detection (G-Class 5 saves, Land Rover 5 saves, builder profile 52 equipment saves), cross-platform intent view (Blazer=2416 platform vehicles with avg sale $57K), complementary discovery (Grand Wagoneer, CJ-7, F-100), geographic coverage view (Phoenix 31 taste-matches 0 enriched, LA 42 taste-matches 1 enriched). Enricher v1.1 tested: 14/15 enriched, all 80+ priority, 39 fields.
- [frontend] P11: Gallery filter widget wiring — ImageGallery reads galleryFilter + 3 emitters
- [auction-trends] Library-guided depth: Added 3 new sections to auction trends — Bid Depth (CBT principle: price monotonic with bid count, thin=K→intense=K), Comment Mood (19 auctions, 78.9% positive, sold=0.67 vs unsold=0.89 sentiment), Market Quality badge (HIGH confidence, N=1175, 99.4% competitive, CBT-weighted avg K vs raw K). SQL function get_auction_trends_v2() updated. Segments now show density label (sparse/moderate/deep). Frontend updated with 3 new components.
- [profile-surface] P10-P15 vehicle profile computation surface: P10 (DayCard narrative), P11 (gallery filter wiring), P12 (powerplant dedup), P14 (timeline filters), P15 (provenance depth) were already implemented in commit 7489c4d5e. P13 (analysis signals sweep): created scripts/backfill-analysis-signals.mjs, tested on 10 vehicles (14 signals each), npm run ops:signals. 279K vehicles eligible for backfill.
- [frontend] P10: Day Card seven-level analysis narrative — useDayCardContext hook + computeNarrative template + ANALYSIS section rendering
- [frontend] QI page full-resolution data explorer — 7 drill-down components (QIBreadcrumb, QIL1Detail, QICategoryDetail, QICommentRow, QIFieldDetail, QIAuthorDetail, QIPagination), URL-driven navigation via search params, clickable treemap/charts/table/gap bars, 2 composite indexes on auction_comments (idx_ac_question_l2_posted, idx_ac_vehicle_l2_posted) — Index Scan confirmed (cost 10 vs 1M seq scan)
- [backend] P13: Analysis signals compute sweep for most-viewed vehicles
- [auction-trends] Library audit fixes: replaced all hardcoded spacing with var(--space-*) tokens (73 uses), fixed font sizes (13px→11/12px), fixed 4px grid violations (-10px→-8px, 6px→8px, 1px→2px), bars 6px→4px. Zero TypeScript errors. Created coordination files for multi-agent audit (.claude/agents/coordination/). Created /audit-and-fix slash command for future coordinated audits.
- [frontend] QI explorer audit fixes — 41 violations found, all fixed: 4 CRITICAL SQL injection (whitelist validation on field column, sanitize l2/author), 16 HIGH (all font sizes to CSS vars, 6px/7px bumped to 8px min, transition timing fixed), 14 MEDIUM (1px borders→2px, any→proper types, empty shell gates, stable keys), 7 LOW (shared constants.ts extracts L1_COLORS/TaxonomyRow/fmtK/isValidFieldColumn). Pre-commit audit command created.
- [map-audit] Audit fixes: removed border-radius+box-shadow from proximity pin (F1), removed empty FOLLOW/BUY/ADD TO COLLECTION buttons (F3/W3), added statement_timeout to points/histogram pool path (W6), deduplicated STATE_FIPS from useStateData.ts to use shared.ts (W2), registered map-vehicles + RPCs in TOOLS.md (F5). NukeMap rewrite documented as intentional refactor (F3).
- [library-audit] Audited org profile work against library. Added 6 dictionary terms (Circular Estimate, Comp Method, Competitive Context, Ownership Classification, Seller Track Record, Self-Price Fallback mapped to existing entries). Added 3 encyclopedia subsections (Seller→Org Resolution, Ownership Classification, Valuation Circularity Fix). Registered 3 scripts in TOOLS.md. Created .claude/commands/audit.md for multi-agent audit coordination.
- [observation] Built ingest-observation-batch edge function (max 200/req, stop_on_error/gap_fill/write_evidence options, delegates to ingest-observation via internal HTTP). Added Observation Ingestion section to TOOLS.md with 4 entries.
- [performance] 10 library-grounded audits complete + audit-and-coordinate.md v2 written
- [observation] Retrofitted 4 extractors to use observationWriter.ts: batch-extract-snapshots, enrich-listing-content, extract-cars-and-bids-core, discover-description-data. All use fire-and-forget pattern alongside existing vehicle writes.
- [hygiene] Dead code audit: deleted 84 definitely-dead edge functions (403→319). Categories: 8 deleted features, 13 defunct extractors, 7 defunct OAuth, 7 defunct agents, 5 defunct receipt funcs, 5 defunct analytics, 8 defunct org/seller, 3 defunct scrapers, 8 defunct image/photo, 20 defunct misc. Identified 127 empty tables (14 safe to drop, 60+ digital twin schema to keep). Full report: .claude/dead-code-audit-results.md
- [library] Extended Entity Resolution encyclopedia chapter from 403 to 1275 lines. Added: philosophical foundation (testimony model), real-time resolution cascade with code (4-pass ingest-observation), URL normalization (12 platforms), VIN validation (fake VIN rejection), merge_into_primary SQL function (full annotated source), batch dedup system (exact + normalized modes), AI-verified merge proposals (26 proposals, decision distribution), chassis observations table, canonical evidence hierarchy (4 tiers), source trust scores (160 sources), implementation phases status, 8 known limitations. All claims backed by code references and live database stats.
- [commits] Coordinated 6-commit batch from multi-agent session: (1) 86 dead edge functions removed (-34K lines), (2) auction trends v2 with library audit fixes, (3) library encyclopedia + atlas + almanac, (4) map choropleth overhaul, (5) pipeline/extractor improvements, (6) frontend: org profiles, vehicle intel, QI explorer, page fixes. Total: 168 files across 6 agents' work.
- [performance] All perf deep plan work committed (c8cab71 + 634a4d5). 10 audits complete. audit-and-coordinate.md v2 shipped.
- [cleanup] Post-audit sweep: purged 82 Helvetica font violations across 8 files, cleaned 15 stale worktrees (~25GB), cleaned 7 stale agent registrations, verified VehicleIntelligencePanel vs AnalysisSignalsSection not duplicate
- [library] Extended trust scoring papers with implementation reality. trust-scoring-methodology.md: added 160-source production hierarchy (12 categories with actual base_trust_score values from DB), exact confidence compounding code from ingest-observation, production distribution (5.7M obs: 96.3% high, 0.5% verified, 3.2% medium), field_evidence gap analysis (3.29M rows parallel to observation system), theory-vs-implementation gap table. dynamic-trust-model.md: added infrastructure inventory (what exists to enable dynamic trust), corroboration opportunity analysis (37K vehicles with 3+ sources), distance-from-theory-to-production appendix.
- [dead-code] Deleted 7 PROBABLY DEAD edge functions with zero references: analyze-vehicle-tags, extract-historics-uk, extract-rennlist, extract-thesamba, geocode-vehicle-locations, message-fb-seller, yono-export-training. Kept 32 with references (cron/edge/frontend/external-webhook/API-keys).
- [observation-system] Task #19: Deployed ingest-observation-batch. Retrofitted 4 more extractors to observationWriter.ts: extract-hagerty-listing (next_data_json_parse), import-pcarmarket-listing (dom_parse), extract-rmsothebys (rmsothebys_api, both update+insert), extract-facebook-marketplace (fb_direct_input, both update+new). Skipped collecting-cars-discovery (writes to import_queue, no vehicleId). Observation adoption now 13/30 extractors.
- [dead-code] Phase 2 dead code cleanup: deleted 14 more edge functions (api-v1-export/listings/makes/organizations/pipeline-status/valuations, extract-pdf-text, geocode-organization-locations, part-number-ocr, process-bat-buyer-monitors, process-bat-seller-monitors, quickbooks-webhook, stripe-webhook, work-order-lifecycle). Total: 403→300 functions. 14 empty tables already dropped by other agent (127→113).
- [observation-system] Task #22: Retrofitted 2 more extractors to observationWriter.ts: extract-mecum (next_data_json_parse, trust 0.8) and extract-jamesedition (firecrawl_markdown_parse, trust 0.6). Observation adoption now ~15/30 extractors (10 files import observationWriter + 5 that already use ingest-observation directly).
- [platform] Border violations: 67 fixes across 24 files. Dead code: 7 edge functions deleted, 14 empty tables dropped. 14 wrongly-deleted functions restored.
- [almanac] Updated platform-stats.md with sprint W1 close snapshot (2026-03-30): field_evidence 44.7%, edge functions 298, tables 716, DB 89GB, ARS 39.1%, all coverage % deltas. Updated PROJECT_STATE.md with sprint W1 achievements summary.
- [map] Map workspace session complete. All waves shipped: choropleth, make heatmap, proximity search, vehicle sidebar, temporal queries, confidence pins. Committed as 6f157def3. Audited against library — fixes applied. Edge function deployed.
- [sprint] Deep Sprint W1: 5-agent team, 22 tasks. field_evidence 0.21→10.2/vehicle, edge functions 403→300, observation adoption 15/30, ARS 100% coverage (324K scored), ER chapter 3x'd, ClassicCars enrichment 0.5%→24%+ (still running), /sync command created
- [frontend] Migrated 5 vehicle profile hooks from useState+useEffect to React Query: useVehicleIntel, useBuildStatus, useBuildProfile, useFieldEvidence, useBuildLog. Zero TSC errors.
- [extraction] BJ+Mecum snapshot audit: BJ has 43K snapshots but parser extracts 0 fields for 95% of them (48,832/51,302). Root cause: generic snapshot parser doesn't match BJ HTML structure. Mecum healthy at 82% desc, 88% price, queue draining. BJ needs dedicated parser or parser fix before re-queue.
- [frontend] CSS semantic tokens (--vp-danger, --vp-sold, --vp-brg) + 22 hardcoded hex replaced. React Query migration for all 5 profile hooks (useVehicleIntel, useBuildStatus, useBuildProfile, useFieldEvidence, useBuildLog). Net -55 lines.
- [feed-query] Fixed cold-start 500s: replaced 130-entry inline NOT IN with DB lookup table non_auto_makes_excluded + module-level cache. Added connection warm-up on cold start. Added timeout fallback query. 0/5 cold calls now 500 (was 2/5). Cold start 4.7s, warm 1.1-1.6s.
- [dead-code] WS6 dead code cleanup: deleted BuildLog.tsx, ImageGalleryV2.tsx, VehicleImageGallery.tsx (all confirmed zero importers). Removed dead type interfaces from types.ts. useBuildLog.ts kept (types used by DayCard via BarcodeTimeline). MobileImageGallery kept (used by VehicleHeroImage). 4 parallel system issues flagged in ISSUES.md for W3. Build and tsc pass clean.
- [frontend-hooks] WS5: Created 3 React Query hooks (useVehicle, useVehicleEvents, useAuctionComments/useAuctionCommentStats). Migrated VehicleBanners + VehicleBadgeBar to use shared hook. useQuery count: 17 -> 23.
- [frontend-images] WS3: Image optimization wave 2. Added optimizeImageUrl to 15 files: HomePage, VehicleThumbnail, RecentlyViewed, FreshFindsStrip, VehiclePopup, AuctionMarketplace, DayCard, ChoroplethMap (4 instances), AIDataIngestionSearch, VehiclePricingWidget, StorefrontVehicleDetail, ZipSidebarPanel, MapOrgDetail, MapVehicleDetail, EventMap. Usage count: 49 -> 63. Added loading=lazy to all non-hero vehicle img tags.
- [frontend-ws2] Loading state cleanup: deprecated .loading-spinner/.spinner CSS (removed animations), fixed 20+ Suspense fallbacks to null/static, replaced 25+ Loading... text with return null or static fills, removed skeleton animations in FeedSkeleton/SearchResults/HomePage/Dashboard/MarketSegments/NukeEstimatePanel. Violations 118→79.
- [frontend-ws4] Progressive density fixes: VehicleDescriptionCard returns null when empty+!editable, SimilarSalesSection returns null when no sales, NukeEstimatePanel returns null when loading, removed CollapsibleWidget wrappers around ExternalListingCard/VehicleVideoSection (both self-guard), VehicleReferenceLibrary returns null for all users with 0 books+docs.
- [frontend] Sprint W2 complete: 6 workstreams — feed-query cold start (5/5 200s), loading violations (118→79), image optimization (49→63), progressive density (6 violations fixed + BuildTimelineChart), React Query hooks (17→23 files), dead code (3 files deleted + 4 parallel systems flagged)
## 2026-03-28
- [granholm-k2500] Vision-classified 379/381 build photos (99.5%) via local Ollama llama3.2-vision. Area distribution: exhaust 49%, brakes 20%, suspension 12%, engine 5%. Created scripts/classify-work-photos.py for reusable work photo classification.
- [granholm-k2500] Inserted 13 line items into WO d18b1119 (Pre-Delivery). 6 complete, 7 in progress. 3 comped (window motor, speedo, rear brakes). Updated exhaust WO DT-001 line item costs. Total: 28h labor, ,580.71 parts, ,260.71 billable.
- [granholm-k2500] Updated INV-K2500-0227-001 (,795.56 total, ,316.73 paid, .83 due). Created supplemental INV-K2500-0327-002 for exhaust (,822.19 due, draft). Combined outstanding: ,301.02.
- [frontend/hero-image] Fixed hero showing wrong vehicle from iphoto contamination. Added is_primary priority (Attempt 0), source trust scoring (+50 for bat/user_upload over iphoto/drop-folder), removed auto-override of valid primaries, defaulted hero to cover mode. Set correct K2500 sunset photo as is_primary in DB.
- [docs] The Extraction Handbook — 3,059 lines, 14 chapters, 5 parts. Complete institutional knowledge of extraction ops. Live-tested by 4 cold-start agents. Chapters: History (16 eras), Repeated Mistakes (12 patterns tallied), Methods (16 ranked), LLM Config, Architecture, Failures (16 documented), Runbooks (7), Current State, Source Notes (18 platforms), Facebook (SLC use case), Stalled Initiatives (9 audited), Prompt Evolution (8 eras), Graduation Path (4 layers). Security-reviewed, structurally verified, self-corrected from test findings.
- [extraction] 4 high-ROI actions: BaT queue 44K→590 failed, find_vehicles_near() RPC deployed (35 sq bodies near SLC), Mecum real coverage at BaT-tier (86%/79%/81%), BJ VIN chassis backfill, critical repopulate_snapshot_queue() bug fixed (blocked ALL platforms)
- [question-taxonomy] Pipeline built + 100K/1.65M classified (6%). 48 taxonomy entries, 82.8% regex hit rate, 57% precision (needs 15K sample rerun). Infra: 5 cols on auction_comments, question_taxonomy table, mv_question_intelligence view, question_classify mode in analyze-comments-fast, question_classify_llm mode in batch-comment-discovery, discovery script. Chain running autonomously.
- [data] Vehicle Realization Plan v2: table-driven persist_realization_plan() + get_comp_tiers() + stage_transition_labor_map extended (28 rows, 6 categories) + wired into 3 edge functions (enrich-listing-content, discover-description-data, process-cl-queue)
- [frontend] Build log on vehicle profile: BuildLog.tsx (container with calendar strip, stats, sign-off, billing), DayCard.tsx (expandable day cards with photos grouped by area, receipts, events), WorkOrderProgress.tsx (sign-off checkboxes update line item status), GenerateBill.tsx (inline invoice renderer with send-to-customer via Resend), useBuildLog.ts (hook calling get_vehicle_work_dates + lazy get_daily_work_receipt RPCs). Wired into WorkspaceContent.tsx left column after Build Status. Self-guarding: returns null if no work_sessions. K2500 test vehicle has 26 sessions, 381 photos, 16 line items.
...
- [vehicle-profile] Cleaned up photo-export contamination: deleted ~24,500 junk photo-export rows across 9 vehicles, fixed hero image race condition in VehicleProfileContext.tsx (heroResolvedRef), set correct BaT primary for K2500. Identified 10+ profile issues in audit session with user - documented in ISSUES.md
- [map] County choropleth map built: Leaflet-based county heatmap with 4 colorways (amber/plasma/viridis/heat), draggable info panel, dark/light basemap switching. Refreshed stale mat views (48K→117K vehicles, 2109 counties). Added 6-hour cron for mat view refresh. deck.gl wrapped in error boundary. Default mode is now county.
- [map-data] Rewrote county/state mat views to use PostGIS spatial lookup instead of regex zip extraction. Imported 3,231 US county boundaries into us_county_boundaries table with GIST index. Data went from 48K (stale) → 117K (refreshed zip) → 212K (spatial). 98.8% lat/lng match rate to counties. Updated cron job.
- [vehicle-profile] Conflict classification (genuine/refinement/synonym/variance), image grid default GRID, timeline label removed, badge padding, hero-to-columns spacing. 3 edge functions deployed (discover-description-data, enrich-listing-content, process-cl-queue).
- [images] Removed 4 internal analysis badges (EXTERI/SCANNED/AI/TIER1) from all image gallery views. Source badge overflow fixed.
- [extraction] FB Marketplace description enrichment: Googlebot-from-residential-IP approach validated (96-100% success vs 13% from cloud). Built fb-desc-enrich.mjs (Googlebot, parallel capable) + fb-desc-enrich-playwright.mjs (browser fallback). Backfilled 9 orphaned descriptions from marketplace_listings. Created propagate_fb_descriptions() SQL function. Added 8 npm scripts (fb:desc-enrich:*). 500-batch run in progress, ~50 enriched so far at 0.1/s.
- [docs] Stale docs audit: categorized 339 non-library docs into ARCHIVE (95), DUPLICATE (8), REVIEW (36), KEEP (~200). Created docs/STALE_DOCS_AUDIT.md
- [library] Almanac: created docs/library/reference/almanac/platform-stats.md — 735K vehicles, 36M images, 5.3M observations, 12.6M comments. Platform source breakdown, coverage metrics, refresh queries.
- [library] Atlas: created docs/library/reference/atlas/sources.md — 119 observation sources mapped across 12 categories. Trust scores, base URLs, access methods, notes. 4 duplicate slug issues flagged.
- [map-data] FIPS backfill complete: 155,630 VLO observations tagged with county_fips via PostGIS spatial lookup. Created 4 new analysis mat views (county_market_analysis, make_geographic_density, platform_geographic_coverage, state_market_summary). Added cron for all 6 geo views. Added index on vlo.county_fips. 94K US observations unmatched due to simplified boundary gaps — could improve with higher-res boundaries.
- [backend] Nuke estimate live invalidation: trigger marks comps stale on price change, cron now recomputes stale estimates (was NULL-only). 661K estimates, auto-refresh cycle ~1500/hr.
- [vehicle-profile] Hero EXIF orientation fix (skip Supabase render endpoint), tab bar active dot, nuke estimate live invalidation trigger+cron
- [extraction] Snapshot extraction throughput fix: BaT from 20/hr to 2400+/hr. Root cause: edge function timing out downloading HTML from Supabase Storage (batch_size=50, 120s timeout). Fixes: (1) marked 5,877 already-complete BaT items done, (2) created drain_bat_queue_from_metadata() SQL function that applies pre-parsed data from origin_metadata without storage downloads, (3) drained ~4,500 BaT items via SQL metadata drainer, (4) reduced BaT edge function batch_size 50->10, (5) increased Gooding batch_size 50->100, (6) new cron job 'drain-bat-queue-from-metadata' (every 5 min, 200/batch), (7) queued 12 missing BaT vehicles. BaT pending: 16K->9K, Gooding pending: 3.7K->3.2K. Combined throughput: 3,100+/hr vs 620/hr before.
## 2026-03-27
...
- [security] Fixed CRITICAL shared fetcher redirect/login detection vulnerability: batFetcher.ts (redirect:manual + isLoginPage + authRequired flag), hybridFetcher.ts (redirect:manual + login detection on redirects and 200 OK), archiveFetch.ts (isLoginPage in isGarbageHtml + authRequired propagation). Deployed 12 affected edge functions. Marked CRITICAL issue FIXED in ISSUES.md.
- [frontend] Fixed all HIGH + MEDIUM frontend bugs: (1) removed overflow:hidden from .header in AppHeader.css, (2) removed hideSearch logic from AppHeader.tsx so search shows on homepage, (3) deleted legacy .header rule from design-system.css, (4) fixed --header-height from 48px to 40px in unified-design-system.css, (5) fixed navigation by extracting RoutedApp with location-aware ErrorBoundary resetKeys in App.tsx, (6) fixed CSS variable root causes: --transition to 180ms cubic-bezier, --border-width: 2px added, --shadow/--shadow-inset/--shadow-lg set to none, 4 border:1px instances fixed to var(--border-width)
- [build-tracking] Contact-vehicle resolver: vehicle_resolver_aliases table (34 aliases, 9 vehicles) + resolve_vehicle_from_input() RPC — 'granholm' instantly resolves to K2500 via 4 match paths. Gmail receipt mining: 5 receipts (3 Summit + 2 Amazon, ,238.41 total) with item-level JSON and gmail_message_id cross-references. Work order update: pre-delivery WO expanded from original 7-item scope to full 3x scope (15 parts), Carr hoop steps cancelled, Davis Off Road steps added, master cylinder upgraded to received. Exhaust WO corrected (not Y-pipe, straight piping). Parts transfer tracking: parts_transfers table with 4 Blazer-to-K2500 transfers ( replacement value) all flagged for reorder.
- [pipeline] Deployed realization v2 (3 edge functions), fixed LLM fallback chain (gemini-2.5-flash-lite primary, 4-provider fallback), parallelized BaT queue (3x concurrent), updated GOOGLE_AI_API_KEY secret. Mecum snapshot queue healthy at 1.1K/hr (16.7K remaining, ~15h ETA).
- [work-photos] Activity linking system: tracked_devices + device_locations + known_places tables, device-ping edge function deployed, 8K phone pings backfilled from EXIF, 71 C10 AirTag registered, Karmann Ghia vehicle created (7bb50537), drop-folder ingestor wired with AirTag co-location resolution before vision fallback, iOS Shortcut endpoint live
- [fb-marketplace] National sweep pipeline: GPS backfill 38%->84% (16,501 vehicles geocoded via city_geocode_lookup table), fixed location propagation in scraper + refine-fb-listing, motorcycle cleanup (1,881 Honda/Suzuki misclassified -> rejected), find_vehicles_near() working with FB data (SLC square body test: 34 results), deployed updated refine-fb-listing edge function
- [stylometrics] Batch profiled 300 new BaT users (1,021 total, target was 1,000). Zero failures. Layer 0 stylometric fingerprints: writing signature, vocabulary metrics (Yule's K), function word fingerprints, punctuation signatures, epistemic stance, taste fingerprints. All stored in bat_user_profiles.metadata.stylometric_profile. Added --skip-existing, --quiet flags to user-stylometric-analyzer.mjs.
- [library] Rebuilt library INDEX: 11 edge functions -> 75+ functions across 12 categories with cron schedules. Added By Edge Function (cron-driven + on-demand), By Table (24 core tables), By Shared Utility, By Script, Key Principles sections. Updated DICTIONARY with 16 new terms (Agent Hierarchy, Analysis Engine, ARS, Era Detection, Stylometric Profile, Palimpsest, Widget, Yule's K, Layer 0, etc.). Updated ISSUES.md status.
- [frontend] Popup data renovation: VehiclePopup wired to comment/description intelligence + nuke estimate + apparition history + comps via popup_vehicle_intel RPC. MakePopup shows live inventory + recent sales + model taxonomy via popup_make_intel. SourcePopup shows sold-this-week + live now + source character via popup_source_intel with alias normalization. ModelPopup fixed useNavigate bug. FeedFilterSidebar sticky positioning fixed with CSS vars. 3 new DB RPCs with index for performance.
- [granholm-build] Dated 350 drop-folder photos via IMG sequence interpolation against iphoto anchor points. Created 26 work sessions (Feb 27 - Mar 27). Linked 381 photos to sessions. Built get_daily_work_receipt() and get_vehicle_work_dates() RPCs. 5 receipts totaling $1,239.41 across Summit Racing and Amazon.
- [frontend] Make search input in filter sidebar, ModelPopup rewritten with single RPC + price scatter canvas, MakePopup price scatter added
## 2026-03-26
