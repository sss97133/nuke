...
> **Reference conventions in this doc:** `commit-sha` and `supabase/migrations/<file>` are committed on `main`. Paths tagged *(local)* are operator scripts that exist in the working tree but are **not yet committed to `main`** — they're real and in use, but don't grep them on the remote.
...
**1. The intake pipeline (overnight fleet sprint, June 9→10, committed on `main`).** A real self-healing queue now drives photos from capture to analysis with no human babysitting:
- **Passive capture** — iPhone → nuke photo-sync daemon, zero touches (`65fdbd6e6`), with an on-device privacy/ownership gate (`5c1f28543`).
- **Self-healing queue** — `vehicle_images.ai_processing_status` (`pending → processing → completed`/`failed`), `reset_stuck_photo_pipeline_images()` re-arms anything stuck, orchestrator trigger fires on insert (`701e86df1`, migration `supabase/migrations/20260609000001_photo_pipeline_self_healing.sql`).
...
- **Pulse control room** — `get_pipeline_pulse()` (function, migration `supabase/migrations/20260610000005_pipeline_pulse.sql`) surfaced in `nuke_frontend/src/pages/SystemStatus.tsx`: throughput per organ per day, red at zero (`1abcbea99`).

**2. The detective analysis (BYOK, operator scripts — *local*, not yet committed).** Claude Code *is* the compute, zero API spend (`scripts/deep-image-analysis-byok.mjs` *(local)*, `scripts/daily-receipt/byok-image-batch.sh` *(local)*, `scripts/daily-receipt/build-day.mjs` *(local)*). DAY is the unit. Each frame gets a **context briefing**:
- **Dossier** — identity + build_summary + work timeline, assembled server-side (the `_shared/dossier.ts` helper) and rendered in `nuke_frontend/src/pages/vehicle-profile/VehicleDossierPanel.tsx`.
...
- 🟡 **Provenance — observability only.** The pulse function makes throughput visible, but the byok writer still doesn't land typed `agent_model` / `extraction_method` / `agent_cost_cents` / `agent_duration_ms`. "Numbers carry source DNA" is still half-broken at the row level. Fix is still in `scripts/deep-image-analysis-byok.mjs` *(local)*. This was crack #1.
- 🔴 **No confirmation loop — UNTOUCHED, now the top priority.** `ai_processing_status='completed'` means *the AI analyzed the frame* — **not that Skylar confirmed what happened that day.** The strongest compounding input (owner ground truth becoming a prior for every future day) still does not exist. This was crack #2.
...
### Scope: confirmation is a DAY-level concern, kept separate from frame-level status

These are two distinct status systems and the design must not conflate them:

- **Frame-level (unchanged):** `vehicle_images.ai_processing_status` stays `pending → processing → completed`/`failed`. It tracks *AI analysis completion only* and gets no new states.
- **Day-level (new):** confirmation lives on `work_sessions` (DAY is the analysis unit). The committed `work_sessions` schema (migration `20250831155000_create_work_sessions.sql`) has **no status or confirmation column today** — so this is a net-new, supersession-safe addition, not a repurposing of an existing enum.

The day-level confirmation flow:

```text
work_sessions row (AI-inferred labor day)
        │  confirmed_at IS NULL  ── surfaces in triage
        ▼
   owner confirms ──────────────► confirmed_at / confirmed_by / intent set  (owner-trust prior)
   owner corrects ──────────────► supersede the AI verdict, write a corrected row
   owner says "not mine" ───────► detach the day (do not delete testimony)
```

Concretely, against tonight's substrate:

1. **Migration:** add `confirmed_at`, `confirmed_by`, and `intent` to `work_sessions` (supersession-friendly; never overwrite the AI's verdict in place). Candidate days = `work_sessions` rows that carry a labor claim but `confirmed_at IS NULL`.
2. **Triage surface that costs Skylar minutes, not hours.** One card per unconfirmed labor day: the day's hero frames, the AI's claimed intent (labor / inspection / parts_sourcing / comms), claimed hours, claimed location. Three actions: **confirm**, **correct** (change intent/hours), **not-mine**. Render it where he already is — top of the workspace (the `UserTodayCard` lane, `863ee4843`) or as a tab on the vehicle profile, selecting `work_sessions WHERE confirmed_at IS NULL`.
3. **A confirmation write path.** A `confirm_work_session` MCP tool is referenced in agent tooling but has **no in-repo implementation/registration yet** (not found in `supabase/functions/mcp-connector/index.ts`). Building/registering that write path — or routing through `ingest-observation` per the agent-trust rule — is part of this work, not a given. A confirmation is testimony: owner-trust, highest tier.
4. **The loop closes when** the next day's BYOK briefing reads *confirmed* prior days distinctly from *inferred* prior days — so the detective compounds on ground truth, and metric #3 climbs from 0%.
...
- **E — Pipeline reliability + provenance fix.** Drain reliability is now largely shipped (fleet). Remaining: land full source DNA in the byok writer (`scripts/deep-image-analysis-byok.mjs` *(local)*).
