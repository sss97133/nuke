...
The alternator conversation was a prototype for the supply side. The Smith build tracking conversation was a prototype for the parts inventory layer. Every time an agent does manual research that could have been a query, the system has failed to capture knowledge that it demonstrated it could acquire.
