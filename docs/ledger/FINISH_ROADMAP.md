...
**Already built:** work_sessions 1,899 rows with a live writer in the image drain; confirm_work_session owner-gate (confirm/amend/reject); full invoice engine (resolve_work_order_status RPC + project_customer_invoice MCP tool + editable statement) proven on the Smith K2500; ShopFinancials.tsx live in Profile.
