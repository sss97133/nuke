...
- Create `vehicle_image_tags`/`likes`/`facts`/`assets` tables — they DO NOT EXIST by design (universal-search/tagService were repointed to the real `image_tags` table 2026-07-12; a vehicle_image_tags table still needs a design decision before anyone creates it). `vehicle_image_angles` (0 rows) is the scar from baking a categorical into schema.
