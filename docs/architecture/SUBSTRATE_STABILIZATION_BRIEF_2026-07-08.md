...
- **Only 2 of 38 are genuine DUPLICATE-RECORD candidates** (same make+model, ≤1yr apart) — both **owner-gated (Skylar's vehicles)**:
  ...
  2. **`a90c008a` (1983 GMC K2500, VIN 1GTGK24M0XX000000, bat, OWNED, 2202 img) ↔ `d6a01df2` (1983 GMC K2500, no VIN, user-submission, OWNED, 1457 img)** — same Y/M/M, both OWNED, ~6 shared images (weaker). Plausible one-truck-two-records (bat listing vs user submission); reconcile VIN onto the richer record if confirmed. Owner call.
