-- 🚀 FIXED NUKE PLATFORM SETUP
-- Copy this ENTIRE file into Supabase Dashboard → SQL Editor and run it
-- Fixed the syntax error in UNIQUE constraint
...
-- Vehicle service roles table (FIXED SYNTAX)
...
-- Add unique constraint separately (FIXED)
CREATE UNIQUE INDEX IF NOT EXISTS unique_active_vehicle_service_role 
ON vehicle_service_roles(vehicle_id, service_provider_id, role) 
WHERE status = 'active';
...
-- Sample professionals for testing (using dummy UUIDs)
INSERT INTO professional_profiles (
  user_id, business_name, business_type, specializations, 
  hourly_rate_min, hourly_rate_max, service_radius_miles,
  years_experience, city, state, is_verified, description
...
(
  ...
  'Bay Area Motors', 'garage',
  ARRAY['transmission', 'engine', 'electrical'],
  85.00, 150.00, 75, 15, 'Oakland', 'CA', true,
  'Full-service automotive repair specializing in European vehicles.'
),
(
  '22222222-2222-2222-2222-222222222222',
  'Mobile Mechanic Pro', 'mobile',
  ARRAY['diagnostics', 'maintenance', 'brakes'],
  65.00, 100.00, 25, 8, 'San Jose', 'CA', true,
  'Mobile automotive services - we come to you for convenient repairs.'
),
(
  '33333333-3333-3333-3333-333333333333',
  'Performance Plus', 'specialty_shop',
  ARRAY['performance', 'tuning', 'modifications'],
  100.00, 200.00, 100, 12, 'Sacramento', 'CA', true,
  'High-performance vehicle modifications and racing preparation.'
)
...
-- Show setup results
SELECT '🎉 SETUP COMPLETE - SYNTAX FIXED!' as message;
SELECT 'Professionals available: ' || COUNT(*)::text as professionals_count FROM user_professional_status;
SELECT 'Your profile exists: ' || CASE WHEN EXISTS(SELECT 1 FROM profiles WHERE id = '0b9f107a-d124-49de-9ded-94698f63c1c4') THEN 'YES ✅' ELSE 'NO ❌' END as profile_status;
