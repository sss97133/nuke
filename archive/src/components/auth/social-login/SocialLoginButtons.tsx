
import { Button } from "@/components/ui/button";
import { Github } from "lucide-react";
import { Provider } from "@supabase/supabase-js";

interface SocialLoginButtonsProps {
  onSocialLogin: (provider: Provider) => void;
  isLoading: boolean;
  onError?: (error: any) => void;
}

export const SocialLoginButtons = ({ onSocialLogin, isLoading, onError }: SocialLoginButtonsProps) => {
  const handleSocialLogin = async (provider: Provider) => {
    try {
      await onSocialLogin(provider);
    } catch (error) {
      console.error(`Error logging in with ${provider}:`, error);
      if (onError) {
        onError(error);
      }
    }
  };

  return (
    <>
      <div className="flex flex-col space-y-3">
        <p className="text-sm text-center text-gray-500 font-medium mb-1">Continue with</p>
        
        <Button
          variant="outline"
          onClick={() => handleSocialLogin('github')}
          disabled={isLoading}
          className="w-full py-2.5 border border-gray-300 bg-white hover:bg-gray-50 text-gray-800 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-gray-400"
        >
          <Github className="mr-2 h-4 w-4" />
          <span className="font-medium">GitHub</span>
        </Button>
        
        <Button
          variant="outline"
          onClick={() => handleSocialLogin('google')}
          disabled={isLoading}
          className="w-full py-2.5 border border-gray-300 bg-white hover:bg-gray-50 text-gray-800 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-gray-400"
        >
          <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" aria-hidden="true">
            <path
              fill="#EA4335"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#4285F4"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          <span className="font-medium">Google</span>
        </Button>
        
        <Button
          variant="outline"
          onClick={() => handleSocialLogin('facebook')}
          disabled={isLoading}
          className="w-full py-2.5 border border-gray-300 bg-white hover:bg-gray-50 text-gray-800 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-gray-400"
        >
          <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" aria-hidden="true">
            <path
              fill="#1877F2"
              d="M20 12a8 8 0 10-9.25 7.903v-5.59H8.719V12h2.031V9.797c0-2.066 1.198-3.187 3.022-3.187.875 0 1.79.156 1.79.156v2.031h-1.008c-.994 0-1.304.62-1.304 1.258V12h2.219l-.355 2.313H13.25v5.59A8.002 8.002 0 0020 12z"
            />
          </svg>
          <span className="font-medium">Facebook</span>
        </Button>
      </div>
    </>
  );
};
