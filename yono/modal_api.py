"""
YONO API Plane — Scale-to-Zero SDK Endpoint

Thin FastAPI endpoint for real-time SDK requests (nuke.vision.analyze(url)).
Replaces always-on yono-serve with scale-to-zero.

Cost: $0 when idle. Cold start ~3-5s (load ONNX from volume).

Deploy:
    modal deploy yono/modal_api.py

Get URL:
    modal app show yono-api
...
def _download_florence2():
    ...
    print("[YONO-API] Downloading Florence-2-base weights...")
    ...
    print("[YONO-API] Florence-2-base cached.")
...
# ============================================================
# Model loaders (identical to modal_serve.py)
# ============================================================
...
def _load_onnx_models():
    ...
    if flat_path.exists() and flat_labels_path.exists():
        ...
        print(f"[YONO-API] Flat model: {len(flat_labels)} classes")
        ...
        for family in family_labels:
            ...
            if t2_path.exists():
                try:
                    ...
                    print(f"[YONO-API] Skipping {family} tier-2 (load error: {e})")
        print(f"[YONO-API] Hierarchical: tier1={bool(tier1_sess)}, tier2={list(tier2.keys())}")
...
def _load_zone_classifier():
    """Load Florence-2 zone classifier head."""
    import json
    import torch
    ...
    if not (head_path.exists() and labels_path.exists()):
        print("[YONO-API] Zone classifier files not found — skipping.")
        return None
    ...
    try:
        ...
        head = head.to(device)
        head.eval()
        print(f"[YONO-API] Zone classifier loaded: {n_zones} zones")
        ...
        print(f"[YONO-API] Zone classifier load failed: {e}")
        return None
...
def _load_vision_analyzer():
    ...
    print("[YONO-API] Loading Florence-2-base for vision analysis...")
    try:
        ...
        model.eval()
        print("[YONO-API] Florence-2-base loaded.")
    except Exception as e:
        print(f"[YONO-API] Florence-2 load failed: {e}")
        return None
    ...
    if head_path.exists() and config_path.exists():
        try:
            ...
            print(f"[YONO-API] Vision head loaded (val_loss={head_config.get('best_val_loss', '?'):.4f})")
        except Exception as e:
            print(f"[YONO-API] Vision head load failed: {e} — falling back to zero-shot")
            head = None
            head_config = None
...
@app.function(
    ...
    min_containers=0,       # SCALE TO ZERO — was 1 (then 2) on yono-serve
    max_containers=4,       # hard cap
    scaledown_window=120,   # 2 min idle → scale down
    timeout=600,
)
@modal.asgi_app()
def fastapi_app():
    ...
    api = FastAPI(title="YONO API Server")
    api.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
