...
async function syncConnection(db: ReturnType<typeof svc>, connectionId: string, opts: { maxPages: number }) {
  ...
  if (Date.now() - created > 45 * 24 * 3600 * 1000) {
    try {
      ...
      // store new + repoint the row BEFORE deleting old — a crash between delete and
      // store would leave the connection holding a dead secret id
  ...
  let pages = 0;
  let complete = true;
  let oldestSeen: string | null = null; // bounds partial-walk deletion detection
  ...
  // Complete walk: reconcile the whole cache. Partial walk (feed longer than maxPages):
  // reconcile the window we actually saw — cached rows at/after the oldest post seen.
  // Purge means purge: binaries out of storage, caption gone; row stays as a tombstone.
  if (seen.length && (complete || oldestSeen)) {
    let vq = db.from('org_instagram_media')
      .select('id, storage_path, thumb_path')
      .eq('connection_id', conn.id).is('deleted_at', null)
      .not('ig_media_id', 'in', `(${seen.join(',')})`);
    if (!complete) vq = vq.gte('taken_at', oldestSeen as string);
    const { data: gone } = await vq;
    if (gone?.length) {
      const paths = gone.flatMap((r) => [r.storage_path, r.thumb_path]).filter(Boolean) as string[];
      for (let i = 0; i < paths.length; i += 100) {
        await db.storage.from('concierge-media').remove(paths.slice(i, i + 100));
      }
      await db.from('org_instagram_media').update({
        deleted_at: new Date().toISOString(), caption: null, storage_path: null, thumb_path: null,
