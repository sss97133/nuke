...
serve(async (req) => {
  ...
  try {
    ...
    try {
      const { data: coreResult, error: coreError } = await supabase.functions.invoke("extract-premium-auction", {
        body: {
          url,
          max_vehicles: 1,
        },
    ...
    if (results.vehicle_id) {
      ...
      try {
        const { data: commentResult, error: commentError } = await supabase.functions.invoke("extract-auction-comments", {
          body: {
            auction_url: url,
        ...
        if (commentError) {
          ...
          const errorMsg = `extract-auction-comments failed: ${commentError.message}`;
