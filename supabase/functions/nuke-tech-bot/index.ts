...
Deno.serve(async (req) => {
  ...
  if (m.photo?.length && tech) {
    ...
    await supabase.from("vehicle_images").insert({ vehicle_id: tech.active_vehicle_id, url: pu.publicUrl, source: "telegram" });
    await send(chatId, `📸 Saved!` + (m.caption ? ` "${m.caption}"` : ""));
    return new Response("OK");
  }
