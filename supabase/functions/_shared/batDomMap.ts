...
function extractGalleryImagesFromHtml(html: string): { urls: string[]; method: string } {
  ...
  try {
    ...
    if (m?.[1]) {
      const jsonText = safeDecodeHtmlAttr(m[1]);
...
function extractLocation(
  ...
  // 1) Badge with title="Listing location" (user-reported pattern)
  try {
  ...
  // 3) Fallback: try any element that contains "Location:"
  try {
...
function extractSellerBuyer(doc: any, essentials: Record<string, string>): { seller: string | null; buyer: string | null; method: string } {
  const seller =
    essentials['seller'] ||
    (() => {
      const m = textContent(doc.body).match(/\bSold by\s+([A-Za-z0-9_]+)\b/i);
      return m?.[1] || null;
    })();
  ...
  const buyer =
    ...
    (() => {
      const m =
        textContent(doc.body).match(/\bSold to\s+([A-Za-z0-9_]+)\b/i) || textContent(doc.body).match(/\bto\s+([A-Za-z0-9_]+)\s+for\b/i);
      return m?.[1] || null;
    })();
...
function extractSaleAndCurrentBid(doc: any): { sale_price: number | null; current_bid: number | null; method: string } {
  ...
  const bid = body.match(/\bCurrent Bid\b[\s\S]{0,120}?\bUSD\s*\$([\d,]+)/i) || body.match(/\bBid to\b[\s\S]{0,120}?\bUSD\s*\$([\d,]+)/i);
  if (bid?.[1]) {
    const n = asInt(bid[1]);
...
function extractDescription(doc: any): { text: string | null; method: string } {
  ...
  if (t && t.length >= 50)
    return { text: t.slice(0, 4000), method: el?.className ? `selector:${String(el.className).split(' ')[0]}` : 'selector:description' };
...
function extractCommentsAndBids(doc: any, html: string): { comments: number; bids: number; method: string } {
  ...
  try {
    ...
    if (bidCount === 0) {
      ...
      const sm = h.match(/data-stats-item="bids"[^>]*>\s*([\d,]+)\s*bids?\s*</i) || h.match(/\b([\d,]+)\s*bids?\b/i);
...
function extractLocation(doc: any, essentials: Record<string, string>): { raw: string | null; clean: string | null; method: string; warnings: string[] } {
  const warnings: string[] = [];
  ...
  try {
    ...
    const t = textContent(badge);
    if (t) {
      // Badge sometimes contains extra UI copy; strip known suffixes.
      const cleaned = t
        .replace(/View all listings.*/i, '')
        .replace(/Notify me about new listings.*/i, '')
        .trim();
...
function extractSellerBuyer(doc: any, essentials: Record<string, string>): { seller: string | null; buyer: string | null; method: string } {
  // Prefer member links
  try {
    const soldBy = doc.querySelector('a[href^="/member/"]');
    const u = textContent(soldBy);
    if (u) {
      // Not perfect (may find buyer or commenter); we still prefer essentials when present.
    }
  } catch {
    // ignore
  }
  ...
  const buyer =
    essentials['buyer'] ||
    (() => {
      const m = textContent(doc.body).match(/\bSold to\s+([A-Za-z0-9_]+)\b/i) || textContent(doc.body).match(/\bto\s+([A-Za-z0-9_]+)\s+for\b/i);
      return m?.[1] || null;
    })();
...
function extractLotNumber(essentials: Record<string, string>, url: string): { lot: string | null; method: string } {
  // Prefer explicit Lot: # in essentials if present
  const fromEssentials = essentials['lot'] || essentials['lot #'] || null;
  if (fromEssentials) {
    const m = fromEssentials.match(/(\d{2,})/);
    return { lot: m?.[1] || fromEssentials, method: 'essentials' };
  }
  // Fallback: last numeric token from /listing/...-12345/
  try {
    const u = new URL(url);
    const m = u.pathname.match(/\/listing\/.*-(\d+)\/?$/i);
    if (m?.[1]) return { lot: m[1], method: 'url' };
...
function extractAuctionDates(doc: any, html: string): { start: string | null; end: string | null; sale: string | null; method: string; warnings: string[] } {
  const warnings: string[] = [];
  const h = String(html || '');

  // Most accurate for active auctions (JS attribute)
  const ends = h.match(/data-auction-ends=["'](\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2})["']/);
  if (ends?.[1]) {
    const parts = ends[1].split('-').map((x) => Number(x));
    if (parts.length === 6 && parts.every((x) => Number.isFinite(x))) {
  ...
  // Sold/ended date (text)
  const body = textContent(doc.body);
  const saleM = body.match(/\bSold for\b[\s\S]{0,120}?\bon\s+(\d{1,2}\/\d{1,2}\/\d{2,4})/i) ||
    body.match(/\bBid to\b[\s\S]{0,120}?\bon\s+(\d{1,2}\/\d{1,2}\/\d{2,4})/i) ||
    null;
  const parse = (d: string): string | null => {
    const dt = new Date(d);
...
function extractSaleAndCurrentBid(doc: any): { sale_price: number | null; current_bid: number | null; method: string } {
  const body = textContent(doc.body);
  const sold = body.match(/\bSold for\b[\s\S]{0,120}?\bUSD\s*\$([\d,]+)/i) || body.match(/\bSold for\b[\s\S]{0,120}?\$([\d,]+)/i);
  if (sold?.[1]) {
    const n = asInt(sold[1]);
    if (n && n >= 1000) return { sale_price: n, current_bid: n, method: 'text:sold_for' };
  }
...
function extractDescription(doc: any): { text: string | null; method: string } {
  const el =
    ...
    doc.querySelector('[class*="description"]');
  const t = textContent(el);
  if (t && t.length >= 50) return { text: t.slice(0, 4000), method: el?.className ? `selector:${String(el.className).split(' ')[0]}` : 'selector:description' };
  return { text: null, method: 'none' };
}
...
function extractCommentsAndBids(doc: any): { comments: number; bids: number; method: string } {
  try {
    ...
    // Bids are indicated in comment text: "$X bid placed by"
    const body = textContent(doc.body);
...
function computeScore(x: Partial<BatDomMapHealth>): number {
  ...
  // Weights: images + location + title/identity are core. Comments/bids are important but not always present in static HTML.
  let score = 0;
  const add = (ok: boolean, w: number) => { if (ok) score += w; };
...
export function extractBatDomMap(html: string, url: string): { extracted: BatDomExtracted; health: BatDomMapHealth } {
  ...
  const fields: Record<BatDomFieldKey, BatDomFieldHealth> = {
    title: { ok: !!title, method: titleMethod, value_preview: title ? title.slice(0, 120) : null },
    identity: { ok: !!identity.year && !!identity.make && !!identity.model, method: 'parse:title', value_preview: identity.year ? `${identity.year} ${identity.make || ''} ${identity.model || ''}`.trim().slice(0, 120) : null },
    lot_number: { ok: !!lot, method: lotMethod, value_preview: lot },
    location: { ok: !!loc.clean || !!loc.raw, method: loc.method, value_preview: (loc.clean || loc.raw || null)?.slice(0, 120), warnings: loc.warnings },
    seller: { ok: !!sb.seller, method: sb.method, value_preview: sb.seller },
    buyer: { ok: !!sb.buyer, method: sb.method, value_preview: sb.buyer },
    auction_dates: { ok: !!dates.end || !!dates.sale, method: dates.method, value_preview: dates.end || dates.sale || null, warnings: dates.warnings },
    sale: { ok: !!sale.sale_price || !!sale.current_bid, method: sale.method, value_preview: sale.sale_price ? String(sale.sale_price) : (sale.current_bid ? String(sale.current_bid) : null) },
    images: { ok: gallery.urls.length > 0, method: gallery.method, value_preview: gallery.urls[0] || null },
    description: { ok: !!desc.text, method: desc.method, value_preview: desc.text ? desc.text.slice(0, 120) : null },
    comments: { ok: cb.comments > 0, method: cb.method, value_preview: cb.comments ? String(cb.comments) : null },
    bids: { ok: cb.bids > 0, method: cb.method, value_preview: cb.bids ? String(cb.bids) : null },
