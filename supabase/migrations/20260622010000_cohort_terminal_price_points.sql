-- 20260622010000_cohort_terminal_price_points.sql
-- Applied to prod 2026-06-22 (additive, read-only). Adds `price_points` to
-- get_make_model_terminal: EVERY priced cohort sale, uncapped + unbiased, with its
-- source and date-or-null — so the cohort page plots all the dots colored by source
-- instead of a median (a lie for individuated collectibles) or a single-source line.
-- The comps block (LIMIT 24 by price DESC) stays for the thumbnail list; it is biased
-- to the priciest and date-required, so it CANNOT be the scatter source.
-- n_dated exposes the time-coverage gap: for the K5 Blazer cohort, 32 priced sales /
-- 9 sources but only 14 carry a date (all BaT) — the client SHOWS the 18 undated in a
-- gutter rather than dropping them off the time axis.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'note', 'Cohort terminal: live aggregates ASSEMBLED from substrate + curated fields '
            'PROJECTED via project_attribute. A block with populated=false is an intake gap.'
  );
END;
$function$;