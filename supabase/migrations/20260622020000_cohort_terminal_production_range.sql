-- 20260622020000_cohort_terminal_production_range.sql
-- Applied to prod 2026-06-22 (CREATE OR REPLACE, additive to the function body).
-- SUPERSEDES 20260622010000_cohort_terminal_price_points.sql: that migration added the
-- `price_points` block (every priced cohort sale, uncapped, with source + date-or-null +
-- n_dated) to get_make_model_terminal. This migration keeps price_points AND changes the
-- `production` block to:
--   (1) match on canonical model (canonical_models alias resolution) OR the raw p_model,
--       so production rows registered under either spelling are found;
--   (2) return a RANGE (min_produced / max_produced / total_produced) instead of a single
--       figure, because uncited per-trim figures shouldn't masquerade as one authoritative
--       number for the cohort;
--   (3) carry a `verified` flag (bool_or source_url is not null) + a representative source_url,
--       so the client can mark cited-vs-uncited production data.
-- This is the current live definition of get_make_model_terminal(text,text,integer,text).
-- Mirrors prod; do not edit to "improve" — pull the live def and re-sync if it drifts.
...
BEGIN
  ...
  SELECT jsonb_build_object('populated', count(*) > 0, 'source', 'vehicle_events.final_price (uncapped per-sale)',
           'n', count(*), 'n_dated', count(*) FILTER (WHERE d IS NOT NULL),
           'points', COALESCE(jsonb_agg(jsonb_build_object('vehicle_id', vehicle_id, 'price', price, 'date', d,
                       'source', source, 'miles', miles, 'url', url) ORDER BY price), '[]'::jsonb))
    INTO v_points FROM (
    SELECT ve.vehicle_id, ve.final_price AS price, COALESCE(ve.sold_at, ve.ended_at)::date AS d,
