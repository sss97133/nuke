...
CREATE INDEX idx_admin_notifications_ownership_verification ON admin_notifications(ownership_verification_id);
CREATE INDEX idx_admin_notifications_vehicle_verification ON admin_notifications(vehicle_verification_id);
...
BEGIN
  ...
  SELECT ov.*, u.email INTO verification_record, user_email
  FROM ownership_verifications ov
  JOIN auth.users u ON u.id = ov.user_id
