-- Algorithmic completion calculator v2 (idempotent)
...
BEGIN
  ...
  SELECT COALESCE(score, 0) INTO timeline_score FROM timeline_calc;
  ...
  SELECT COALESCE(score,0), COALESCE(cohort_count,0) INTO field_score, cohort_size FROM field_calc;
  ...
  SELECT COALESCE(total_trust,0) INTO trust_score FROM trust_indicators;
  ...
  SELECT COALESCE(COUNT(*) + 1, 1) INTO cohort_rank
  FROM vehicles v
  WHERE v.make = v_data.make
    AND v.year BETWEEN (v_data.year - 3) AND (v_data.year + 3)
    AND COALESCE(v.completion_percentage,0) > COALESCE(final_completion,0);
  ...
  RETURN jsonb_build_object(
    ...
    'rank_percentile', CASE WHEN cohort_size > 0 THEN ROUND((1.0 - (cohort_rank::NUMERIC / cohort_size)) * 100, 1) ELSE NULL END
  );
END;
...
RETURNS TRIGGER AS $$
DECLARE completion_data JSONB;
BEGIN
  completion_data := calculate_vehicle_completion_algorithmic(NEW.id);
  NEW.completion_percentage := COALESCE((completion_data->>'completion_percentage')::INTEGER, NEW.completion_percentage);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN QUERY
  WITH updates AS (
    SELECT v.id, v.completion_percentage as old_pct,
  ...
  SELECT id, old_pct, new_pct, (new_pct - old_pct) as change FROM updates ORDER BY change DESC;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION recalculate_cohort_completion IS 'Batch recalculate completion for vehicle cohort when new data shifts averages.';
