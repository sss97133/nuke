-- Live auctions: extend timer to 2 minutes on every bid.
-- - Keeps flash auctions "alive" by resetting end_time on each bid.
-- - Applies only to sale_type = 'live_auction'; standard auctions keep soft-close window behavior.
