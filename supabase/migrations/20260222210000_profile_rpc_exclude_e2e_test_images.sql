-- Exclude E2E test images from vehicle profile RPC so test data never appears as primary or in gallery.
-- Images with source = 'e2e_test' are from BaT E2E tests and can be wrong vehicle/placeholder.
  ...
  SELECT json_build_object(
    ...
    'images', (
      ...
      WHERE vi.vehicle_id = p_vehicle_id
        ...
        AND (vi.source IS NULL OR vi.source <> 'e2e_test')
    ),
    'timeline_events', (
    ...
    'price_signal', (
      ...
      FROM public.vehicle_price_signal(ARRAY[p_vehicle_id]) vps
      LIMIT 1
    ),
...
COMMENT ON FUNCTION public.get_vehicle_profile_data(uuid) IS 'Vehicle profile payload; excludes e2e_test images so test data never appears as primary or in gallery.';
