...
CREATE INDEX IF NOT EXISTS idx_user_interactions_action_type ON public.user_interactions(interaction_type);
...
CREATE INDEX IF NOT EXISTS idx_user_interactions_user_action_time ON public.user_interactions(user_id, interaction_type, created_at);
...
DROP FUNCTION IF EXISTS analyze_user_interests(uuid);
...
BEGIN
    ...
    SELECT 
        CASE 
            WHEN ui.target_type = 'discovery' OR ui.target_id LIKE 'discovery_%' THEN 'vehicle_discovery'
            WHEN ui.target_type = 'for_sale' OR ui.target_id LIKE 'forsale_%' THEN 'for_sale'
            WHEN ui.target_type = 'text' OR ui.target_id LIKE 'text_%' THEN 'text_updates'
            WHEN ui.target_type = 'maintenance' OR ui.target_id LIKE 'maintenance_%' THEN 'maintenance'
            ELSE 'other'
...
DROP FUNCTION IF EXISTS get_feed_recommendations(uuid, integer);
...
SELECT 
    ...
    COUNT(DISTINCT ui.target_id) as unique_targets_interacted,
...
COMMENT ON TABLE public.user_interactions IS 'Tracks user interactions with feed content for interest learning and personalization';
