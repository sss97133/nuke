-- get_vehicle_profile_data: use vehicle_price_signal( uuid[] ) instead of vehicle_price_signal_view
-- so the RPC works when the view is not present (e.g. view lives in frontend sql only).
...
IS 'Returns complete vehicle profile data in a single query. Uses vehicle_price_signal() so it does not depend on vehicle_price_signal_view.';
