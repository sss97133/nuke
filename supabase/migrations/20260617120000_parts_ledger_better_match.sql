-- REVERT: the token-extraction "improvement" regressed (broke the 3 real LMC matches, priced 0,
-- and added a slow leading-wildcard scan). The non-LMC parts (Baer/Holley/ACDelco) simply aren't
-- in catalog_parts — that's a real catalog-coverage gap the function already flags, not a matcher
-- failing. Restore the original whole-normalized-part-number exact match (fast, prices the LMC parts).
