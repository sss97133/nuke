...
-- but without per-row function call overhead, enabling 10K+ rows/batch
--
-- SECURITY DEFINER required for SET session_replication_role = 'replica'
-- which disables triggers during the bulk UPDATE (29 triggers on vehicles table!)
--
-- The scoring formula (max 100):
...
Uses session_replication_role=replica to bypass 29 triggers on vehicles table.
Use: SELECT fast_backfill_quality_scores(10000); -- processes 10K rows per call
Throughput: ~800-1000 rows/sec with triggers disabled';
