-- 20260622040000_get_make_model_sentiment_points.sql
-- Applied to prod 2026-06-22 (new STABLE function + grants).
-- NEW: get_make_model_sentiment_points(make, model, year, grain) — per-comment points for
-- the cohort sentiment alignment map. Each point carries comment_id (drillable to source),
-- vehicle_id, sentiment (-1..1 from sentiment_score/100), stance (-1..1 community_stance_score,
-- null = unscored), comment kind/seller flag/author/likes, and truncated text. Excludes bids.
-- Returns three blocks: `spectrum` (1-D sentiment aggregate), `comment_points` (the scatter),
-- and `second_axis` (community_stance aggregate — the Y axis of the alignment map).
-- Granted to anon + authenticated so the public cohort page can render it.
...
begin
  ...
  select jsonb_build_object('populated', count(*)>0, 'source','auction_comments', 'n', count(*),
     ...
     'points', coalesce(jsonb_agg(jsonb_build_object(
        ...
        'likes', ac.comment_likes, 'text', left(ac.comment_text,280)) order by ac.sentiment_score), '[]'::jsonb))
    into v_points
...
GRANT EXECUTE ON FUNCTION public.get_make_model_sentiment_points(text, text, integer, text) TO anon, authenticated;
