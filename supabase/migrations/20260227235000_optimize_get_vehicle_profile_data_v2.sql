-- Optimize get_vehicle_profile_data v2: handle extreme outlier vehicles
--
-- Problem found during verification (task 6d9cbe9f):
-- Vehicles with 1K+ images (1,597 vehicles, ~3% of DB) still hit the 3s anon timeout.
-- Root cause: The 2.8s execution time is dominated by:
--   1. json_agg of 200 full-column image rows (~1.5s for serialization)
--   2. row_to_json(v.*) across 150+ column vehicle row (~0.7s)
--   3. stats subqueries (5x COUNT queries) (~0.5s)
-- The function passes under light load but fails under DB load for heavy vehicles.
--
-- Fixes applied:
-- 1. Reduce images LIMIT 200 → 50 (profile initial load shows ~24 images; 50 covers gallery)
-- 2. Remove redundant ORDER BY inside json_agg (inner subquery already ordered)
-- 3. Replace filtered image_count (COALESCE-heavy, heap scan) with simple total count
--    using index-only scan on vehicle_id — 57ms → <10ms on heavy vehicles
--
-- Separately filed: data quality task to deduplicate vehicles with 1K+ duplicate images
...
SECURITY DEFINER
SET search_path TO ''
SET statement_timeout TO '8s'   -- override anon role's 3s default; function has internal LIMITs
AS $$
DECLARE
  result json;
BEGIN
  SELECT json_build_object(
    ...
    'stats', json_build_object(
      -- Simple count uses index-only scan, no heap fetch needed
      'image_count', (
