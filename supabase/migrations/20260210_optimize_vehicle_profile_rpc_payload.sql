-- Optimize get_vehicle_profile_data: select only needed image columns
-- Before: vi.* serialized 100+ columns per image → 1.5MB JSON for 428 images
-- After: 16 essential columns → 301KB JSON (80% smaller, 74x faster)
--
-- Performance: 1,124ms → 15ms execution time
...
-- Add composite index for vehicle page comment queries
CREATE INDEX IF NOT EXISTS idx_auction_comments_vehicle_source
ON auction_comments(vehicle_id, source_url);
