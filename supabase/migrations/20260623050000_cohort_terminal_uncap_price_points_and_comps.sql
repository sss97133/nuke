-- 20260623050000_cohort_terminal_uncap_price_points_and_comps.sql
-- Applied to prod 2026-06-23 via apply_migration (mirrors live). STOP CAPPING.
--
-- get_make_model_terminal.price_points was self-labeled "uncapped per-sale" yet
-- carried ORDER BY final_price LIMIT 1000; comps carried LIMIT 24. Both are the
-- full cohort sold set. The platform handles millions of points — surface every
-- sale (the client virtualizes/paginates). For the K5 Blazer model cohort this
-- lifts price_points 1000 -> 1138 and comps 24 -> 394.
--
-- Body == the live drifted def (date_exact recovery + cited-wins production from
-- the post-020000 hotfixes) with the two LIMIT clauses removed. Mirrors prod; do
-- not hand-edit to "improve" — pull the live def and re-sync if it drifts.
...
BEGIN
  ...
  -- price_points: the HONEST full set — every priced cohort sale, UNCAPPED.
  ...
  -- comps: full cohort sold set (UNCAPPED), priciest first. The client paginates/virtualizes.
  SELECT jsonb_build_object('populated', count(*) > 0, 'source', 'vehicle_events (cohort sold)', 'n', count(*),
    ...
    FROM public.vehicle_events ve JOIN public.vehicles v ON v.id = ve.vehicle_id
    WHERE ve.vehicle_id = ANY (v_ids) AND ve.final_price IS NOT NULL AND ve.final_price > 0
      AND COALESCE(ve.sold_at, ve.ended_at) IS NOT NULL ORDER BY ve.final_price DESC) c;
