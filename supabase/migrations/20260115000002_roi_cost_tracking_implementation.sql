...
-- Check if organization_vehicles table exists (correct table name)
DO $$
BEGIN
  IF EXISTS (
    ...
    DROP TRIGGER IF EXISTS trigger_auto_attribute_consignment_fee ON public.organization_vehicles;
    CREATE TRIGGER trigger_auto_attribute_consignment_fee
      AFTER INSERT ON public.organization_vehicles
      FOR EACH ROW
...
-- Function to link existing API usage to vehicles via images
CREATE OR REPLACE FUNCTION link_api_usage_to_vehicles()
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  -- Link API usage from image analysis to vehicles
  -- Uses api_usage_logs table (which has cost_cents, image_id, vehicle_id)
  INSERT INTO public.spend_attributions (
    vehicle_id,
    direction,
    amount_cents,
  ...
  SELECT DISTINCT
    COALESCE(aul.vehicle_id, vi.vehicle_id) AS vehicle_id,
    'outflow' AS direction,
    aul.cost_cents AS amount_cents,
    'overhead' AS spend_category,
    jsonb_build_object(
      'fee_type', 'platform_ai',
      'source', 'api_usage_logs',
      'usage_log_id', aul.id,
      'provider', aul.provider,
      'model_name', aul.model_name,
      'function_name', aul.function_name,
      'tokens_used', aul.tokens_used,
      'extracted_at', NOW()
    ) AS metadata,
    aul.user_id AS created_by
  FROM public.api_usage_logs aul
  LEFT JOIN public.vehicle_images vi ON vi.id = aul.image_id
  WHERE (aul.vehicle_id IS NOT NULL OR vi.vehicle_id IS NOT NULL)
    AND aul.cost_cents > 0
    AND aul.success = true
    -- Only link if not already attributed
    AND NOT EXISTS (
      SELECT 1 FROM public.spend_attributions sa
      WHERE sa.metadata->>'usage_log_id' = aul.id::TEXT
    )
  ON CONFLICT DO NOTHING;
...
COMMENT ON FUNCTION link_api_usage_to_vehicles IS 'Links existing API usage logs to vehicle spend attributions for platform/AI costs.';
...
-- ==========================
-- 7) MANUAL EXTRACTION RPC (Run on-demand)
-- ==========================

CREATE OR REPLACE FUNCTION extract_all_receipts_to_spend_attributions()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_receipts_extracted INTEGER := 0;
  v_items_extracted INTEGER := 0;
  v_api_usage_linked INTEGER := 0;
BEGIN
  -- Extract receipts (idempotent - only extracts new ones)
  INSERT INTO public.spend_attributions (
    vehicle_id,
    receipt_id,
    direction,
  ...
  SELECT
    ...
    FLOOR(COALESCE(r.total, 0) * 100)::BIGINT AS amount_cents,
    COALESCE(r.currency, 'USD') AS currency,
    categorize_receipt_spend(r.vendor_name, NULL, NULL) AS spend_category,
    CASE
      WHEN r.vendor_name IS NOT NULL THEN 'Extracted from receipt: ' || r.vendor_name
      ELSE 'Extracted from receipt'
    END AS notes,
  ...
  GET DIAGNOSTICS v_receipts_extracted = ROW_COUNT;
  ...
  GET DIAGNOSTICS v_items_extracted = ROW_COUNT;

  -- Link API usage
  SELECT link_api_usage_to_vehicles() INTO v_api_usage_linked;

  RETURN jsonb_build_object(
    'success', true,
    'receipts_extracted', v_receipts_extracted,
    'receipt_items_extracted', v_items_extracted,
    'api_usage_linked', v_api_usage_linked,
    'total_new_attributions', v_receipts_extracted + v_items_extracted + v_api_usage_linked
  );
END;
$$;

COMMENT ON FUNCTION extract_all_receipts_to_spend_attributions IS 'Manually triggers extraction of all receipts and API usage to spend_attributions. Idempotent - only processes new items.';

GRANT EXECUTE ON FUNCTION get_receipt_extraction_summary() TO authenticated;
GRANT EXECUTE ON FUNCTION link_api_usage_to_vehicles() TO authenticated;
GRANT EXECUTE ON FUNCTION extract_all_receipts_to_spend_attributions() TO authenticated;
