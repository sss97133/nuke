...
CREATE INDEX idx_vehicle_support_vehicle_id ON vehicle_support(vehicle_id);
CREATE INDEX idx_vehicle_support_supporter_id ON vehicle_support(supporter_id);
...
SELECT 
  ...
  ARRAY_AGG(DISTINCT supporter_id ORDER BY credits_allocated DESC) FILTER (WHERE NOT is_anonymous) as top_supporters
FROM vehicle_support
GROUP BY vehicle_id;

CREATE UNIQUE INDEX idx_vehicle_support_summary_vehicle_id ON vehicle_support_summary(vehicle_id);
...
CREATE INDEX idx_builder_payouts_user_id ON builder_payouts(user_id);
CREATE INDEX idx_builder_payouts_status ON builder_payouts(status);
