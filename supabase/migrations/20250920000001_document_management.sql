...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'document_privacy') THEN
...
DROP POLICY IF EXISTS "Public documents viewable by all" ON vehicle_documents;
CREATE POLICY "Public documents viewable by all" ON vehicle_documents
    FOR SELECT USING (privacy_level = 'public');

DROP POLICY IF EXISTS "Owner can view all their documents" ON vehicle_documents;
...
DROP POLICY IF EXISTS "Owner can manage their documents" ON vehicle_documents;
...
DROP POLICY IF EXISTS "Only owner can access sensitive data" ON document_sensitive_data;
...
DROP POLICY IF EXISTS "Owner can view document access logs" ON document_access_logs;
...
DROP TRIGGER IF EXISTS trigger_create_timeline_from_document ON vehicle_documents;

DROP FUNCTION IF EXISTS create_timeline_event_from_document();
