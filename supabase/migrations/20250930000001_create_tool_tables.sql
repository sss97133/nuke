...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE user_tools
      ADD COLUMN catalog_id UUID REFERENCES tool_catalog(id);
  END IF;
END;
$$;
...
DROP POLICY IF EXISTS "Anyone can view tool brands" ON tool_brands;
CREATE POLICY "Anyone can view tool brands"
  ON tool_brands FOR SELECT
  USING (true);

-- Tool Catalog: Public read, admin write  
DROP POLICY IF EXISTS "Anyone can view tool catalog" ON tool_catalog;
CREATE POLICY "Anyone can view tool catalog"
  ON tool_catalog FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Anyone can insert tool catalog" ON tool_catalog;
