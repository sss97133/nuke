...
DROP POLICY IF EXISTS "Vehicle owners and contributors can upload to storage" ON storage.objects;
    ...
    AND (
        ...
        (string_to_array(name, '/'))[2] IN (
            ...
            WHERE user_id = auth.uid() 
              ...
              AND role IN ('owner','co_owner','mechanic','appraiser','moderator','contributor','photographer','dealer_rep','sales_agent')
        )
    )
);
