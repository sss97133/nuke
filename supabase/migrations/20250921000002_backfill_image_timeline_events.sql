...
SELECT 
    ...
    'cosmetic' as event_category,
    'system_backfill' as source,
    'service_record' as source_type,
    CASE 
...
WHERE NOT EXISTS (
    ...
    SELECT 1 FROM timeline_events vte
...
BEGIN
    ...
    FROM vehicle_images vi
    JOIN vehicles v ON vi.vehicle_id = v.id
    JOIN timeline_events vte ON vte.vehicle_id = vi.vehicle_id
...
FROM vehicle_images vi
WHERE timeline_events.vehicle_id = vi.vehicle_id
AND timeline_events.event_type = 'image_upload'
AND timeline_events.metadata->>'image_url' = vi.image_url
AND timeline_events.metadata->>'image_id' IS NULL;
...
BEGIN
    ...
    FOR vehicle_record IN 
        ...
        LEFT JOIN timeline_events vte ON v.id = vte.vehicle_id 
            AND vte.event_type = 'image_upload'
