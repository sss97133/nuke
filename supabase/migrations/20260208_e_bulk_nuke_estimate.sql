-- Bulk Nuke Estimate computation
-- Computes base price from comps + basic signals entirely in SQL
-- for massive throughput. Edge function can enrich later with full 8-signal model.
  ...
  estimates AS (
    SELECT
      ...
      CASE
        ...
        WHEN ((cp.median_price - COALESCE(cp.asking_price, cp.current_value)) / cp.median_price * 100) >= 20 THEN 'steal'
        WHEN ((cp.median_price - COALESCE(cp.asking_price, cp.current_value)) / cp.median_price * 100) >= 10 THEN 'good_deal'
        WHEN ((cp.median_price - COALESCE(cp.asking_price, cp.current_value)) / cp.median_price * 100) >= -5 THEN 'fair'
        WHEN ((cp.median_price - COALESCE(cp.asking_price, cp.current_value)) / cp.median_price * 100) >= -15 THEN 'overpriced'
        ELSE 'way_overpriced'
