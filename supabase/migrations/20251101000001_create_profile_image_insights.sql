...
DO $$
BEGIN
    IF to_regclass('public.profile_image_insights') IS NULL THEN
        RAISE NOTICE 'Skipping profile_image_insights RLS setup: table does not exist.';
        RETURN;
    END IF;

    EXECUTE 'ALTER TABLE public.profile_image_insights ENABLE ROW LEVEL SECURITY';

    EXECUTE 'DROP POLICY IF EXISTS "Users view own image insights" ON public.profile_image_insights';
    EXECUTE 'DROP POLICY IF EXISTS "Service role upserts image insights" ON public.profile_image_insights';
    EXECUTE 'DROP POLICY IF EXISTS "Service role updates image insights" ON public.profile_image_insights';
