...
ALTER TABLE businesses
  ...
  ADD COLUMN IF NOT EXISTS stock_symbol TEXT UNIQUE,
...
CREATE INDEX idx_org_contributors_org ON organization_contributors(organization_id);
CREATE INDEX idx_org_contributors_user ON organization_contributors(user_id);
...
CREATE INDEX idx_org_ownership_org ON organization_ownership_verifications(organization_id);
CREATE INDEX idx_org_ownership_user ON organization_ownership_verifications(user_id);
CREATE INDEX idx_org_ownership_status ON organization_ownership_verifications(status) WHERE status IN ('pending', 'human_review');
...
-- Enhance business_timeline_events if exists, or create new
...
CREATE INDEX idx_org_images_user ON organization_images(user_id);
CREATE INDEX idx_org_images_gps ON organization_images(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
...
CREATE INDEX idx_org_vehicles_org ON organization_vehicles(organization_id);
CREATE INDEX idx_org_vehicles_vehicle ON organization_vehicles(vehicle_id);
...
CREATE INDEX idx_org_offerings_org ON organization_offerings(organization_id);
CREATE INDEX idx_org_offerings_symbol ON organization_offerings(stock_symbol);
CREATE INDEX idx_org_offerings_status ON organization_offerings(status) WHERE status = 'active';
...
CREATE INDEX idx_org_holdings_holder ON organization_share_holdings(holder_id);
CREATE INDEX idx_org_holdings_offering ON organization_share_holdings(offering_id);
...
CREATE INDEX idx_org_orders_offering ON organization_market_orders(offering_id);
CREATE INDEX idx_org_orders_user ON organization_market_orders(user_id);
CREATE INDEX idx_org_orders_active ON organization_market_orders(status) WHERE status IN ('active', 'partially_filled');
...
CREATE INDEX idx_org_trades_offering ON organization_market_trades(offering_id);
CREATE INDEX idx_org_trades_buyer ON organization_market_trades(buyer_id);
CREATE INDEX idx_org_trades_seller ON organization_market_trades(seller_id);
CREATE INDEX idx_org_trades_time ON organization_market_trades(executed_at);
...
CREATE TABLE IF NOT EXISTS organization_etf_holdings (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE (etf_offering_id, underlying_organization_id)
);

CREATE INDEX idx_etf_holdings_etf ON organization_etf_holdings(etf_offering_id);
  ...
  IF NEW.latitude IS NOT NULL AND NEW.longitude IS NOT NULL THEN
    ...
    LOOP
      ...
      VALUES (
        ...
        'work_location',
        true,
        GREATEST(0, LEAST(100, (1 - (nearby_org.dist_km / 0.5)) * 100)),  -- confidence decays with distance
        NEW.user_id
      )
...
BEGIN
  ...
  SET
    total_events = (
      ...
      WHERE business_id = NEW.business_id OR business_id = NEW.organization_id
    ),
    total_images = (
      SELECT COUNT(*) FROM organization_images
      WHERE organization_id = NEW.business_id OR organization_id = NEW.organization_id
    ),
    total_vehicles = (
