...
-- Remove legacy constraint if it exists
ALTER TABLE public.vehicle_drafts
  DROP CONSTRAINT IF EXISTS max_drafts_per_user;

-- Enable RLS
ALTER TABLE public.vehicle_drafts ENABLE ROW LEVEL SECURITY;

-- RLS Policies
DROP POLICY IF EXISTS "Users can view own drafts" ON public.vehicle_drafts;
CREATE POLICY "Users can view own drafts" ON public.vehicle_drafts
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own drafts" ON public.vehicle_drafts;
CREATE POLICY "Users can insert own drafts" ON public.vehicle_drafts
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own drafts" ON public.vehicle_drafts;
CREATE POLICY "Users can update own drafts" ON public.vehicle_drafts
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own drafts" ON public.vehicle_drafts;
CREATE POLICY "Users can delete own drafts" ON public.vehicle_drafts
  FOR DELETE USING (auth.uid() = user_id);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_vehicle_drafts_user_id ON public.vehicle_drafts(user_id);
CREATE INDEX IF NOT EXISTS idx_vehicle_drafts_updated_at ON public.vehicle_drafts(updated_at DESC);

-- Function to auto-update updated_at
DROP FUNCTION IF EXISTS update_vehicle_draft_timestamp();
...
DROP TRIGGER IF EXISTS update_vehicle_drafts_timestamp ON public.vehicle_drafts;
