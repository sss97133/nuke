...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'select_work_sessions_by_org_or_self' AND tablename = 'work_sessions' AND schemaname = 'public'
  ) THEN
    EXECUTE 'DROP POLICY "select_work_sessions_by_org_or_self" ON public.work_sessions';
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'insert_own_work_sessions_with_membership' AND tablename = 'work_sessions' AND schemaname = 'public'
  ) THEN
    EXECUTE 'DROP POLICY "insert_own_work_sessions_with_membership" ON public.work_sessions';
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'update_own_work_sessions' AND tablename = 'work_sessions' AND schemaname = 'public'
  ) THEN
    EXECUTE 'DROP POLICY "update_own_work_sessions" ON public.work_sessions';
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'delete_own_work_sessions' AND tablename = 'work_sessions' AND schemaname = 'public'
  ) THEN
  ...
  EXECUTE 'CREATE POLICY "select_work_sessions_by_org_or_self" ON public.work_sessions
    FOR SELECT
    USING (
      auth.uid() = user_id
      OR EXISTS (
  ...
  EXECUTE 'CREATE POLICY "insert_own_work_sessions_with_membership" ON public.work_sessions
    FOR INSERT
    WITH CHECK (
      auth.uid() = user_id
      AND EXISTS (
  ...
  EXECUTE 'CREATE POLICY "update_own_work_sessions" ON public.work_sessions
    FOR UPDATE
    USING (auth.uid() = user_id)
    WITH CHECK (auth.uid() = user_id)';

  -- DELETE: only author can delete their sessions
  EXECUTE 'CREATE POLICY "delete_own_work_sessions" ON public.work_sessions
    FOR DELETE
    USING (auth.uid() = user_id)';
