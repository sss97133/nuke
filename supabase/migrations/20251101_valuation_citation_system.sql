...
CREATE INDEX idx_valuation_citations_vehicle ON valuation_citations(vehicle_id);
CREATE INDEX idx_valuation_citations_submitter ON valuation_citations(submitted_by);
CREATE INDEX idx_valuation_citations_component ON valuation_citations(component_type, component_name);
CREATE INDEX idx_valuation_citations_evidence ON valuation_citations(evidence_type);
CREATE INDEX idx_valuation_citations_tag ON valuation_citations(source_image_tag_id) WHERE source_image_tag_id IS NOT NULL;
CREATE INDEX idx_valuation_citations_doc ON valuation_citations(source_document_id) WHERE source_document_id IS NOT NULL;
...
CREATE INDEX idx_user_valuation_inputs_user ON user_valuation_inputs(user_id);
CREATE INDEX idx_user_valuation_inputs_vehicle ON user_valuation_inputs(vehicle_id);
CREATE INDEX idx_user_valuation_inputs_citation ON user_valuation_inputs(citation_id);
...
CREATE INDEX idx_valuation_blanks_vehicle ON valuation_blanks(vehicle_id);
CREATE INDEX idx_valuation_blanks_status ON valuation_blanks(status) WHERE status = 'open';
CREATE INDEX idx_valuation_blanks_priority ON valuation_blanks(priority) WHERE status = 'open';
...
CREATE INDEX idx_labor_rate_sources_shop ON labor_rate_sources(shop_id);
CREATE INDEX idx_labor_rate_sources_laborer ON labor_rate_sources(laborer_user_id);
CREATE INDEX idx_labor_rate_sources_region ON labor_rate_sources(region);
...
-- Add receipt linking to image_tags
ALTER TABLE image_tags
  ADD COLUMN IF NOT EXISTS receipt_line_item_id UUID REFERENCES receipt_items(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS labor_record_id UUID,  -- future: link to labor_records table
  ADD COLUMN IF NOT EXISTS part_installed_by UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS install_shop_id UUID REFERENCES shops(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS install_labor_hours NUMERIC(6,2),
  ADD COLUMN IF NOT EXISTS install_labor_rate NUMERIC(8,2);

CREATE INDEX idx_image_tags_receipt_item ON image_tags(receipt_line_item_id) WHERE receipt_line_item_id IS NOT NULL;
...
RETURNS TABLE(blank_id UUID, blank_type TEXT, component_name TEXT, impact_usd NUMERIC) AS $$
BEGIN
