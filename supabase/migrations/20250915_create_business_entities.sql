...
CREATE TABLE IF NOT EXISTS business_user_roles (
    ...
    -- Constraints
    UNIQUE(business_id, user_id, role_title) WHERE status = 'active'
);
...
CREATE TABLE IF NOT EXISTS business_vehicle_fleet (
    ...
    -- Constraints
    UNIQUE(business_id, vehicle_id) WHERE status = 'active'
);
...
CREATE INDEX idx_businesses_owner ON businesses(id);
...
CREATE INDEX idx_business_ownership_business ON business_ownership(business_id);
CREATE INDEX idx_business_ownership_owner ON business_ownership(owner_id);
...
CREATE INDEX idx_business_vehicle_fleet_business ON business_vehicle_fleet(business_id);
CREATE INDEX idx_business_vehicle_fleet_vehicle ON business_vehicle_fleet(vehicle_id);

CREATE INDEX idx_business_timeline_events_business ON business_timeline_events(business_id);
CREATE INDEX idx_business_timeline_events_date ON business_timeline_events(event_date);
...
RETURNS TRIGGER AS $$
BEGIN
    -- Create ownership record for the creator
