...
CREATE TABLE IF NOT EXISTS department_presets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_type org_type NOT NULL,
  department_type department_type NOT NULL,
...
-- Insert automotive business presets
INSERT INTO department_presets (business_type, department_type, preset_name, description, typical_roles, is_recommended, sort_order) VALUES
  -- Dealer presets
...
CREATE INDEX IF NOT EXISTS idx_shop_locations_hq ON shop_locations(shop_id, is_headquarters) WHERE is_headquarters = true;
...
CREATE INDEX IF NOT EXISTS idx_shop_departments_location ON shop_departments(location_id);
CREATE INDEX IF NOT EXISTS idx_shop_departments_parent ON shop_departments(parent_department_id);
...
SELECT 
  ...
  EXTRACT(DAY FROM (sl.expiration_date - CURRENT_DATE)) AS days_until_expiration
