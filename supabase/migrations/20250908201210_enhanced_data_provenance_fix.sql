...
ADD COLUMN IF NOT EXISTS verification_notes TEXT; -- Human verification notes -- Human verification notes
...
-- RLS Policies for vehicle_dynamic_data (idempotent)
DO $$
BEGIN
  -- Drop legacy/truncated names if present, then create concise policies
  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'vehicle_dynamic_data'
      AND policyname IN (
        'Users can view dynamic data for vehicles they own or that are public',
        'Users can view dynamic data for vehicles they own or that are p',
        'vehicle_dynamic_data_view')
  ) THEN
    EXECUTE 'DROP POLICY IF EXISTS "Users can view dynamic data for vehicles they own or that are public" ON public.vehicle_dynamic_data';
    EXECUTE 'DROP POLICY IF EXISTS "Users can view dynamic data for vehicles they own or that are p" ON public.vehicle_dynamic_data';
    EXECUTE 'DROP POLICY IF EXISTS vehicle_dynamic_data_view ON public.vehicle_dynamic_data';
  END IF;

  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'vehicle_dynamic_data'
      AND policyname IN (
        'Users can modify dynamic data for vehicles they own',
        'vehicle_dynamic_data_all')
  ) THEN
    EXECUTE 'DROP POLICY IF EXISTS "Users can modify dynamic data for vehicles they own" ON public.vehicle_dynamic_data';
    EXECUTE 'DROP POLICY IF EXISTS vehicle_dynamic_data_all ON public.vehicle_dynamic_data';
  END IF;

  EXECUTE 'CREATE POLICY vehicle_dynamic_data_view '
       || 'ON public.vehicle_dynamic_data FOR SELECT '
       || 'USING ( '
       || '  vehicle_id IN ( '
  ...
  EXECUTE 'CREATE POLICY vehicle_dynamic_data_all '
       || 'ON public.vehicle_dynamic_data FOR ALL '
       || 'USING ( '
       || '  vehicle_id IN ( '
...
-- RLS Policies for evidence_documents (idempotent)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'evidence_documents'
      AND policyname IN (
        'Users can view evidence for vehicles they own or that are public',
        'evidence_documents_view')
  ) THEN
    EXECUTE 'DROP POLICY IF EXISTS "Users can view evidence for vehicles they own or that are public" ON public.evidence_documents';
    EXECUTE 'DROP POLICY IF EXISTS evidence_documents_view ON public.evidence_documents';
  END IF;

  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'evidence_documents'
      AND policyname IN (
        'Users can modify evidence for vehicles they own',
        'evidence_documents_all')
  ) THEN
    EXECUTE 'DROP POLICY IF EXISTS "Users can modify evidence for vehicles they own" ON public.evidence_documents';
    EXECUTE 'DROP POLICY IF EXISTS evidence_documents_all ON public.evidence_documents';
  END IF;

  EXECUTE 'CREATE POLICY evidence_documents_view '
  ...
  EXECUTE 'CREATE POLICY evidence_documents_all '
