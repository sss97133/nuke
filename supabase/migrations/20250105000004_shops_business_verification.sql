...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'documentation_type') THEN
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_name = 'vehicle_timeline_events' AND table_type = 'BASE TABLE'
  ) THEN
ALTER TABLE vehicle_timeline_events
  ADD COLUMN IF NOT EXISTS shop_id UUID REFERENCES shops(id);
  ELSE
    RAISE NOTICE 'vehicle_timeline_events is a view, skipping shop_id column addition.';
  END IF;

  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_name = 'user_contributions' AND table_type = 'BASE TABLE'
  ) THEN
...
DROP POLICY IF EXISTS "Shop owners can view their verification requests" ON shop_verification_requests;
...
DROP POLICY IF EXISTS "Shop owners can create verification requests" ON shop_verification_requests;
...
DROP POLICY IF EXISTS "Shop members can view their shop documents" ON shop_documents;
...
DROP POLICY IF EXISTS "Shop owners can upload documents" ON shop_documents;
...
DROP POLICY IF EXISTS "Admins can view all shop documents" ON shop_documents;
...
DROP POLICY IF EXISTS "Admins can view all verification requests" ON shop_verification_requests;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_name = 'shop_documents' AND column_name = 'verification_request_id'
  ) THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_shop_documents_request ON shop_documents(verification_request_id)';
  END IF;
END;
$$;
